...
- **最新: ビルド23配信済み（2026-08-05早朝・version23/VALID裏取り済み）**。中身=深夜詰めラウンドW-1〜W-8: ツアー地図に仮サムネ・**ツアー1本目だけタップ可=YouTube往復練習**(復帰検知で「おかえり！」→記録練習合流・Q-4の部分改訂)・グラデ8箇所彩度立て・使い方ピル白+濃枠・placeholder全数棚卸し・**押下ハロー13%%実装=P-9完結**(P-8はグロー消滅を本人実機確認済み・完全クローズ)・スプラッシュWeb版同等(850ms)・**ダーク階層化d1**(子面トークン・選択ノブ黄文字・設定画面含む・よびな欄の白面バグ修正)。
