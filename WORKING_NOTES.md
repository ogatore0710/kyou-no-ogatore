...
- **`TASK-C2-2026-07-30-icon-system-addendum-chips.md`（進行中・TaskList #2 #5）**:
  部位選択（腰・肩など）・時間帯選択（朝/夜/お風呂上がり）のイラスト化。
  ...
  実物。squeeze-hunterのアンカーは使わない）。出力は`.art-staging/`（作業用）→
  alan5が実際のchip背景色に合成して目視確認→OK後に`assets/`へ、の順。
  **腰チップv1は線が波打っていてタブバーと質感が合わず差し戻し・v2（幾何学的な人型シルエット
  +黄色ブロック）でOK。iOS実装待ちの状態で中断**（次の自分はここから: iOS実装→両OS確認→
  残り8箇所へ）。
