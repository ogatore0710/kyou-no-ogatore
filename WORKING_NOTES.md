...
## 📮 alan5 → alan5 自己引き継ぎメモ（代替わり直後の自分はここを最初に読む・最終更新 2026-08-09）

### 🛌 2026-08-09 休眠チェックイン: Monitor死亡→張り直し

起動して現状確認したところ、旧Monitor（PID 12829系）のプロセスは`ps aux`に見当たらず停止していた（確認コマンド: `ps aux | grep 12829`該当なし・原因未確認、Mac再起動等の可能性）。役目③に基づき同じ読み取り専用fetch版（168行目以降のスクリプト）を張り直し済み（新task id=`bwbyuos3j`・persistent・description=`appdev commits (read-only fetch) + even-sync conflict warnings on kyou-no-ogatore`）。他は静観・待機に戻る。**最新地点はbuild37配信済み(2026-08-08・下の⚡節参照)。appdevとの直接ラリーは平常運行中とみなし介入なし。**

### 🛌 2026-08-06 役割変更: alan5は「休眠頭」へ（本人GO・監督裁定）

**普段のラリーは本人⇔appdev(C2)直接に切り替わった**（C2 Fable余剰/C1節約・スクイーズ方式）。alan5の残る役目: ①このメモの管理 ②節目の独立検収のみ（ストア提出前・大型リリース前に監督か本人から声がかかる） ③常設Monitor（読み取り専用fetch・PID 12829系）維持 ④普段は静かに待機。検収文化（qa全走・実描画・ASC裏取り・根拠併記）はappdevへ引き継ぎ済み（`HANDOFF-alan5-to-appdev-2026-08-06.md`参照）。
...
### 🛑 2026-08-10夕 alan5、休眠頭→窓口へ復帰（appdev休止引き継ぎ受領）

監督・appdev両方から引き継ぎ着信を受領。対応済み: ①C3 bulkdevへ完了報告の受け口をappdevからalan5へ変更する旨をdoor-deliver.shで配達済み(`OK: 7920.bulkdev 配達・処理開始を確認`・報告ファイルパス/検収基準/書込許可範囲は発注書のまま無変更) ②`ogatore-hub/BALLS.md`の本人ボール2件(build40実機確認/緑残存4箇所裁定)の回答先案内を「appdevセッションへ」から「alan5セッションへ」に修正済み。bulkdev総監査の報告ファイル(`ogatore-hub/2026-08-10-C3bulkdevからappdevへ報告-ストア提出前総監査.md`)は本メモ確認時点(2026-08-10)で未作成=未完了(根拠: `ls`該当なし)。以後この現在地セクションの直下に検収記録を追記していく。役割: 本人窓口・設計・調整・検収を巻き取り、実装が要る案件はC3 bulkdevへ発注書で回す。appdev再開目安は金曜16時C2リセット後・本人判断(appdevからの伝聞・未確認扱い)。

### 🔍 2026-08-10夜 C3総監査(18件)を検収・BALLS.mdで裁定伺い中

bulkdevの報告書(`ogatore-hub/2026-08-10-C3bulkdevからappdevへ報告-ストア提出前総監査.md`・13KB)を受領。**Sランク0件・A級4件・B級6・C級8**。鵜呑みにせず主要3件を自分でgrep/Read検収済み: A-1(iOS `SettingsView.swift:114-116`=丸めた値を`_icsMinute`という@Stateへ入れるだけでstore.set無し・コメントは「表示・保存とも丸める」と書いていながら保存コードが実在しないことを確認)/A-2(`RecordCore/RecordStore.swift:101-110`=`persist()`が`try? data.write(...options:.atomic)`で失敗を握りつぶし・`set()`は121行で無条件`return true`、`grep -rn fsync ios-native/`=0件を確認)/A-4(`voices.json:125,293,657`に「治った」「劇的に改善」「完璧に治りました」の完治断定引用を確認)。**3件とも報告どおりで実在**。qa.js463/Android unit276/iOS RecordCore5件のPASSはbulkdev実走・実描画は未実施と明記(報告書に自己申告あり・信頼できる形式)。
...
### ✅ 2026-08-20 第2ラウンド報告受領・検収→voices.json追加修正の方針確認

factcheckの報告を受領。柱1(build41/42のA-1/A-2/B-3+ツアー不算入+図鑑ダーク)は全件「発注どおり・リグレッションなし」・柱3(前回18件棚卸し)は対応済み3/現存15・格上げ該当なし。**最大の新事実=柱2 R2-A1**: voices.json全113件走査で前回未走査だった高リスク完治断定11件・中リスク13件・身長増加系(生理的に非現実的な誇大表現)2件を発見。alan5がL27/L34/L139/L153/L475/L608/L636/L776/L643/L699の10行を実物grepで確認(報告の代表例と完全一致)。

本人へAskUserQuestionで対応範囲を確認→**「高リスク11件+身長2件=13件を修正」で決定**(中リスク13件は今回見送り)。ただし報告書には高リスク11件のうち8件しか具体的行番号が書かれておらず、残り3件が特定できないため、factcheckへ完全な行番号リストを追加依頼(新規監査ではなく既存結果からの抽出のみ・配達済み)。回答が来次第、build41 A-4と同型(index.html最小編集→本人の都度承認→gen-voices.mjsで3ファイル再生成)でbulkdevへ発注する。

### 📋 2026-08-20 voices.json高リスク13件の差し替えを発注

factcheckから完全リスト回答を受領(残り3件=L405「接骨院とはさようなら」/L468「寝違え…これらがなくなりました」/L629「腰痛ちゃんと別れました」)。alan5がL405・L629を実物grepで確認(L468は既に確認済み)。**13件全件の実在確認完了**(L27/34/139/153/405/468/475/608/629/636/776+身長L643/699)。

発注書=`ogatore-hub/2026-08-20-alan5からC3bulkdevへ発注-voices高リスク13件差し替え.md`をbulkdevへ配達済み(`OK: 7920.bulkdev 配達・処理開始を確認`)。**index.html編集の承認は本人が対応範囲を決めたAskUserQuestion(2026-08-20・編集方式=index.html最小編集→gen-voices.mjs再生成を明記した上でのGO)を今回分の承認として扱った**(build41 A-4のような単独の追加確認は省略)。各行に文言案(例)を添えたが「完治・症状消失・通院不要・非現実的な身長増加の断定を弱める」狙いを外さなければbulkdevの言い換え裁量を許容。中リスク13件(L48/69/97/195/312/531/538/559/587/601/622/727/783・行番号のみ判明)は今回見送りのまま。報告先=`ogatore-hub/2026-08-20-C3bulkdevからalan5へ報告-voices13件差し替え.md`。

### ✅ 2026-08-20 voices13件差し替え完了報告を検収→本人向け報告を用意

bulkdevの完了報告(commit`f914441`・13行ペア=26行diff・md5一致・qa.js exit0)を受領し、alan5が自分でも検収: `git show --stat f914441`で変更ファイルが報告どおり(WORKING_NOTES/voices.json×3/index.html)・index.html diff行数28(ヘッダ込み26行+コンテキスト)を確認。3ファイルmd5=`031294fc...`一致を自分のmd5コマンドで再確認。`node scripts/qa.js`を自分で再実行=463項目exit0。index.html内の実テキストをgrepで8件直接確認(だいぶらくになりました/気にならなくなり/伸びた気がします等)=報告のbefore/after表と一致。**問題なし・出荷対象**。

監督より「本日の大放出成果を本人が順に訪れるので詳細報告を用意して待機」の指示を受け、`REPORT-2026-08-20-honnin-houkoku.md`を作成(①全体像②発見ハイライト2件(voices13件発見の経緯・build41/42健全性の裏取り)③判断事項2件(build43出荷GO可否・中リスク13件の扱い)④Tailnetリンク)。本人来訪時にすぐ提示できる状態で待機。

### 📬 2026-08-20夜 監督直発注6本の予告(FYI)
...
- **本人ボール2件=`ogatore-hub/BALLS.md`記帳済み**: ①build40実機確認4点(初回ゲート/ツアー静けさ/立体ボタン/ダーク新基調) ②緑の残存4箇所の消す/残す裁定(検索カテゴリチップb系統/オンボ緑吹き出し/相談室パレット緑/Mintグラデ)。
- **C3 bulkdevへ「ストア提出前・総監査」発注済み**(2026-08-10・本人GO・監督指示)。発注書=`ogatore-hub/2026-08-10-appdevからC3bulkdevへ発注-ストア提出前総監査.md`(対象HEAD=ed2656a6・4本柱・書込許可=hub側報告ファイル1本のみ)。配達確認ログ「OK: 7920.bulkdev 配達・処理開始を確認」。**報告先=`ogatore-hub/2026-08-10-C3bulkdevからappdevへ報告-ストア提出前総監査.md`・完了連絡はdoor-deliver.sh appdev宛の取り決め**→appdev休止中のため、届いた報告の受領・検分・修正判断の采配は**alan5(C1頭)が受け口**。appdevに着信した場合は受領記帳とalan5転送のみ行い修正作業はしない。
- **ストア提出までの残作業**(2026-08-06合意の棚卸し+その後の追加): ①Android実機通し確認 ②App Storeメタデータ一式(スクショ/説明文/ASCプライバシー質問=収集なし申告。ロゴはPIL+同梱フォントの型で素材再利用可=build37節参照) ③ストア前のalan5独立検収 ④C3総監査の指摘消化。技術負債=targetSdk 36前のインセット対応(R-64・監査報告参照)。
- 検収規律(qa.js全走+実描画目視+ASC API裏取り+根拠併記)と検証ナレッジは下の各⚡節・`HANDOFF-alan5-to-appdev-2026-08-06.md`のとおり。仕掛かり中の実装はゼロ(build40出荷でラウンド完結・未コミット作業なし。根拠: `git status`=WORKING_NOTES本ファイルの記帳差分のみ)。
- **【追記 2026-08-10 19時頃】C3総監査の報告受領済み**: `ogatore-hub/2026-08-10-C3bulkdevからappdevへ報告-ストア提出前総監査.md`(13KB・実在確認済み)。要点=Sランク(提出ブロッカー)0件・A級4件(①iOS合図の表示時刻と実通知の乖離・自己修復なし ②iOS persist失敗握りつぶし+fsync欠如 ③Android全書き込みメインスレッドfsync ④voices.json「治りました」断定引用=審査1.4.1リスク)・B級6・C級8・裁定待ち4項は報告書末尾。qa.js463全緑+Android unit276全緑+iOS RecordCore 5件PASSはC3実走確認・実描画は未実施と明記あり。**休止取り決めどおりappdevは受領記帳とalan5転送のみ実施・修正判断はC1司令塔の采配待ち。**
...
- **R-58(最重要)**: 記録全消失シナリオ恒久対策。Androidをアトミック書き込み(tmp+fsync+rename)化、両OSにサルベージ(生bool/null自己修復)+破損隔離(.corrupt-*、2個保持)+隔離失敗時のみpersist封印。新規ユニット11本(途中切れ→保全の直接証明)。Fable設計レビュー裁定: AtomicFile不採用(素のJVMテスト維持)・自動復元不採用(サルベージが実質それ)・通知なし(静かなUX)。
- R-59 iOS SafetyCore絵文字4箇所撤去(build16取り残し・KBの63個はWeb原典由来で対象外) / R-60 Androidボタン族padding縦横逆5箇所(Web=縦16横18) / R-61 折りたたみトグル10箇所をKyonoFoldToggleRow共通部品化(isButton+開閉状態読み上げ) / R-62 深夜記録でリマインド1回欠落(両OS・判定を発火時刻のアプリ日へ) / R-63 iOSオンボをObAsyncChannel(CONFLATED相当)化(高速タップ無反応+リーク解消・実機4設問即タップ通過=verify 132-133)。
- **B群**: R-64 targetSdk/compileSdk 35(edge-to-edgeはvalues-v35のwindowOptOutEdgeToEdgeEnforcementで従来維持=**targetSdk 36前に要インセット対応の技術負債**。Robolectric 4.14.1へ)。R-65 upload keystore生成→`~/Claude/ogatore-hub/secrets/`保管(このリポ厳禁=auto-sync公開)・signingConfigs存在チェック付き・apksigner/AAB検証済み。R-66 **完了**: プライバシーポリシー公開済み=https://kyou-no.ogatore.net/privacy.html (運営者=株式会社OGATORE・本人GO・実照会200)+ASCのappInfoLocalization(ja)へURL登録済み(PATCH 200)。PlayConsole側はストア登録作業時に同URLを使用。**注意**: pages.ymlはallowlist方式のため新規公開ファイルはワークフローに追加が要る(今回踏んだ)。
- **build36出荷済み**(VALID/紐付け204/IN_BETA_TESTING/whatsNew200/Push1/1)。**出荷時の学び**: R-63初版(AsyncStreamイテレータをクラスプロパティに持つ形)がReleaseアーカイブ時のみswift-frontend 6.3.3をクラッシュさせた(EarlyPerfInlinerの暗黙deinit処理・Debug/シミュレータでは無症状)→イテレータをasync関数ローカルへ移して回避。アーカイブはDebugビルドと別物として検証すること。**iPad裁定済み(2026-08-07)**: iPhoneのみ(R-67・TARGETED_DEVICE_FAMILY=1×4config・本人裁定)。iPadスクショ提出・iPad審査は不要になった。これで監査B群の本人ボールは全消化。ストア提出の残りはメタデータ作業(スクショ/説明文/ASCプライバシー質問=収集なし申告)とストア前のalan5独立検収。C群(中粒)/D群(掃除+qa.jsガード)は監査報告参照。
...
- **最新: ビルド29配信済み（2026-08-05 23:46・version29/VALID/紐付け204/whatsNew200裏取り済み・本日計7本=23〜29・ラウンド7つ全消化）**。中身=ラウンド7: R-19 ツアー練習ブロックの文字削除（R-2のピル+2行は1日で引退・ボタンのみ残す） R-20 つづけた日数の数字black900（bananaフォントは細字専用と判明・56pt/pinkInk据え置き） R-21 みどころ3枚に実画面ミニモック（相談室/通信/マイ記録・KyonoTourMockups流儀・見出し⇔絵一致を検収） R-22 チャットチップ文字black900。**本日の教訓**: ①本人の「太く/はっきり」系指摘が続いた=このアプリの文字は基本black900に寄せるのが本人の好み ②スクショ画素置換モックの型が確立（chip-recolor2.py系・固定ジオメトリ+縁は形状全塗り+fill閾値35） ③動画指摘はffmpegフレーム分解で座標と正体を掴んでから発注 ④文言は本人が2回校正してくる前提で仮案を出す。
- 旧: ビルド28配信済み（2026-08-05 22:39・version28/VALID/紐付け204/whatsNew200裏取り済み・本日6本目=23〜28）。中身=ラウンド6: R-16 起動を常時ライト固定（本人裁定。端末ダーク設定でも暗い段階ゼロをフレーム分解で確認・Androidは元々ライト固定で無変更） R-17 **相談室送信ボタン縦化の根治**（正体=7/31の折り返し許可×固定幅90pt×もじの大きさ「おおきめ」の潜在バグ・ビルド退行ではない・appdevが実機再現で立証。KyonoPrimaryButtonにsingleLine新設・固定幅撤去・**qa.jsに再発防止検査追加=462項目化**。教訓: 「おおきめ」設定での検証が漏れていた→実描画検証は文字サイズ両設定で） R-18 ツアー中カード前後の余計な瞬間2箇所省略（本人動画をffmpegフレーム分解→2箇所特定→本人確認GO→即カード/閉じたら即step5・通常ユーザー演出は不変）。**本人は22時半すぎ「1度待機します」で本日ラウンド終了**。残ボール: ①ビルド24〜28の実機まとめ確認待ち ②練習記録の実カウント算入と「1日目の記録をつけにいく」文言の整合（申し送り済み・返事待ち） ③Android実描画検証はストア前宿題のまま。
- 旧: ビルド27配信済み（2026-08-05 18:39・version27/VALID/紐付け204/whatsNew200裏取り済み・本日5本目）。中身=ラウンド5: R-10 とどくメーター2行化（本人文言・いたみ注意行はカードから削除=申し送り済み） R-11 **起動LaunchScreenをアプリ内スプラッシュと同一図柄に**（「一瞬映る文字なし画面」解消・alan5が動画をffmpegフレーム分解して原因特定→ImageRendererでSwiftUI実物をPNG化する実装・ライト/ダーク両対応・Androidは#ロゴのみ=API制約） R-12 ツアー中ペースの目安カード非表示（本人赤ペン） R-13 ツアー練習カード「0日目！」+保存シェア案内（**練習の実カウント算入は既存挙動のまま=本人へ申し送り済み・返事待ち。「1日目の記録をつけにいく」ボタンとの文言同居も指摘済み**） R-14 ツアー終了ポップ文言（本人指定） R-15 **選択肢チップの意味リンク配色**（本人裁定案①硬い=赤・柔=緑・悩み/時間帯も確定・colorKey方式でi%n巡回をフォールバック化・新色purple/neutral両テーマ・Q1-Q4は既に意味順なので不変）。モックv1で縁残り/キャラ絵食われ→自分の検分で差し戻しv2で解消（教訓: 画素置換は固定ジオメトリ+縁は形状で全塗り+fill閾値35でキャラ絵保護）。
- 旧: ビルド26配信済み（2026-08-05 16:18・version26/VALID/紐付け204/whatsNew200裏取り済み）。中身=ラウンド4: R-6 動画復帰後の練習ブロック二重表示解消（!showDoneNudgeガード・本人赤ペン指摘） R-7 練習ピルのピンク化16pt+ふわふわ±4pt/1.6s（reduceMotion静止・モック2往復+GIFで本人合意・1本目カードhero化・案内行明示改行） R-8 ホームのセグメント下間隔 R-9 **ライト背景ピーチ#FAEDE2**（4候補モック→本人裁定案c・line玉突き#EEDECE・bg/lineトークン一元管理をgrep確認・スプラッシュ#FFFAF3は独立で不変）。**モック手法が定着**: 本人スクショの画素置換で色裁定→本人GO→発注の型（scratchpadのchip-recolor2.py/grad-mock.py/pink-mock.py系）。進行中=ビルド27発注済み: R-10 とどくメーター文言ダイエット（本人指定2行・医療注意行はカードから消える旨を本人へ申し送り済み・返答待ち）。
- 旧: ビルド25配信済み（2026-08-05 14:07・version25/VALID/紐付け204/whatsNew200裏取り済み）。中身=本人ラウンド3: R-3 おすすめ3本の短タイトルst化（VideoRowにuseShortTitle新設・既定false・結果画面2箇所のみtrue。**検索タブはフルタイトル維持=本人裁定確定**） R-4 動画をひらく練習ピル（ツアー中のみ・文言は本人2回校正の最終版「＼ 動画をひらく練習 ／」+「今は1本目だけタップできるよ！動画をひらいてもどってきてね！」） R-5 結果カードグラデ桃ひと系統（.soft=#FFF3F7→#FFD6E4・soft使用は結果カード1箇所のみgrep棚卸し済み・モック3案から本人裁定案b）。モック手法はビルド24と同じスクショ画素置換（grad-mock.py=グラデ版）。
- 旧: ビルド24配信済み（2026-08-05昼・version24/VALID/紐付け204/whatsNew200裏取り済み）。中身=本人起床後ラウンド2件: R-1 選択肢ボタンのはっきり塗り化・案A'（obgLight/OBG_LIGHTを高彩度bg+ink文字に刷新・縁とダーク据え置き・QuizOptionCardのnote行を段階色カード限定でink化。裁定経緯=実寸モック3案→案A→本人「黄CTAくらいはっきり」→A'確定。モック手法=実機スクショの画素置換・スクラッチパッドchip-recolor2.py） R-2 ツアー記録練習の誘い文を練習ピル「＼ きろくの れんしゅう ／」+優しい2行に（本人生指摘「全然優しくない」対応・ボタンと挙動は不変）。検収=パレット値grep両OS・qa.js461自走・実描画13枚（不足のQ1選択肢4枚全部写しは追加依頼して取得）。
- 旧: ビルド23配信済み（2026-08-05早朝・version23/VALID裏取り済み）。中身=深夜詰めラウンドW-1〜W-8: ツアー地図に仮サムネ・**ツアー1本目だけタップ可=YouTube往復練習**(復帰検知で「おかえり！」→記録練習合流・Q-4の部分改訂)・グラデ8箇所彩度立て・使い方ピル白+濃枠・placeholder全数棚卸し・**押下ハロー13%%実装=P-9完結**(P-8はグロー消滅を本人実機確認済み・完全クローズ)・スプラッシュWeb版同等(850ms)・**ダーク階層化d1**(子面トークン・選択ノブ黄文字・設定画面含む・よびな欄の白面バグ修正)。
- 旧: ビルド22（2026-08-04夜）。中身=本人の詰めラウンド一括: 黄ボタン復帰(案B=#FFD93B+文字#26261F+縁#8A6D00・**藍D2は1ビルドで引退**・トークン撤去済み)・案B全展開・初回チャット選択肢の濃色化(5色実測表あり)・カード間隔+7pt・**ライト背景#F7EEDC**(line=#EBDFC8に玉突き調整)・ダーク磨き(bg#1C1915?報告参照・card分離1.42:1・構造枠7.7:1、**カテゴリ識別色の枠は据え置き=台帳**)・つづけた日数再設計(通算全廃・数字主役・メモ1行合体)・ホーム相談チップ削除・図鑑1枠化。配色の経緯: 濃地白文字D2(ビルド21)→本人実機で黄色回帰。**色の裁定は必ず実機確認後に確定と心得よ**。
- 旧: ビルド21(2026-08-04夜・藍D2)・ビルド20(同午後)。中身=①配色刷新D2（本人の本家色: 主ボタン藍#073A71×白・ダークは#2278C0を実測選定・青#3E70F5は枠役・黄は面に使わない） ②Y-1〜Y-6（よびなピル余白/「きょうの◯◯用」行削除/ツアー図の画像フォールバック=GPT生成絵待ち・指示書=art-raw/gpt-prompt-tour-map-illust.md/図鑑ちら見せ4枚/カレンダー登録削除=EventKit後始末込み/文字サイズふつう1.08・おおきめ1.30）。宿題: ダーク枠線1.72:1（appdev発見・未着手）。フォントはM PLUS 1p維持（本人裁定・比較表提示済み）。
- 旧: ビルド20配信済み（2026-08-04午後）。中身=①ホーム動画カードの引き算（サムネ+タグ+短タイトル`st`。**st142本は本人と4ラウンド往復して確定**: ねらい型→○○分ストレッチ等の種別語形式→分数はタイトル表記優先→『超』硬い人向け看板復活・衝突組は代表版のみ。正本=両OSの`catalog.json`・レビュー経緯=`REPORT-C2-2026-08-04-build20-st-*.md`群） ②ツアー2段構え（初回=地図+予告3枚／使い方タブ再生=フル7枚・バー非表示） ③よびな設定（6文字・端末内のみ・「あなた」置換は最小4箇所） ④通知セクション化（許可切れ案内つき） ⑤白文字入力バグ修正（文字色の全数棚卸し）。**環境ナレッジ: AndroidのJDK/SDKは`~/android-toolchain/`**（java_home/Homebrewには無い。「JDKが無い」騒動をalan5がgradleデーモンログから解決・2026-08-04）。**Android実描画検証はストア提出前の宿題として持ち越し中**。
- 旧: ビルド19（2026-08-04未明）。中身=ツアー再設計・案2「体験一本道＋予告3枚」（本人カード裁定）: 再説明4枚削除・予告3枚（相談室/通信/マイ記録）＋締めに「忘れても」吸収・進捗バー5段統合（チェック/けっか/きろく/カード/みどころ）・ボタン列再構成（黄1本＋テキストリンク）・縦中央寄せ・字組み引き締め。**T-1=ビルド18の実バグ**（7枚化でKyonoTourMockupsのswitchが8枚時代のまま＝3枚目以降の絵ズレ。Fable設計官2回目が発見・alan5検収漏れ）→修正済み・**以後「見出し⇔絵の一致」を全ビルドの恒久検収項目とする**。Fable設計官の診断全文はこの発注の元=`TASK-C2-2026-08-04-build19-tour-redesign.md`冒頭参照。
- 旧: ビルド18（2026-08-03夜）。中身=①テーマ混在バグ根治（ResultViewだけ`themeSetting:"auto"`直書きだった→store参照に統一・リテラル全数grep 0件） ②Fable監査（本人指示・1体139k tokens・コード実測つき）由来のチュートリアル品質10件（`TASK-C2-2026-08-03-build18-tutorial-quality.md`・本人カード裁定4件は全部推奨案でGO）: 練習遷移の整理・ジャーニーバー4段化・ボタン縁取り（実測4.91:1/5.40:1）・練習ボタン「きょうやった！」化・ツアー8→7枚・初回チャット4点バー削除・ツアー中FAB非表示ほか。その前のビルド17=本人のビルド16実機フィードバック13件（P-1〜P-7＋Q-1〜Q-4=ツアー内かたさチェックを本物フル結果に格上げ・練習ポップアップ廃止・サムネはツアー中不可=本人裁定）。
- **未消化: P-8/P-9（相談室タッチグロー）**。正体は「コードに実装が無いのに実機で光る」珍物件。alan5がフレーム差分で実在を実測（輝度+2%相当・シート内限定・タッチ連動・**指追従ではなく下部中央固定のドーム**）。ビルド17に`presentationBackground(colors.bg)`明示の暫定対処を同梱したが**シミュレータで再現せず効果未検証→本人の実機確認待ち**。消えていれば正体=シート既定背景の透けで確定し、P-9=「押下要素への淡いハロー（不透明度10-15%・半径1.5倍・フェード0.15s）」を相談室チップ+かたさチェック選択肢に意図的実装（本人GO済み）。消えていなければ再調査。裁定全文は`REPORT-C2-2026-08-02-build17-p8p9-blocker.md`とドア配達文。
- **appdev運用の注意（2026-08-03実害）**: appdevがアップロード前に選択カードUIで数時間スタックした。無反応時はhardcopy→内容検分→推奨選択肢ならEnter(`\r`)送信で解除。
- **検収の型（確立済み・変えるな）**: 報告は鵜呑みにせず自分のgrep＋実描画＋`npm test`再実行。絵は両テーマ実背景色合成・実寸で見る。動きは録画フレーム抽出。ビルドはASC API実照会（`ogatore-hub/secrets/asc-api.json`・鍵はkey_path参照・PyJWT）。whatsNewはalan5が視聴者コピー流儀で書く。通知は`alan-push.py`
- **本人の運用スタイル（この3日で確立）**: 実機を触ってスクショ連投→「つづく」→「終わり」で締まる収集ラウンド制。仕分けて発注書1本にまとめ、ゲートは全部alan5、**完成したビルドだけ見せる**。判断は結論先出し＋カードUI（ただし流れの中では平文の一言で返ってくることも多い）
- **駐車中（急がない）**: ①redFlags設計案件（持病・服薬中・脳卒中サイン。ドラフト2本=`soudan-pattern-drafts/soudan-draft-redflags-copy.md`+`-critique.md`・**エンジン改修を含む設計案件と確定・本人ゲート必須**） ②相談室下書き14本の統合（Sonnetで機械統合可） ③Web実配信（sw.js版数上げ）は本人ゲートのまま ④全身/背中のクローズアップ絵の様子見
- **僕がこの期間に踏んだ失敗（次の自分への警告）**: 確かめずに断定4件でOpus休止→Sonnet→本人判断でFableに昇格して今に至る。**根拠併記・「無い」と言う前に検索パターンを疑う**（アンダースコア/ハイフン等の命名差）。Fable検証に渡すメモに自分の誤記憶を書くな（2回やった）。時刻を書く前に`date`を打て（2回間違えた）

### （初回の代替わり記録・2026-07-30。手順の背景はここから下）

**これは何**: 移植監督の会話文脈が満杯になったため`/clear`で自己代替わりした。会話の記憶は
失われている。**このセクションが唯一の引き継ぎ。** 読む順は ①ここ → ②HANDOFF.md冒頭の
⚠️エントリ群 → ③`REPORT-C2-2026-07-30-testflight-build7.md`。

### いまどこにいるか

**TestFlightビルド7を配信し、ASC APIで自分で裏を取り、本人に報告済み。**
`version=7 / state=VALID / group=3b3f7a0b-3063-451d-acdd-404432f08a76`。
**次の一手は本人の実機フィードバック待ち。** 待ちの間に下の保留3件を進める。

ビルド7で本人に見てもらうよう頼んだのは4点: 見出し・ボタンのアイコン化（使い方タブの
FAQ見出し、設定「記録のひっこし」の3ボタン）／相談室の動画サムネイルカード／
ホーム「きょうの1本」が実動画（ビルド6由来）／動画を探すの絞り込みピル（同）。
**ビルド6は本人が飛ばした可能性があるため、6のぶんもwhatsNewに但し書き付きで残してある。**

### 2026-07-30後半: 演出思想相談 → D4/D10完了・D8進行中（旧「保留3件」を置き換え）

**旧「D4は本人指摘=無機質」は誤りだった**（このセクションの少し下の訂正参照。「無機質」は
先代の要約で、本人の生の言葉ではなかった）。同日、本人に直接もう一度話してもらい
（生の音声フィードバック全文はFable監査プロンプトに残っている）、そこから設計→Fable監査
（`Agent id a6819489f9954454e`相当の記録は無いが結論は下記3発注書に反映済み）→本人GO
「すべてやりましょう」で3本の発注書を作り、appdevが実装・報告まで完了した。

- **`TASK-C2-2026-07-30-onboarding-scroll-and-copy.md`（完了・`REPORT-C2-2026-07-30-onboarding-scroll-and-copy.md`）**:
  初回チャットの自動スクロール競合（iOS/Androidで移植元が違う）・CTAボタンのScrollView埋没
  （ツアー画面のD6パターンを横展開）・かたさチェックのボタン見分け・練習ガイド/ツアー文言。
  **旧D10（タップできることが伝わらない）はここで解決済み。**
- **`TASK-C2-2026-07-30-completion-moment-redesign.md`（完了・phase1+2両方、
  `REPORT-C2-2026-07-30-completion-moment-phase1.md`/`-phase2.md`）**: **旧D4の正体**は
  「労い・紙吹雪・カードモーダルが同一フレームで同時発火し、労いがカードに埋もれて消えていた」
  構造。演出を足すのではなく順番を直した（0.7秒の間→カードがフェード入場）。特別tier
  （記念日/季節/レア）だけ既存の節目ポップインカーブを流用して「性格の違い」程度の入場差。
  **禁止事項として「開封・めくり・タメの文法を入れない」を明記**（Fable監査:
  レアは「運ではない」とFAQで約束済みなので、演出で運っぽさを出すと矛盾する）。
  **tier軸のまま実装されていることをalan5がコードで確認済み**（初出/再会など別軸への
  差し替えは却下された。alan5自身が最初その差し替えを提案しかけてFableに正された経緯あり）。
- **`TASK-C2-2026-07-30-icon-system-addendum-chips.md`（完了・
  `REPORT-C2-2026-07-30-icon-addendum-complete.md`）**: 部位選択（腰・肩など）・
  時間帯選択（朝/夜/お風呂上がり）・設定画面「やるタイミング」の全9種、両OS実装・
  alan5がMD5照合＋実背景色への合成＋タブバー比較で目視確認済み。
  ⚠️ **旧メモの「スクイズハンターのトーンに合わせる」は誤り。訂正: トンマナは
  タブバー（`KyonoTabBar.swift`）に揃える。スクイズハンター調は硬さチェック結果6タイプ
  （`KyonoTypeArt.swift`）だけの例外で、これ以外には適用しない**（本人が明示的に
  例外指定・2026-07-30生発言で確認済み）。生成パイプラインは`scripts/gen-chip-art.py`
  （`gen-type-art.py`踏襲だがアンカーは`.art-staging/icon-anchor-tabbar.png`＝タブバーの
  実物。squeeze-hunterのアンカーは使わない）。iOS絵=`ChipArt/chip-<v>.png`、Android絵=
  `drawable-nodpi/chip_<v>.png`、9種ともMD5完全一致。iOS/Androidとも`chip.v`から動的に
  読み込む形に汎用化済み（当初「腰」だけのハードコードだったのを解消）。
  ⚠️ **alan5の検索ミス（同日3回）の記録**: サンプル確認の過程で「iOS未実装では」と
  3回指摘したが、いずれも誤りだった。原因は`grep`の検索パターンがAndroidの命名規則
  （`chip_youtsuu`アンダースコア）のままで、iOSの実際のファイル名（`chip-youtsuu`ハイフン・
  `ChipArt/`ディレクトリ）に一致していなかったこと。`grep -rl`はデフォルトでファイル内容
  しか見ずファイルパス自体は拾わない点にも注意。**教訓は
  [[feedback-evidence-required-no-assertion]]に追記済み**: 「無い」という結論を出すときは
  検索パターンがプラットフォーム間の命名規則の違いを跨いでいないか疑うこと。appdevから
  コミットハッシュ・MD5等の具体的な反証が来ているのに自分の否定的な検索結果だけで
  押し返しているときは、まず自分の検索方法を疑う。

⚠️ **訂正（2026-07-30・代替わり後のalan5が実測）**: ここには当初「アイコン方針④で『👇』絵文字は
文言を残して絵文字だけ落としてある」と書いてあったが**誤り。落ちていない。** ビルド7時点の
コードに👇はユーザーに見える文言として残っている（iOS: `HomeView.swift:402,607,854` ／
`OnboardingViews.swift:263,647,892`。Android: `MainActivity.kt:1222,1492,1678` ／
`OnboardingScreens.kt:296,651,918`）。**発注書④が名指ししたのは2箇所**
（「👇 つぎは ここを押してみて」「👇 タップでそのまま聞けるよ」）で、指示は
「アイコンに置き換えず絵文字だけ落として文言は残す」だった。**appdevは虚偽報告していない**:
trial1報告で「👇の適用もその後です」と後回しを明言し、以降のバッチ報告に一度も現れないまま
落ちた。**僕が確かめずに『落としてある』と書いた**のが誤りの原因（先代の観点4「文字列を見た
だけで書くな・確認済みと監査の指摘のままを分ける」の再発）。
→ **未消化の発注として生きている。** D10の設計が決まった時点で「行ごと消す／絵文字だけ落とす」
を確定させて一緒に発注する（発注書④自身が「行ごと消すか残すかはD10の設計が決まってからで
構わない」と書いているので、いま単独で消す必要はない）。

### appdev（C2・`screen -S appdev`）を見る観点

配達は `bash ~/Claude/ogatore-hub/scripts/door-deliver.sh appdev <msgfile>`。
指示書は`TASK-C2-YYYY-MM-DD-*.md`でcommit+pushしてから配達。
**screen stuffは1KB超で黙って落ちるので長文は必ずファイル経由。**

**appdevは信頼できるが、報告をそのまま採らない。これが僕の役目。**
今日この観点で拾った実例:

1. **「直りました」を絵で確かめる。** Canvasアイコンは**ビルドもテストも通るのに絵が
   間違っている**。SwiftのCanvasパスをSVGに書き出して`qlmanage -t -s 512 -o <dir> f.svg`で
   PNG化して**目で見る**。これでクラッカー（再生ボタンに見えた）とパレット（絵の具が
   本体からはみ出していた）の2件を差し戻した。**この工程を省くと絵の欠陥は実機まで残る。**
2. **「全種類そろえた」を md5 で確かめる。** E1で新規PNGだけ配って3体が旧絵のまま
   だった。`assets/`と両ネイティブツリーのmd5照合。**F3dとしてqa.jsに機械化済み**
   （現459 checks）なので、いまは`npm test`が見張っている。
3. **auto-syncは壊れた状態も拾う。** even-syncが10分ごとに自動commitするので、
   「わざと壊して検査が赤くなることを見る」検証は危険。今日1回混入した。
   appdevへ安全手順は渡してある。
4. **僕が確かめずに書いた指示で今日5回まわしている。** 指示書には
   **「自分で確認済み」と「監査の指摘のまま」を必ず分けて書く。**
...
- 本人向け通知は`~/Claude/ogatore-hub/apps/alan-ai/alan-push.py`経由のみ
- Androidのテストは`--rerun-tasks`付き

### 設計の物差し

**本人のガイドライン > Web版1:1移植**（両者が食い違ったらガイドラインを採る。7/29確定）。
**節目でFable監査**を回す（今日、僕自身の投資計画を数値基準で却下させて助かった）。
...
「おかえりなさい」表示、末尾180dpによるスクロール範囲確保、いずれも正常と確認。padding変更の
回帰(カード間隔・タブバー重なり・末尾余白)も崩れなしとのこと。これで差し戻しゼロ・発注22件
すべて検収完了。
...
`pm clear`後の初回起動でオンボ1問目の選択肢をタップすると、選択されずに背後のHome画面の
オガトレ相談室が開いてしまい、**初回体験の入口が完全に塞がっていた**。

**根本原因(uiautomatorで実機解析して特定)**:
1. `screen-transitions.md`でオンボをシート化した際、スクリム(`Box.background(Color.Black
   .copy(alpha=0.55f))`)に`clickable`が無かった。Composeでは明示的なclickable/pointerInput
   モディファイアが無いBoxはタップを素通しする(相談室のスクリムには元々clickableがあり、
   この穴は無かった)。
2. オンボの吹き出し/選択肢を積むColumnが`verticalScroll`はしていたが自動スクロールしておらず、
   新しい設問が追加されるたびにその選択肢がシートの可視領域(クリップ境界)ぎりぎり〜はみ出た
   位置に描画されていた。実測(uiautomator): シートの内側ScrollViewはy=2215で切れているのに、
   最後の選択肢の実際のタップ判定はy=2278まで及んでおり、その差分がちょうど背後のHomeの
   相談室カード(y=2215-2337)と重なっていた。

**対応**:
- スクリムに no-op の`clickable{}`を追加(タップを吸収するだけ・閉じない。Web版も同じ挙動)。
- オンボのColumnに`bubbles.size`/`activeQuestion`/`routeCta`の変化をキーにした自動スクロール
  (`animateScrollTo(maxValue)`、60ms待ちはtodayCard自動スクロールと同じ猶予)を追加し、
  最新の選択肢が常に可視領域の内側(はみ出さない位置)に来るようにした。
- iOS側も同じ2点を先回りで対応(スクリムに明示的な`onTapGesture{}`吸収+`ScrollViewReader`で
  最下部への自動スクロール)。SwiftUIの`Color`は既定でヒットテストするはずで実害は未確認だが、
  Android実機で確認された不具合パターンなので保険として同じ形にしておいた。

**`updateFabs()`(index.html:1419-1434)の移植漏れも修正**: 従来はFAB表示条件が
`currentTab != null`のみで、Web版が2026-07-19/07-20/07-21の3回の実測で積み上げた個別の
非表示条件(相談室カードとの重複でホームでは相談室FABを出さない・使い方タブはFAQ見出しの▾に
被るため両FABとも出さない・1日目チュートリアル当日は通信FABを出さない)がすべて欠落していた。
既存の`HomeLogic.fdActive`を呼ぶだけで判定ロジックは再実装せず、両OSに実装した。
...
(safety-fixtures 111/111)・両OSビルド成功。判定・ルート決定ロジックは無変更。
...
Android/iOSと突き合わせた。結果一覧:

| 項目 | 判定 | 内容 |
|---|---|---|
| cpop(応援メッセージのポップイン) | **差があり修正** | cheerText表示に演出が無かった。scale.85→1+opacity.4→1の.3s ease-outを追加 |
| doneNudgePulse(戻ってきたときの2回パルス) | **差があり修正** | 「きょうやった！」ボタンにパルス演出が無かった。scale1↔1.045・0.7s×2を追加 |
| .bar>div(進捗バー幅のtransition .4s) | **差があり修正** | マイ記録msBar・相談室planBar双方が瞬間切替だった。animateFloatAsState/.animation(value:)を追加 |
| .vin(せんぱいの声カードの3Dフリップ .55s) | **差があり修正** | タップでめくる際が瞬間切替だった。rotationY+cameraDistanceの3Dフリップを追加(裏面は逆回転で鏡像を打ち消す) |
| sddot(相談室タイピングドット) | 一致 | `soudan-staged-reveal`/`ios-sdbubble-unstable-id`タスクで対応済み・再確認のみ |
| コピー2秒revert(index.html:2949関連・実質B項目) | 一致 | 両OSともLaunchedEffect/Task.sleep(2000ms)で実装済み確認 |
| fdBob/fdBreathe/fdPop(はじめの1本ガイドの指差し/呼吸/カードポップ) | **該当なし(構造的欠落・要判断)** | 下記参照 |
| sdup/obpop(相談室シート/オンボシートのせり上がり・拡大演出) | **該当なし(構造的欠落・要判断)** | 下記参照 |
| faq-g-arrow(使い方タブのアコーディオン矢印回転) | 該当なし | ネイティブの使い方タブはアコーディオン自体が無く全セクション常時展開(意図的簡略化とみられる・不具合ではない) |
| toastIn(汎用トースト) | 該当なし | Web版でも到達可能な箇所での使用が見当たらず(コピー完了は上記の別実装) |

**構造的欠落として別途判断を仰ぐ2件**(「演出のみ追加」の範囲を超えるため、このタスクの
「やらないこと: ロジック・判定・データ構造は変更しない」に照らして今回は実装しなかった):

1. **「はじめの1本ガイド」の指差し演出一式が丸ごと未実装**: Web版はfd guide中、結果画面が
   おすすめ3本ではなく「①だけを表示+指差しアニメ(fd-point/fdBob)+練習宣言の吹き出し+
   OS別もどりかた案内」という別UIに切り替わる(app-quiz.js:305-320)。さらにmarkDone後は
   makeCardBtn上に「👇つぎはここ」ヒント+呼吸アニメ(fd-breathe)が付き(app-record.js:196-206)、
   1日目クリア時のcheerには記録カードのサンプル画像がポップイン(fd-cardpop、app-record.js:145)
   する。ネイティブのResultViewは常に「おすすめの3本」を表示するのみで、この一連の
   「動きで導く」演出(2026-07-21 5視点検証D・PO承認済みの機能)がAndroid/iOS双方に存在しない。
   HomeLogic.fdActive()自体はガイド判定として機能しているので、判定ロジックの欠落ではなく
   UIブランチが無いだけ。演出3種の追加ではなく新規UIブランチの実装が要るため今回のタスク
   スコープ外と判断。
2. **画面遷移アニメーションが全画面で皆無**: ネイティブは手組みのScreen sealed class/enumで
   画面を丸ごと切り替える方式(§1-4・NavHost不使用)のため、相談室(sdup: 下からせり上がる
   .25s)もオンボ(obpop: 拡大+フェード .28s)も、Web版のような「モーダルシートが浮き上がる」
   演出が無く、単に他の画面と同じ瞬時切り替えになっている。加えて相談室・オンボとも
   ネイティブの見た目自体がWeb版のような「画面下からのシート(角丸+スクリム背景+部分高さ)」
   ではなく通常の全画面コンテンツとして実装されている。これは相談室・オンボ2画面だけの
   問題ではなく、Screen方式を採用している約15画面すべてに共通する設計特性であり、直すなら
   画面遷移の仕組み自体に手を入れる必要がある(個別の演出追加では済まない)。

いずれも修正自体は可能だが、「演出・タイミングの修正のみ」というこのタスクの範囲を超え
UIブランチ追加/画面遷移アーキテクチャ変更が必要なため、alan5/本人の優先度判断を仰いでから
着手する。

回帰確認: Android実機で cpop(cheerText表示確認)・doneNudgePulse(パルス2回をピクセル測定で
実測: ピーク時ボタン幅867→905px≒1.044倍、仕様の1.045とほぼ一致)・vcard flip(タップ後
352msで裏面へ切替済みを確認、瞬間切替でないことを確認)を確認。iOSはビルド確認+コードレビュー
(同一ロジックのSwiftUI実装)。`npm test` 443緑・Android`testDebugUnitTest`緑・iOS
SafetyCore(111+8)/RecordCore(35)/CardCore(16+55)緑。判定ロジック無変更。
...
タップ→416ms後には全吹き出しが同時表示済みと判明)。Web版`sdPush()`(index.html:3090-3134)は
複数吹き出しの返信をタイピングドット(「…」)を挟みながら1つずつ段階的に表示する演出があるが、
ネイティブは`applyResponse()`が生成した吹き出し配列を`messages = messages + newMsgs`で一括
追加していただけで、この演出が丸ごと欠落していた。

`applyResponse()`をsuspend関数化し、bot発言だけを`revealBotMessages()`で1つずつ表示するよう
変更(ユーザー自身の発言は従来どおり即時追加・変更なし)。待ち時間の計算式
(`base=min(1600,500+文字数*22)`・1個目=400ms固定・2個目=base・3個目以降=base*2)は
index.html:3053 `sdMsgLen()`・index.html:3113-3114の計算式をそのまま機械移植(手で丸めない)。
`chipsMode`の更新はWeb版の`sdRenderChips()`と同じく全吹き出しの表示完了後にまとめて行う
(表示中は直前のチップ列がそのまま残る=Web版の実際の挙動と一致)。カテゴリタブの早期畳み
(index.html:3097-3100・スクロール見切れ対策のWeb固有の最適化)はComposeのレイアウトでは
該当する問題が起きないため見送り(表示タイミングの本質=段階表示自体は1:1)。

多重タップ対策として`sdPending`(Web版index.html:3096の1:1移植)を追加し、演出中の
チップタップ/送信を無視するようにした(演出中に複数の応答が同時進行して吹き出し順序が
入り乱れるのを防ぐ、Web版の`if(sdPending) return`と同じ安全対策)。送信ボタンも演出中は
非活性化(Web版`sdSyncSend()`相当)。タイピングドットのUIはavatar+吹き出し内に3つの点を
位相差(150ms刻み)でアルファ明滅させるアニメーションとして両OSに新規実装。

Android実機で`adb shell screenrecord`+`ffmpeg`でのフレーム抽出により検証: チップタップ直後に
ユーザー吹き出しが即時表示→タイピングドット表示→約400ms後(15fps撮影で6フレーム=400ms一致)に
1個目の吹き出しに置き換わる→再度タイピングドット→2個目・3個目と続く→動画案内の吹き出しまで
順に表示→全て表示完了後にチップ列がフォローアップ(「この悩みを2週間プランにする」等)へ切り替わる、
という一連の流れを画像・タイミングともに確認。

安全系テスト(SafetyCore 111/111 fixtures)・card-golden 55/55・RecordCore 35/35・
`npm test` 442・Web版配信ファイル無変更を確認。マッチング・判定ロジック(どの回答を選ぶか)は
無変更・表示タイミングのみの変更。オンボーディングの吹き出し演出(既存実装済み)には触れていない。

## 2026-07-27 設定画面: クリップボード自動読みこみボタン+テーマ「じどう」説明文を実装

`TASK-C2-2026-07-27-settings-clipboard-import-and-hints.md`(alan5第2弾監査)。設定画面
「記録のひっこし」欄に、Web版(index.html:839,2067-2074 `importFromClipboard()`)にある
「📋 コピーした記録を自動で読みこむ」ボタン(クリップボードを直接読み取り自動でimport実行)が
無く、手動貼り付け欄のみだった。Web版のコード内コメント「高齢者・デジタル機器が苦手な方向け:
長押しコピー→貼り付けという操作を、ボタン1つで完結させる(2026-07-19 Fableレビュー『コピペ方式は
挫折点』対応)」という意図的な設計のため追加実装。加えてテーマ「じどう」の説明文
(index.html:807「「じどう」は夜(19時〜朝5時)やスマホがダーク設定のとき暗くなります」)も
表示のみで欠落していたため追加。

自動読みこみボタンはAndroid: `ClipboardManager.primaryClip`、iOS: `UIPasteboard.general.string`
でクリップボードを読み取り、空/空白のみのときはWeb版と同趣旨のフォールバック文言
「クリップボードが空みたい\nコピーできているか確認してね」を表示して下の手動欄へ誘導。
内容があれば`importInput`に自動投入し、既存の確認ダイアログ(`confirmImport`)→
`KyonoTransfer.importString()`という既存のインポートフロー(判定・変換ロジックは無変更)へ
そのまま合流させる(Web版の`importFromClipboard()`→`importData()`という2段呼び出しと同じ構造)。

Android実機で確認: ①「記録をコピーする」でクリップボードにexport文字列をコピー→
②「📋 コピーした記録を自動で読みこむ」タップ→自動で確認ダイアログが開く→「書きかえる」→
「よみこみました！」を確認(クリップボード内容→自動import本文投入→確認→書き込みの一連の
流れが動作)。空クリップボード時のフォールバック文言表示は、`text.isBlank()`一発判定という
既存の類似パターン(ObuPreviewPopupの投稿ゼロ時分岐等)と同型の単純分岐のためコードレビューで
確認(実機での空クリップボード再現は行っていない)。テーマ「じどう」の説明文は実機で表示を確認。

安全系テスト(SafetyCore 111/111 fixtures)・card-golden 55/55・RecordCore 35/35・
`npm test` 442・Web版配信ファイル無変更を確認。手動貼り付け欄・エクスポート機能・
テーマ判定ロジック自体は無変更。

## 2026-07-27 とどくメーターのお祝いメッセージ3分岐を実装

`TASK-C2-2026-07-27-reach-meter-messages.md`(alan5第2弾監査=未監査領域スイープで発見)。
とどくメーターの段位ボタンをタップした直後のメッセージが、Web版(app-record.js:238-243
`setReach()`)は3パターン分岐しているのに対し、ネイティブは常に固定文言だった。

- 自己ベスト更新時(`lv > best && best > 0`): 「🎉 自己ベスト更新！「{段位名}」」(太字ピンク)
  + 「記録カードにも入ります」(通常色)
- 初回でいきなり高レベル(`lv >= 4 && best === 0`): 「最初から「{段位名}」！すばらしい」(太字ピンク)
- それ以外: 「記録しました！じわじわ伸びていきますよ」

`best`(タップ前の自己ベスト)は`RecordLogic.setReach()`呼び出しで store が更新される**前**に
必ず算出する必要がある(既存の`reachList`state・`getReach()`から算出。判定ロジック自体=
`setReach`本体は無変更)。表示メッセージ自体に太字+ピンクの部分強調があるため、Android版は
`reachMsg`の型を`String?`から`AnnotatedString?`に、iOS版は`String?`から`Text?`に変更し、
既存の同ファイル内の類似パターン(いまの記録/自己ベスト表示)と同じ手法で部分装飾を実現。

Android実機で3パターンとも確認: ①未記録状態(best=0)で「つま先」タップ→「最初から」
メッセージ、②同日内で「ゆか」に上書き(best=4→5)→「自己ベスト更新」メッセージ、③さらに
「ひざ」に上書き(自己ベストが下がる方向のタップ)→通常メッセージ、をそれぞれ実機で確認。

安全系テスト(SafetyCore 111/111 fixtures)・card-golden 55/55・RecordCore 35/35・
`npm test` 442・Web版配信ファイル無変更を確認。段位の記録・判定ロジック自体は無変更。

## 2026-07-27 節目カードの記録ひかえ促し(cardMsExportNudge)を実装

`TASK-C2-2026-07-27-milestone-card-export-nudge.md`。記録カードモーダル調査で発覚、
Web版に2026-07-18本人承認済みで実装されている「節目カード(通算3日/7日/14日等)表示時の
記録のひかえ(エクスポート)促し」(index.html:1199,2783 `cardMsExportNudge`)が両OSとも
丸ごと未実装だった。設定画面のエクスポート機能自体は既存実装済みで、そこへの導線(促し文言+
ボタン)だけが欠けていた。

`renderTodayCard()`(既存・カード描画の共通関数。ホームの「きょうの記録カード」・マイ記録
カレンダーの「この日の記録カードを見る」の両方から呼ばれる)は内部で`milestone`判定
(`MILESTONES.contains(effTotal)`)を計算していたが、呼び出し元にはBitmap/UIImageのみを返し
判定結果を捨てていた。`TodayCardResult(bitmap/image, isMilestone)`にラップして返すよう変更し、
両方の呼び出し元(ホーム画面のカードダイアログ・カレンダー日別カードダイアログ)で
`isMilestone`のときだけ「せっかくの節目！記録のひかえを取っておくと あんしんです📦」+
「記録のひかえを取る」ボタンを表示するよう追加。ボタンタップでカードを閉じてから設定画面
(`Screen.Settings`/`.settings`)へ遷移する(Web版`goToExportFromCard()`と同じ「閉じてから
移動」の順序)。じまんカード(#brag発)は元々別のダイアログ/画面のためこの変更の影響を受けず、
自然に対象外のまま(Web版の「じまんカードは節目対象外」という除外条件と同じ結果になる)。

日別カード(カレンダーの過去日カード)もWeb版`makeCard(ds)`が両方の呼び出し元で共有されている
ことを確認したうえで同様に対応(過去のtotalが節目日と一致する日を見返したときも同じ促しが
出るのがWeb版の挙動と一致するため)。

Android実機で確認: ①store操作でtotal=3(節目)を再現→「記録カードを見る」→ゴールドカード+
促し文言+ボタン表示を確認→②「記録のひかえを取る」タップでカードが閉じて設定画面(続ける設定・
「記録のひっこし」セクション)へ遷移することを確認→③total=2(節目でない)で再現→通常の青カード
のみ・促しなしを確認。

安全系テスト(SafetyCore 111/111 fixtures)・card-golden 55/55・RecordCore 35/35・
`npm test` 442・Web版配信ファイル無変更を確認。エクスポート機能自体・MILESTONES判定ロジック
自体は変更せず、表示・導線の追加のみ。
...
「雑談KB(smalltalk)・タイプ別パーソナライズ(sdTypeFlavor)・自由記述同義語追跡(SD_FU_KW)は
未移植」というコード内コメントについて、必須の欠落ではなく任意拡張として対応要否の判断を
委ねられた。3点とも今回は見送り。理由: 雑談KBはsoudan-kb.js内だけで約270行・キーワード群
25件超のデータがあり機械抽出+ストリーク管理の実装量が相応にある一方、安全性・コア機能
(かたさチェック→動画提案→記録)に影響しない領域。タイプ別パーソナライズ・同義語追跡は
データ量・実用インパクトともに小さいと判断。コード変更なし(判断のみ・ドア報告のみ)。

## 2026-07-27 相談室の安全文言・締めメッセージ・逃げ道リンクの欠落を修正

`TASK-C2-2026-07-27-soudan-safety-copy-and-links.md`。12セクション監査の対象外だった相談室
(ボトムシート型モーダル)をalan5が別途Web版と突き合わせ、4箇所の文言・機能欠落を発見。

1. **ディスクレーマー1行不足**(index.html:1223 `.sd-disc`): 「※回答はオガトレ監修のパターン集
   から選んでいます」の1行目が欠落し2行目のみだった。両OSとも2行表示+2行目後半
   「強い痛み・しびれがあるときは医療機関へ」を太字化して1:1移植。
2. **開始あいさつが静的キャプションだった**(index.html:3479 `sdEnsureGreeting`): 「肩こりや
   腰痛など…」という固定文言をチャット枠の外に表示していたのを廃止し、Web版と同じ「こんにちは、
   オガトレです！…」を吹き出し形式の1通目として表示するよう変更。`sdGreeted`(Web版のモジュール
   レベル変数)相当をルート階層のプロセス内メモリ状態(`obTourDone`等と同じ設計・§2-3)にして、
   アプリのセッション内で初回オープン時だけ表示。
3. **未マッチ時の安全文言+3リンクが消えていた**(index.html:3323-3330 `sdAnswerFallback`):
   フォールバック応答時、安全文言(小さめ注意書き)と3つの逃げ道リンク(①mailto届け先②クリップ
   ボードコピー③検索タブへ遷移)が全く出ていなかった。`SoudanEngine`/`SoudanEngine.swift`の
   `SoudanResponse`に表示専用フラグ`isFallback`を追加(判定ロジック自体は変更せず、
   `fallbackResponse()`が組み立てた結果を運ぶだけ)。UI層が文字列マッチ等でフォールバックか
   どうかを再判定するアンチパターン(§3-2)を避けるための設計。mailto導線は検索タブの
   リクエスト導線(`openMailIntent`/`openMailTo`)をprivate解除してそのまま再利用。
4. **かたさチェック未受診者への誘導チップが無かった**(index.html:3160-3163):
   体の悩み系intent(`SOUDAN_BODY_INTENTS`9件)への回答時、まだかたさチェックを受けていない
   ユーザー(`store.get("type")==null`)にだけ「30秒のかたさチェックやってみる?」チップを
   followups列に追加(nextBestチップの後・「べつの悩みをそうだん」の前、Web版と同じ並び順)。

Android実機で4項目全て確認: ①②起動直後のディスクレーマー2行+吹き出し形式あいさつを確認、
③「肩こり・首こり」チップ→未受診時のみ「30秒のかたさチェックやってみる?」チップが表示され
タップでかたさチェックQ1へ遷移することを確認、④英数字の未知語を送信→安全文言+3リンクが表示
され、mailto(Gmail起動)・クリップボードコピー(「コピーしました✅」+クリップボード候補に
反映)・検索タブ遷移の3つとも動作することを確認。

安全系テスト(SafetyCore 111/111 fixtures)・card-golden 55/55・RecordCore 35/35・
`npm test` 442・Web版配信ファイル無変更を確認。スコアリング・マッチング判定ロジック自体は
一切変更せず、表示・文言・リンクの追加のみ。
...
代えて、index.html全12セクション(#home/#quiz/#result/#history/#brag/#fun/#reach/#voices/#obu/
#playlists/#search/#guide)を要素レベルで1つずつ照合する体系的監査。見つけた欠落はその場で実装。

**#home**（再確認）: streakCard内に3件の欠落を発見・実装。
- `memoRow`（ひとことメモ入力欄+「メモをのこす」ボタン）が完全に未実装だった。上記
  「⚠️ 未対応で発覚」（HANDOFF.md）の指摘を解消——既存の`RecordLogic.saveMemo`/`loadMemos`
  （純粋関数・Step3で移植済み）を呼ぶだけで実装。
- `plateauNote`（通算12-16日/28-34日の停滞期はげまし文言・とどくメーターへのリンク付き）が欠落。
- `fdDoneStaticNudge`（はじめの1本ガイド中・未記録時の常時案内）・`cardHint`（記録カードボタン下の
  常時ヒント）も欠落していたため追加。`KyonoLineButton`（両OS）に`enabled`パラメータを新設
  （メモ保存ボタンの保存済み無効化表示に使用）。

**#quiz**（未調査だった）: 3件の欠落を発見・実装。タップ誘導文言「👇 タップしてえらんでね」・
「← まえの質問へ」戻るボタン(Q1以外で表示)・「ホームにもどる」ボタン(回答済み時は確認ダイアログ)。
従来は質問を間違えても引き返せず、中断してホームに戻る手段も無かった。

**#result**（type images以外未調査）: 6件の欠落を発見・実装。
- `rPT`（タイプ別「理学療法士のひとくち解説」）: `TypeInfo`に`pt`フィールドが無く、データ自体が
  未抽出だった(既存コメントで「pt/rx/poolは動画カタログ連動でStep7aの範囲」と一括除外されていたが、
  ptは動画非依存の解説文なので個別に追加。§1-2に基づきapp-quiz.jsから機械抽出)。
- `rReachNote`+自動転記: とどくメーターが未記録なら、かたさチェックQ1(前屈)の回答を初回記録として
  自動書きこみ(`REACH_FROM_MOMO`)、結果画面に一言表示。データの二重入力を防ぐ導線が欠落していた。
- `rPace`(ペースの目安)・免責注意書き・`rSoudanLink`(タイプ別相談室逆導線)・`rRecheckBtn`
  (もう一回チェックする)も欠落。
- **follow-up課題(未実装・要検討)**: `rxList`/`worryExtra`/`rotate-note`/`rDoneNudge`/`rTourBtn`
  (タイプ別おすすめ動画3本の表示・はじめの1本ガイド専用UI・オンボ→ツアー継続導線)は、Web版専用の
  小規模動画カタログ`V`(app-quiz.js外部・index.html:1458〜)の新規抽出+`currentRx()`ローテーション
  ロジックの移植+専用UIが必要な独立サブシステムのため、今回の監査スコープでは実装せず個別タスク
  化を提案する。

**#history**: カレンダーの記録済み日をタップしても何も起きなかった(`showDay`相当が未実装)。
その日に見た動画へのYouTubeリンク・のこしたメモ・「印だけ」の場合の説明・記録カード表示を追加
(既存の`renderTodayCard`を日付指定で再利用)。続けた記録カード・カード図鑑・とどくメーター・
お楽しみ機能・続ける設定は既存タスクで対応済みと再確認(抜けなし)。

**#brag**: `bragDaysNote`(プリフィル理由の説明文言)が欠落。追加時に、日数入力のプリフィル元が
Web版`streak.count`(いま連続)に対しネイティブは`streak.total`(通算)になっていた既存の不一致も
発見・修正("つづいている日数"という文言的にcountが正しい)。

**#fun**: じまんカード/せんぱいの声/ひとことにっきへの導線がマイ記録タブから直接という構造差は
既存タスクで明示的に許容済み。個別の説明文言は各遷移先で読めるため情報欠落ではないと判断・対応なし。

**#reach**: とどくメーターの常時表示の使い方説明・安全上の注意書き「いたみがある日は
むりしないでね…」・前屈のお手本写真(`assets/check/meter.jpg`)が欠落。§1-2に基づき同一ファイルを
Android drawable-nodpi・iOS CharaArt(png変換)へコピーして追加。

**#voices**: 説明文・免責文言(YouTubeコメント原文/個人の感想)・カード一覧とも実装済みで欠落なし。

**#obu**: 説明文「尾形さんからの ひとこと・写真・ラジオを…」が欠落。加えて、index.html:935
`obuReturnTo`(開いたタブへ戻る)がネイティブでは常にホーム固定になっていた不一致を発見・
`Screen.Obu(returnTo:)`に開いた時の画面を持たせて修正。

**#playlists**: ヒント文言含め既存実装で欠落なし。

**#search**: `searchCatalog()`のyearパラメータ自体は既存だったが、選択UI(`#ySel`)が無く常に
nullで呼ばれていた=年フィルタが機能していなかった。カタログの年一覧からプルダウンを実装して接続。

**#guide**: 目次チップ(gtoc)がgd-help〜gd-myrecの6項目のみで、Web版index.html:988の
「❓ よくあるしつもん」チップが欠けていた。既存のFAQジャンプ用アンカーへスクロールするだけの
実装で追加(gd-faq本体のコードは一切触れていない)。6セクション本体・再入場リンクは既存タスクで
対応済みと再確認。

**画面ごとにコミット**(9コミット)。安全系テスト(SafetyCore 111/111 fixtures)・card-golden
55/55・RecordCore 35/35・`npm test` 442 checks・Web版配信ファイルとも全区間で緑を確認。
ロジック・判定・データ構造(SafetyGate/SoudanEngine/QuizEngine/RecordLogic)は一切変更していない
(表示・要素の追加のみ)。
...
(続ける設定)は画面のみため・もじの大きさ・記録のひっこしのみで、Web版(index.html:799-830)に
ある「やるタイミング」表示・変更と「カレンダーのおしらせ時間」(時刻指定つき)が欠落していた。

- **やるタイミング**(index.html:800): 現在値(朝おきてそのまま/おふろ上がりに/寝るまえふとんの
  上で/きめてない・そのつど。オンボーディングのanchor質問と同じキー)を表示し、「変える」タップで
  インライン選択肢が開く。Home側の`anchorCard`(いつやる派？の再選択UI)は別タスク
  (home-structure-fix.md)で明示的にスコープ外・未実装のため、「変える」の遷移先はこの画面内の
  インライン選択に留めた(「変更導線を追加する」という要求は満たしつつ、未実装のHome機能への
  依存を避ける判断)。
- **カレンダーのおしらせ時間**(index.html:809-816): 時刻ピッカー(Android: `TimePickerDialog`・
  iOS: `DatePicker`)+「📅 Appleカレンダーに入れる」「📅 Googleカレンダーに入れる」の個別ボタン。
  マイ記録タブの既存「📅 カレンダーに登録する」(home-structure-fix.mdで追加済み・時刻指定なしの
  簡易版)とは別物として両方残した(Web版もそう)。Appleボタンは既存のカレンダーIntent/EventKit
  ヘルパーにhour/minute引数を追加して再利用(デフォルト値20:00は既存呼び出し元=マイ記録タブの
  挙動を変えないよう据え置き)。Googleボタンはindex.html:2020 `gcalLink`のURL組み立てをそのまま
  移植してブラウザで開くだけ(終了時刻を分+10・59分頭打ちで丸める、時をまたがないWeb版の実装上の
  癖もあえてそのまま踏襲)。
- **「毎日自動でアプリがひらく設定（iPhone）」は移植しない**(index.html:818-827・Shortcutsアプリの
  自動化手順): PWAが通知を送れないことへのiOS限定の回避策で、ネイティブアプリは通常のローカル
  通知が使えるため元から無関係な問題。タスク指示どおり「ネイティブに存在しない問題への回避策は
  持ち込まない」の例外として明示的に除外。
- **実機確認(Android)**: 「変える」タップで4択が開き、選択すると「やるタイミング」の表示が
  即座に更新されることを確認。時刻ピッカーをタップしてOS標準のTimePickerDialogが開き、時刻変更が
  即座に反映されることを確認。「Googleカレンダーに入れる」タップでGoogle Calendarアプリが実際に
  起動する(URLが正しく組み立てられている)ことを確認。「Appleカレンダーに入れる」は同一の実績ある
  カレンダーIntent機構(home-structure-fix.mdで検証済み)を時刻パラメータ化しただけのため、
  この端末にカレンダーアプリが無い制約下での見た目上の反応なしは想定どおり(コードの問題ではない)。
...
## 2026-07-26 使い方タブの詳細ガイド6セクションが丸ごと欠落していたのを修正(alan5独自調査7件目・最大の発見)

`TASK-C2-2026-07-26-guide-sections-missing.md`。alan5独自調査7件目にして今回最大の発見。本人が
「きほんのき、ネイティブ版ならでは見やすい」と評価したのは`gd-faq`（よくあるしつもんQ&A）だけで、
その**上にある詳細ガイド6セクション（目次チップ+階段状の説明カード。index.html:970-1070）が
丸ごと未移植**だった。`gd-faq`は本人評価済みのため絶対に変更しないという制約付き。

- **目次チップ6個**(index.html:982-987。よくあるしつもんチップ自体=988は含まない): 🆘困った
  ときは/🌅はじめての日/▶まいにちの流れ/🛡記録を守る/🎫つづくしくみ/📅マイ記録。タップで対応
  セクションへ自動的に開いてスクロール。
- **6セクションを1:1移植**: ①困ったときは(常時カード・4ボタン)②はじめての日にやること(既定で
  開いた状態・5ステップ+check-q1.jpg画像+「チェックをはじめる」ボタン)③2日目からの毎日(gflow
  矢印図解+セグメントモック+オガトレ通信案内)④記録が消えない3つの守り(chara-good画像+3ステップ・
  card-sample.png見本)⑤続けるしくみ(card-sample-gold.png見本+カード図鑑案内)⑥「マイ記録」タブ
  でできること(5項目+「マイ記録タブをひらく」ボタン)。各セクション末尾に「↑目次へ戻る」リンク。
  gd-startのみ既定オープン、他は折りたたみ。
- **gd-mamoriのステップ1(A2HS手順)はネイティブ向けに置き換え**: Web版のiPhone/Android別「ホーム
  画面に追加」手順・`a2hsShowForce()`ボタンはネイティブに概念自体が存在しないため移植せず、
  「このアプリを削除しないでね」というシンプルな注意書きに置き換えた(タスク指示どおりの判断。
  「守り」の本旨=アプリを手放さなければ記録は消えない、という情報自体は保持)。
- **gd-help「📅 記録が消えた・0日にもどってる」ボタンの遷移先を調整**: Web版の遷移先FAQ
  (「通算が0日にもどってる！」)はLINE内ブラウザ/Safari間のストレージ分離というPWA固有の前提の
  回答で、GuideData.kt/swiftでも同じ理由(§2-2に準ずる判断)で`hidden=true`にされ非表示だったため、
  ネイティブでも有効な同グループの「連続が切れちゃった…」FAQへジャンプ先を読み替えた。
- **gd-faq(よくあるしつもんQ&A)は1行も変更していない**: 目次チップの範囲外・gd-helpのFAQ
  ジャンプもFAQ本体のコード(グループ/アイテムの描画ループ)には一切触れず、直前に置いた高さ最小の
  アンカーへスクロール+既存のopenGroups/openItems状態(FAQ描画ループが読む公開状態)を外から
  セットするだけに留めた(Android: `BringIntoViewRequester`・iOS: `ScrollViewReader`)。
- 画像アセット3点(`check-q1`/`card-sample-gold`/既存の`card-sample`)を両OSへ追加。iOS側は
  `card-sample.png`がTourArtフォルダに既存で重複しビルド競合したため、CharaArtへの重複コピーは
  削除して既存のTourArt版をそのまま参照する形にした。
- **実機確認(Android)**: 目次チップタップで対応セクションが開いてスクロールすることを確認
  (「▶ まいにちの流れ」チップで実機確認)。「↑目次へ戻る」の逆方向スクロールも確認。gd-helpの
  FAQジャンプ(「🔔 通知・リマインダーについて」)で該当グループ・項目が開くことを確認(初期実装は
  スクロール距離が不足するバグがあり、アンカーの高さを0dpから1dpに変えて解消)。「📱 機種変更したい」
  →続ける設定画面・「マイ記録タブをひらく」→マイ記録タブへの遷移も確認。gd-mamori/gd-myrec等
  各セクションの表示内容を画像込みで確認。
- **iOS**: 同一ロジックで実装しビルド成功・3パッケージ`swift test`全緑・シミュレータ起動確認
  (クラッシュなし)。タップ自動化は従来どおり未対応のため、コードレビューでの1:1ロジック照合に留める。
- **回帰確認**: Android `gradle testDebugUnitTest`緑・iOS `swift test`3パッケージ緑
  (SafetyCore 111/111 fixtures・RecordCore 35/35・CardCore 16/16+card-golden 55/55)・
  `npm test` 442 checks green・Web版配信ファイル無変更・gd-faqのコード無変更。

## 2026-07-26 使い方タブの「はじめてガイド/使い方ツアー」再入場チップ欠落を修正(alan5独自調査6件目)

`TASK-C2-2026-07-26-guide-reentry-links.md`。alan5独自調査の同種パターン6件目。使い方ツアーの
締めスライド(`OB_TOUR_CLOSING`)が「このツアーも使い方タブの『📖 使い方ツアー』からいつでももう一度
見られるよ」と案内しているが、その導線自体がネイティブのGuideScreen/GuideViewに存在しなかった
(ツアー自身が約束している導線が実在しない状態)。

- 使い方タブ最上部(「おぼえるのはこれだけ！」カードの直前)に2つの`.daychip`風チップを追加。
  index.html:970の1:1移植: 「🌱 はじめてガイド」(teal系)・「📖 使い方ツアー」(`.daychip-alt`=
  黄系・文字色はライト`#7E6400`/ダーク`#E8C74C`をWeb版CSSから機械確認して移植)。下に説明キャプション
  「「はじめてガイド」＝さいしょの質問からやりなおす／「使い方ツアー」＝つかいかたをスライドで見る」
  も追加。
- 「はじめてガイド」タップ→`Screen.Onboarding`(Android)/`.onboarding`(iOS)へ遷移し、既存の
  オンボーディングフロー(Step5c実装済み)をそのまま起動。「使い方ツアー」タップ→
  `Screen.Tour(showClosing=false)`/`.tour(showClosing:false)`へ遷移。showClosingをfalseにしたのは
  Web版`obOpenTour()`が`obTourAutoClose`フラグ(自動起動時=true専用)をリセットしないため、手動再入場
  では常に締めスライド(OB_TOUR_CLOSING)を含まない8枚構成になる仕様を1:1で踏襲したもの。
- オンボーディング・ツアー本体のロジックは一切変更せず、既存の画面遷移(Screen sealed
  class/enum)を呼ぶだけ。
- **実機確認(Android)**: 使い方タブ最上部にチップ2つがWeb版と同じ色分けで表示されることを確認。
  「使い方ツアー」タップでツアーが1/8スライド目から再起動し、8枚すべてをタップで進めて「おわる」まで
  実機で完走(締めスライドが出ないこと=8枚構成であることも確認)、ホームへ正しく戻ることを確認。
  「はじめてガイド」タップでオンボーディングの最初の挨拶メッセージから再起動することも確認。
- **iOS**: 同一ロジックで実装しビルド成功・3パッケージ`swift test`全緑・シミュレータ起動確認
  (クラッシュなし)。タップ自動化は従来どおり未対応のため、コードレビューでの1:1ロジック照合に留める。
- **回帰確認**: Android `gradle testDebugUnitTest`緑・iOS `swift test`3パッケージ緑
  (SafetyCore 111/111 fixtures・RecordCore 35/35・CardCore 16/16+card-golden 55/55)・
...
(前屈チェック)カードが「いまの記録: 段位N」のみの表示で、Web版(app-record.js:245-264
`renderReach()`が正本)にある3要素が欠落していた。

- **自己ベスト表示**: 全記録中の最高段位を「自己ベスト: ◯◯」(teal色太字)で併記。
- **前回比コメント**(2回目以降の記録があるときだけ・数字プレッシャーをかけない設計=段位の絶対値
  でなく前回との相対比較のみ): 上がった場合「前回（◯◯）より◯段とどくようになった！🎉」(pink強調)・
  同じ場合「前回とおなじ「◯◯」 キープも立派です！」・下がった場合「体は日によってちがうもの
  またコツコツいきましょう🌱」の3分岐。
- **直近14回のトレンド棒グラフ**: `.rbar`(各バーの高さ=段位×20%)相当をteal系グラデーションの
  縦棒で1:1移植。
- 段位名`REACH_LV`(index.html:1971)を両OSへ機械抽出して移植。既存の「いまの記録: 段位N」という
  生数字表示も、今回追加した自己ベスト/前回比と並べたときの一貫性のため`REACH_LV[N]`(段位名)表示に
  合わせて修正した(タスクの「表示の追加のみ」の趣旨に沿う軽微な表示修正。段位の記録・判定ロジック
  自体は無変更)。
- **実機確認(Android)**: `kyono-store.json`のreachを書き換えて4パターン(0件=案内文のみ/16件で
  トレンド14件キャップ+下降分岐/上昇分岐(自己ベスト更新)/2件で同値分岐)を実タップ相当で確認、
  スクショで文言・色・棒グラフの高さが正しいことを確認。
- **iOS**: 同一ロジックで実装しビルド成功・3パッケージ`swift test`全緑を確認。シミュレータでの
  タップ自動化は従来どおり未対応のため、コードレビューでの1:1ロジック照合に留める。
- **回帰確認**: Android `gradle testDebugUnitTest`緑・iOS `swift test`3パッケージ緑
  (SafetyCore 111/111 fixtures・RecordCore 35/35・CardCore 16/16+card-golden 55/55)・
  `npm test` 442 checks green・Web版配信ファイル無変更。

## 2026-07-26 「ひとことにっき」機能の完全欠落を修正(alan5独自調査での発見・優先度高)

`TASK-C2-2026-07-26-diary-list-missing.md`。alan5独自調査の続きで発見: Web版のお楽しみ機能
(index.html:872-892 `<section id="fun">`)にはじまんカード・せんぱいの声に加えて「ひとことにっき」
(保存済みメモの一覧表示)があるが、ネイティブ版には**この機能自体がどこにも実装されていなかった**。
しかも使い方ツアーSlide7の説明文が「🎉お楽しみ機能（じまんカード・せんぱいの声・ひとことにっき）」と
明記しており、存在しない機能をユーザーに約束してしまっていた状態だったため優先度高扱い。

- **マイ記録タブのお楽しみ機能カードに「📔 ひとことにっき」ボタンを追加**（じまんカード・せんぱいの
  声と並列）。タップ先に一覧画面を新設(Android: `DiaryScreen.kt`・iOS: `DiaryView.swift`)。
  app-record.js:267-273 `renderDiary()`の1:1移植: 保存済みメモ(`kyono_memos`)を新しい順に最大7件、
  日付(MM/DD)+本文で表示。0件時は「「きょうやった！」のあとにメモをのこせます」の案内文。
- 見出しアイコンは既存の`KyonoIcon.Notes`/`.notes`(ノート型・index.html:884のSVGと同一形状)をそのまま
  流用——Phase 3で作成済みだが未使用だったアイコンがちょうど一致していた。
- 区切り線はWeb版の`border-bottom:1px dashed`を両OSとも実線で簡略化(標準コンポーネントに破線divider
  相当が無いための軽微な近似・区切りとしての機能は同等)。
- **重要な発見(別件として要判断)**: このタスクの調査中に、Web版の実際のメモ**保存**UI
  (`memoInput`/`saveMemo()`。ホーム画面の「きょうやった！」直後に表示されるテキスト入力欄+
  「メモをのこす」ボタン。index.html:698-699)が**ネイティブのどちらのOSにも移植されていない**ことが
  判明した。`RecordLogic.saveMemo()`(データ層の純粋関数)はStep3で移植済みだが、それを呼び出すUIが
  存在せず、コードベース全体をgrepしても`saveMemo`の呼び出し箇所が1件も無い。つまり**現状のネイティブ
  アプリではユーザーが一度もメモを書けず、今回追加した「ひとことにっき」一覧は実運用では常に空になる**。
  今回のタスク指示(「メモの保存・編集・削除ロジックは変更しない」)の範囲外のため追加はしていないが、
  別タスクとして着手要否を判断してほしい。
- **実機確認(Android)**: `kyono-store.json`のmemosを直接書き換えて確認(保存UIが無いため正規の
  ユーザー操作では検証不能)。0件時の案内文・9件のメモを保存した状態で新しい順に7件だけ表示され
  古い2件が除外されることを確認。HTMLタグを含むメモ文字列もCompose Text()がそのままリテラル表示する
  ことを確認(Web版のようなエスケープ処理は不要——Composeは元々HTMLを解釈しない)。
- **iOS**: 同一ロジックで実装しビルド成功・シミュレータ起動確認(クラッシュなし)。タップ自動化は
  この環境では従来どおり未対応のため、コードレビューでの1:1ロジック照合に留める。
- **回帰確認**: Android `gradle testDebugUnitTest`緑・iOS `swift test`3パッケージ緑
  (SafetyCore 111/111 fixtures・RecordCore 35/35・CardCore 16/16+card-golden 55/55)・
  `npm test` 442 checks green・Web版配信ファイル無変更。

## 2026-07-26 マイ記録タブの「続けた記録」進捗カード欠落を修正(alan5独自調査での発見)

`TASK-C2-2026-07-26-myrecord-progress-card.md`。ホーム構造修正と同種の見落としをalan5が独自調査で
発見: Web版マイ記録タブの先頭カード「続けた記録」(index.html:752-763 `renderHistory()`が正本。
次のお祝いまでの進捗バー+通算/いま連続のミニ表示+おやすみ券の説明文)がネイティブ版に存在しなかった。

- **Android(MainActivity.kt)/iOS(MyRecordView.swift)双方**: マイ記録タブの`KyonoCard`列の先頭
  (既存の「マイ記録」カレンダーカードより前)に「続けた記録」カードを追加。msNote相当(次の節目名を
  ピンク太字で埋め込んだ1文。全節目達成済みなら「全部の節目をたっせい！すごすぎます」に分岐)・
  msBar相当(通算日数/次の節目日数の進捗バー)・通算/いま連続のミニ表示(横並び・pink/teal色分け)・
  おやすみ券の説明文の4要素を1:1移植。
- **MS/MILESTONES(節目データ)は新規定義せずCardCoreからそのまま参照**(Android:
  `CardDataLoader.shared.MS`/`.MILESTONES`・iOS: 同名。Step4で既に移植済みのものを再利用しただけで、
  データ定義・判定ロジックの追加は無し)。
- 「いま連続」の数値はWeb版の`effectiveStreakCount()`(連続が切れているのに気づいていない場合に0へ
  丸める判定)ではなく、既存のHome画面の「続けた日数」カードと同じ`streak.count`をそのまま表示する
  判断とした。この判定関数はネイティブのどのモジュールにも移植されておらず、今回のタスクは「表示の
  追加のみ・判定ロジック変更なし」が明示されているため、新規に判定を持ち込むより既存踏襲を優先。
- **実機確認(Android)**: `kyono-store.json`のstreak2を書き換えて3パターン(total=1→次の節目
  「『気がする』チェックポイント」・progress≈33%/total=50→「習慣化の記念日」・progress≈76%/
  total=1950→全節目達成済み分岐)を実タップで確認、スクショで進捗バー・milestone名の表示が正しいこと
  を確認。
- **iOS**: 同一ロジックで実装しビルド成功を確認。シミュレータでのタップ自動化はこの環境では従来
  どおり未対応のため、アプリ起動確認(クラッシュなし)とコードレビューでの1:1ロジック照合に留める。
- **回帰確認**: Android `gradle testDebugUnitTest`緑・iOS `swift test`3パッケージ緑
  (SafetyCore 111/111 fixtures・RecordCore 35/35・CardCore 16/16+card-golden 55/55)・
  `npm test` 442 checks green・Web版配信ファイル無変更。

## 2026-07-26 見た目パリティ ブラッシュアップ(自己点検フェーズ・コード変更なし)

`TASK-C2-2026-07-26-visual-parity-polish.md`。ホーム構造修正の検収OK後、本人が1時間ほど離席する
間の自己主導QAとして着手。**結論: 両調査項目とも「異常なし/再現せず」でコード変更は行っていない**。

- **ダークモード横断チェック**: Android実機(エミュレータ)で14画面
  (ホーム未チェック/チェック済み両方・マイ記録・図鑑・じまんカード・せんぱいの声・オガトレ通信・
  使い方FAQ・動画を探す・再生リスト・オガトレ相談室チャット・かたさチェック・診断結果・
  続ける設定・使い方ツアー・オンボーディング)をダークモードでスクショ確認。配色破綻・
  コントラスト不足・ライト色の残留は見当たらず。iOSシミュレータでもホーム画面のダーク表示を
  確認(共通`KyonoComponents.swift`の同一色解決ロジックのため他画面もAndroid同様と判断)。
- **ボタン縦積み時のノッチ現象(alan5がPhase1検収時に指摘・保留していたもの)**: 現在のコードに
  存在する「縦に並んだボタン」パターンを全パターン洗い出し、ダークモードで8〜12倍ズームして
  ピクセル単位で確認:
  a) `KyonoGhostButton`×2連続(マイ記録のお楽しみ機能カード)
  b) `KyonoPrimaryButton`(影付き)+`KyonoGhostButton`、画面背景上(使い方ツアー「つぎへ」+
     「ツアーをとばす」・alan5の指摘に最も近い構図として重点調査)
  c) 同(b)をカード内・disabled/alpha0.5状態で(ホームの「きょうの分は完了！」+
     「記録カードを見る」)
  d) `KyonoSegmentedControl`(入れ子の角丸コーナー。設定画面「じどう/ライト/ダーク」)
  e) オンボーディングの選択チップ2個連続(「大きめ」+「ふつう」)
  いずれも継ぎ目に段差・ギャップ・二重輪郭は確認できず。`KyonoPrimaryButton`の影レイヤー色
  (`#8A6D00`)はindex.htmlのdark版box-shadow値と完全一致しており、Web版と同じ意図的な3D影
  表現であることも確認した。**alan5が指摘した「メニューカードのボタン列」自体、直前のホーム
  構造修正タスクでカード自体を削除済み**。Phase1〜3で大幅な作り直しを経ているため、当時見えて
  いた現象は既に存在しないUIだった可能性が高い、というのが現時点の結論(実機で再度見かけたら
  画面名とスクショを添えて追加調査する)。
- **全画面の余白・整列**: 上記ダークモード確認の際に併せて目視確認。マストヘッド・タイプ
  イラスト・サムネイル周りの余白崩れは見当たらず。
- **回帰確認**: コード変更なしのため`git status`差分ゼロ。念のためAndroid `gradle
  testDebugUnitTest`・iOS `swift test`(SafetyCore 111/111 fixtures含む)・`npm test` 442を
  再実行しいずれも緑を再確認。
...
「Phase2確認済み・良い出来でした。続けてPhase3(sec-head手描き風SVGアイコンの両OS移植+残り画面の
カード型レイアウト適用)へ進んでください」との指示。判断に迷ったら手を止めず無難な側(Web版準拠)に
倒して進めてよい、との事前承認のもと、画面ごとに区切ってコミットしながら一晩で全画面を完了させた。

- **KyonoIcons.kt/.swift新設**: index.html全編の`.sec-head`インラインSVG(手描き風アイコン)を
  SF Symbols/Material Iconsに置き換えず、Compose Canvas/Path・SwiftUI Canvas/Pathで1:1移植。
  全24箇所のsec-head呼び出しが形状で見ると14種類(Clock/Question/QuizCheck/SoudanBubble/
  ObuBubble/Play/CalendarCheck/DexBook/Heart/Envelope/Notes/MountainCheck/ShieldCheck/Star)に
  集約されると判明し、`KyonoSectionHeader(icon:title:fill:accent:)`で全箇所を統一的に描画。
- **残り全画面をカード型・Web版デザイントークンへ移植完了**(画面ごとに個別コミット):
  設定・相談室(オガトレ相談室チャットUI・`.sd-b`吹き出し)・診断結果(グラデーションカード)・
  使い方(FAQ・Qプレフィックス)・マイ記録(カレンダー・おやすみ券・とどくメーター)・図鑑
  (`.dex-box`/`.dex-thumb`)・じまんカード・せんぱいの声(`.vcard`めくりカード・`.vfront`グラデ)・
  オガトレ通信(`.obu-post`/`.obu-text`)・動画を探す/再生リスト(`.searchbox`/`.catbtn`/
  `.chip-a〜d`カテゴリ配色)・オンボーディング(`.sd-b`吹き出し流用)・かたさチェック(`.opt.g0〜g3`
  段階色)・使い方ツアー(`.dots`/`.dot.on`)。これで**アプリの全画面がWeb版デザイントークンで
  統一**された(タスクの「5) 画面全体へ適用」を完全達成)。
- **実機タップで発見・修正した表示崩れ2件**(コードレビューだけでは気づけない類の不具合):
  1. とどくメーターの5段階選択ボタンに`KyonoGhostButton`(内部で`fillMaxWidth()`前提)を
     流用したところ、Row内で1個目が全幅を占有し2〜5個目が消える崩れが実機スクショで発覚。
     Web版`.reach-row`(5列グリッド)/`.reach-btn`に忠実な専用スタイルへ差し替えて解消。
  2. 再生リスト/検索画面の末尾`.hint`テキストを固定表示にしたところ、右下FAB2段と重なる
     崩れが発覚(index.htmlのコード内コメントに残る「reach-rowの5番目ボタンとFABが重なった」
     既知バグと同種)。LazyColumn/LazyVStackの末尾項目として組み込み、リストと一緒に
     スクロールする形にして再発を防止。
- **実機/シミュレータ検証**: Android実機(kyono_testエミュレータ)で新規タブ内容(マイ記録の
  カレンダー・とどくメーター選択、図鑑、じまんカード生成、せんぱいの声のめくり操作、
  オガトレ通信、動画を探す/再生リストのカテゴリ・タグ絞り込み)を実タップで確認。
  さらにオンボーディング4問→かたさチェック4問→診断結果→ホーム→「はじめの1本ガイド」
  (使い方ツアー)まで一気通貫のフローを実タップで再確認し、ロジック・判定に一切変更が
  無いことを確認。iOSはビルド成功のみ(タップ自動化はSystem Eventsのアクセシビリティ権限が
  ない実行環境のため不可。ロジックはAndroidと同型のため確認済みとみなす)。
- **回帰確認(各コミットごとに実施)**: Android `gradle testDebugUnitTest`全緑・iOS `swift test`
  各パッケージ緑(SafetyCore 8/8+111 fixtures・RecordCore 35/35・CardCore 16/16+55
  card-golden)・`npm test` 442 checks green・Web版配信ファイル無変更・gitlink化なし、を毎回維持。
- **今回のスコープに含めなかったもの**: obuIsStaleDate/obuFmtDate(オガトレ通信の日付整形・
  古い投稿の淡色表示)は新規ロジック追加になり「見た目のみ」のスコープを超えるため移植せず、
  既存の生日付表示を維持(コード内コメントに明記)。
...
- **今回のスコープに含まれない(要continuation・次のセッションで着手)**:
  ...
  - ホーム以外の画面(かたさチェック・診断結果・相談室・カレンダー/マイ記録・カード図鑑・
    じまんカード・せんぱいの声・オガトレ通信・使い方・設定)は旧デザインのまま未着手。
- **追記(同日内に対応)**: `PlanProgressCardView`/Android版PlanProgressCard(「肩こり・首こりプラン」
  進捗表示)がKyonoCard未適用のまま浮いて見えていた件も、上記報告の作成中に解消した。
  index.html:414-415 `.bar`(teal系グラデーション進捗バー)・671「やめる」(下線付きテキストリンク・
  ボタンではない)を1:1移植・KyonoCard化。Android実機で進捗バーの割合表示を確認済み。これでホーム画面は
  全カードが一貫したデザインになった(残るギャップは下部タブバー本体のみ)。回帰確認・npm testとも緑。
- 途中経過をalan5へドア配達で報告済み(検収基準5項目のうち達成できたもの・継続項目を明記)。
...
- **iOS**: `ios-native/KyouNoOgatore/SafetyCore/`をローカルSPMパッケージとして新設（`project.pbxproj`を手編集してネイティブXCTestターゲットを足すgitlink/破損リスクを避けるため。判定ロジックを`SafetyGate.swift`1箇所に隔離する構造上の強制にもなる）。`SafetyGateTests.swift`（safety-fixtures.json 111件+norm-golden.json 16件をループでXCTAssert）+`SoudanEngineTests.swift`（crisis>赤旗>通常の優先順位を縛るengine-fixtures）を先に作成。スタブ実装（`swift test`→`safety-fixtures: 42/111 pass`+engine-fixtures 2/3失敗=想定どおりの全赤）を`61f8b38`でコミット後、実装（マスタープラン§3-3の許可リスト数値範囲判定・Unicode.Scalar単位処理・「寝転」除去のNSRegularExpression選択置換）を`f415d0c`で緑化。検収基準5「state/symptom文面分岐のロジックテストが緑」を満たすためengine-fixturesにstate分岐(妊娠中で腰が痛い→answerState文面選択)を追加し4件に(`9504258`)。**最終: swift test 6/6 pass（safety-fixtures 111/111+normゴールデン全緑+engine-fixtures 4/4）**。実装コミット自体はeven-syncの自動コミット`4055155`に先に取り込まれた＝艦隊の共有作業ツリー特性。
- **Android**: エミュレータ不要のプレーンJVM JUnitユニットテスト(`app/src/test/`)として実施。JSONデコードはAndroid SDK同梱org.json（JVM単体テストではスタブで動かない既知の罠）を避けkotlinx.serializationを使用。`soudan-kb.json`を`app/src/main/resources/`（AssetManager非依存のクラスパス経由）に配置。JUnit4 `Parameterized`で111件+16件を1件=1テストケースとして展開（`SafetyGateFixtureTest`/`NormGoldenFixtureTest`）+`SoudanEngineTest`（iOS版と同一のengine-fixtures）。スタブで`da12063`（**148 tests, 84 failed**=iOSと同数の69件fixture失敗+engine-fixtures 2/3失敗=想定どおりの全赤）→実装（Kotlin CharがJS/Javaと同じUTF-16コード単位である点を活かしiOS版よりシンプルに移植。JVMの`String.contains()`は正準等価比較をしないためSwift版で必要だった自前スカラー比較の回避策も不要）で`37cf382`緑化。iOSと同じくstate分岐engine-fixtureを追加(`9504258`)。**最終: gradle testDebugUnitTest 149/149 pass**（JUnit XML集計で確認。fail=0）。
- **grep確認**: `norm`/`crisisHit`/`redFlagHit`/`redFlagKind`の定義は両OSとも`SafetyGate.swift`/`SafetyGate.kt`にのみ存在（UI層等への漏洩なし）。
- **gitlink/file-count確認**: iOS 22=22一致。Android 25 vs 26の差分1件は`local.properties`（Step1から継続の既知の意図的gitignore、gitlinkではない）。
- **PWA無変更確認**: `git diff --stat native-base-2026-07-25..HEAD -- index.html app-*.js soudan-kb.js videos.js sw.js manifest.json scripts/*`が空。`npm test`は引き続き**442 checks全緑**（exit 0）。
- push済み（iOS: `f415d0c`→`9504258`・Android: `da12063`→`37cf382`→`9504258`）。検証ログは`ios-native/verify/step2-safetycore-{red,green}.log`・`android-native/verify/step2-safetycore-{red,green}.log`にコミット済み（alan5への報告用証跡）。
- 次: alan5への完了報告をドア配達。Step 3（データ層）以降はalan5の指示待ち。
...
- **成果物**: [NATIVE-MIGRATION-MASTERPLAN-2026-07-25.md](NATIVE-MIGRATION-MASTERPLAN-2026-07-25.md)（362行・§1〜§8構成）
- **Workflow構成**: Understand（gojiai-appのNATIVE-BUILD-GUIDE/DESIGN.md/MASTERPLAN型・本プロジェクトの現状仕様・安全系111テストの実体・PWA実装構造の5本を並列調査）→Draft（起草）→Audit（書式忠実性／安全性の完全性／実現可能性・忠実性の3レンズで独立査読・単発生成を回避）→Revise（査読反映して最終稿確定）。全10エージェント成功・消費**約55.4万トークン**（承認レンジ内）
- **査読で見つかった重大指摘とその反映**（すべて最終稿で確認済み）: ①本文にキリル文字混入+根拠なき「3週間維持」条件→修正 ②111ケースが検証しているのは本番soudan-kb.jsでなくsoudan-ai-poc/data.mjs（自動生成スナップショット）という依存の見落とし→`verify-kb-sync.mjs`（deep-equal照合）を追加 ③crisis+赤旗混在時の優先順序を縛るテストが1件もなかった→engine-fixtures最低3件をStep 2検収に追加 ④「寝転」除去の実装方式（順次replace）がJS実装(単一パス選択置換)と安全でない方向に乖離しうる敵対ケースを発見→正規表現選択置換に変更 ⑤差分同期のgit diffがpathspec指定で安全系テスト資材を読み落とす内部矛盾→pathspecなし全量抽出に変更 ⑥Step 5が肥大（ホーム+記録+カレンダー+オンボ+ツアー+チュートリアルv2フラグ機械を1ステップに束ねていた）→5a/5b/5cに分割 ⑦相談室エンジンの移植範囲がindex.html:3009起点と誤記（実際は2985〜）→訂正。**査読前の草稿には安全性に関わる見落としが複数あり、単発生成だと本人承認なく出荷されかねない内容だった**（多層査読の効果を実証）。
- **本人TODOの状態は変わらず**（HANDOVER-to-alan5-2026-07-24.md参照）。β配布は「iOSアプリにしてから」との本人方針（7/24）により延期中。
- 次: alan5がこのマスタープランを検収→OKが出たら量産移植(Sonnet)を開始する段取り。マスタープラン自体の妥当性検証（gojiai実例との整合性の最終確認等）はalan5の判断。

## 2026-07-25 alan5がネイティブ移植マスタープランを検収→OK・Step 0発注

通しで読んで検収。破綻・見落としなし（appdev内製の3レンズ査読が既に安全性に関わる重大指摘を複数拾って修正済みだったことも確認）。特に評価した点: ①redflag-safety-test.mjsが本番soudan-kb.jsでなくdata.mjs（生成スナップショット）を検証している依存の見落としを見抜き`verify-kb-sync.mjs`で穴埋めした点 ②crisis+赤旗混在時の優先順序を守るengine-fixturesを追加させた点 ③8月βとの差分同期戦略が「延期継続なら空振り確認で完了」まで両対応で設計されている点（7/24の本人方針を正しく織り込み済み）。`git status`でWeb配信ファイル無変更・`npm test`緑も確認。

HANDOFF.mdの体制セクションに残っていた「β=8月上旬に現行PWAのまま実施」という記述（alan監督経由のリリース階段の文言をそのまま引用したもの・7/24の延期方針と矛盾）を削除し、延期中であることを明記する形に訂正した。

`TASK-C2-2026-07-25-native-migration-step0.md`でappdevへStep 0（断面固定・フィクスチャ/ゴールデン採取）を発注。Step 0は本人操作を必要としないスクリプト作業のため即着手。Step 1（Xcodeプロジェクト作成）に進む段階で、本人へ1通にまとめた依頼（プロジェクト作成手順＋Bundle ID仮値の了承）を送る。

## 2026-07-26 meter.jpg圧縮（判断待ちリスト#6・さぶPC・QAはメインPC復帰後に要実行）

2026-07-20判断待ちリスト#6（meter.jpg 567KB・表示212〜250px幅）を実施。`assets/check/meter.jpg`を900×1350/567KB→**750×1125(max-width:250pxの3x)/94KB(83%減)**にsipsで再圧縮（quality85）。差し替え前後をClaude目視で比較し、文字ラベル（ひざ/すね/足首/つま先/ゆか）・点線・人物輪郭とも劣化なしを確認。画像はcache-first永続キャッシュのため**sw.js C=v64→v65**（2026-07-19エントリの教訓どおり）。

**検証の代替（このさぶPCはnode無し・qa/smoke実行不可）**: sw.jsのJXAパースOK／qa.js・smoke.jsにキャッシュ版数・画像サイズのピン留めが無いことをgrepで確認。コード変更はsw.jsの版数1文字のみ。**メインPC復帰後に `npm test`／`npm run smoke` の通し実行が必要**。

## 2026-07-24 β配布を延期・時期未定（本人メモ「iosアプリにしてから」）

23:09、alanメモ機能経由で本人から: 「きょうのオガトレ配布は延期。時期は未定。iosアプリにしてから」。7/25(土)予定だったfanicon 25人へのβ配布は見送り。Googleカレンダーの仮登録（7/25「#きょうのオガトレ β配布 25人・fanicon（仮）」）は削除済み。技術ゲート自体（テスト442件・安全チェック111/111）は既に通過済みで実装側の準備は整っているため、次の着手条件は「iOSアプリ化のめど」——PWA運用のままの配布は本人の判断で見送られた形。次にこのプロジェクトへ着手する時は、まずiOSアプリ化の状況を本人に確認してから配布再検討をすること。

## 2026-07-21 β配布人数を50人→25人に変更（本人決定）

本人の意向でfanicon β配布を50人→25人に縮小（フィードバック対応を丁寧にさばける規模を優先）。声が少なければ第2弾でもう25人追加する方針。invite-kit.mdの表記3箇所（冒頭方針・β用見出し・スクショ注記）を25人に更新。投稿文本文に人数記載はないため変更なし。アプリ側への影響なし。

## 2026-07-22 母艦からの相談「かたさモンスターガチャ」実装是非→Fable判断（未実装・判断のみ）

母艦alan発の相談ファイル（`2026-07-22-アプリセッションへの相談-かたさモンスターガチャ案.md`。owl-lab/katasa-monsters/で企画進行中の別プロダクト「かたさモンスター図鑑」から、アプリへの小さな実装案として「毎日のチェック後にモンスターくじを1回引ける」ガチャ機能を検討）に対し、Fableとして判断のみ返答（コードは未実装）。

**判断: 条件付きYES（提案の形のままでは非推奨・スコープを縮めるなら賛成）**。理由:
1. **技術面は問題なし**: 既存drawCard()は「Math.random/Date.now禁止・完全決定的」が絶対ルールだが、ガチャ機構をdrawCardと完全分離した独立モジュールにすれば衝突しない。ただし一貫性のため乱数は使わず日付+モンスターindexの決定的ハッシュを推奨（再現性・テスト容易性の両方でアプリの流儀と揃う）。
2. **「記録カード=見返す／ガチャ=引く瞬間のドキドキ」という機能分離の仮説には異論あり**: 既存のカード図鑑（じまんカード）は季節/レア/節目デザインを日付ハッシュで決定的に出しつつ、ユーザーには「今日どのデザインが出るか分からない」体験＝**乱数を使わずに「引く瞬間のドキドキ」を既に実現している**。モンスターガチャを並置すると「収集+サプライズ」を担うシステムが2つ並ぶことになり、母艦が他候補（コンプリートカレンダー等）を退けた「記録カードと役割が被る」という同じ懸念が実質再燃する。
3. **タイミングが最悪**: 本日（7/20〜22）の作業は丸ごと「1日目の削ぎ落とし」（初回ポップアップ廃止・お願いキューを1日1枚化・チュートリアルの一本道化・ホームのカード隠し）だった。「毎日のチェック後にくじ」を今この動線に足すのは、直前まで解消していた「お願い渋滞」を新しい形で再発させる。

**推奨スコープ（縮小案）**: 日次フロー（きょうやった！直後）には一切足さない。マイ記録タブ「🎉お楽しみ機能」内の**受け身な追加タイル**として設置し、1日1回そのタイルを開いたときだけ引ける形にする。ホーム画面・FAB・お願いキューには一切露出しない。アニメはowl方式の純CSS（zukan-anime-demo.htmlと同じ手法）を流用。

**追記（本人との壁打りで結論）**: 「隅に置く」も一歩踏み込みすぎ、との本人指摘を受けさらに縮小。7/21時点で企画側はすでに「アプリに作り込まない・リンク1個で外部図鑑へ」と決めていたので、素直にそれ以上のことをしない=**アプリ側に新規タイル・状態は一切追加せず、モンスター図鑑は完全に独立Web作品として外部リンク1本でのみ接続**が最終的な結論。ガチャ機能自体は向こう側（owl-lab作品）に持たせ、`drawCard`の決定的縛りを気にせず本物の乱数を使ってよい。

**本人確定方針（2026-07-22）**: 「毎日記録カードを集めることに集中してほしい」＝アプリの収集ループは記録カード（カード図鑑）1本に一本化する。競合する第2の収集ループ（モンスターガチャ等）は今後もアプリ本体には追加しない。母艦への返答はチャットで直接実施済み（本ファイルは判断ログ）。実装（外部リンクの追加等）は別途本人指示があれば着手。

## 2026-07-22 本人実機レビュー第3弾: 中央ポップアップ化・オンボチップのグラデ化・記録カード誘導（実装済み）

本人フィードバック（動画+スクショ・音声）3件対応。

1. **はじめてガイドを中央ポップアップに**（「チップのスクロールは分かりにくい・一覧でばんと出して・真ん中にどんと出してもいい」）: `#welcome`をalign-items:center化・`.ob-sheet`を全角丸+max-height:92vh+ふわっと拡大アニメ`obpop`（旧せり上がりobupは廃止）。`#obLog`のmin-heightを60pxまで譲れるようにし、チップは実用上常に全数表示（overflowは超小型画面の最終保険のみ）。**a2hsModalも同じ.ob-sheet共有のため同時に中央化**。
2. **オンボ回答チップを段階色グラデに**（旧・質問ごと単色ローテーションOB_CHIP_VARIANTS廃止）: チップ順にobg0〜obg3（かたさチェックのopt.g0〜g3と同パレット・明→暗・ライト/ダーク・文字は--inkでAA実測済み）。
3. **記録カード保存への誘導**: guideのmarkDone直後、`#makeCardBtn`直上に`#fdCardNudge`「👇 つぎは ここを押してみて」+ボタンに呼吸アニメ。保存→閉じる(closeCard)がツアー自動起動の主経路のため、その入口を動きで示す。fdTourMaybeStartが起動時に両方を片付ける。

恒久テスト: qa 432→442（中央化・obpop・obg段階色の存在+明→暗単調性・fdCardNudge挿入/片付け・obLog 60px。旧OB_CHIP_VARIANTSのピンは「廃止済み」検査に反転——**コメント内にその名を書くとhtml全文検索の廃止チェックに引っかかる**ので注意）。sw C=v64。**qa 442／smoke 29/29／webkit 9/9 全PASS**。実走スクショで中央ポップアップ・グラデチップ・誘導表示を目視確認済み。

## 2026-07-21 チップ見切れバグ修正+かたさチェックへの展開（本人指摘・実装済み）

本人実機（iPhone・スクショ2枚）フィードバック対応。
1. **チップ見切れバグ**: チップ1列化で縦に伸びた結果、`.ob-sheet`(max-height:86vh)内で`#obLog`のmin-height:180pxが譲らず最下段チップが画面外に見切れていた（zoom1.18でさらに悪化）。→ obLog min-height 180→90px（ログはスクロール可能なので譲ってよい）+ #obChipsに`overflow-y:auto`の超小型画面保険。
2. **かたさチェックにタップ明示+段階色**: 選択肢直上に「👇 タップしてえらんでね」固定表示。選択肢に**段階色g0〜g3**（柔らかい=明るいミント→黄→オレンジ→硬い=暗いローズ、ダークモードも同輝度勾配）。数値スコアを持つQ1〜Q4だけ対象・Q5(悩み)は従来色。renderQで`typeof o[2]==="number"`判定でクラス付与。全組み合わせで文字AA実測済み（ライトg2/g3のcritのみ濃色#5C594Eに上書き）。
恒久テスト: qa 423→432（見切れ対策2件・tap-hint・g0〜g3のCSS存在+**ライト背景輝度がg0>g1>g2>g3の単調性を機械検証**）。sw C=v63。**qa 432／smoke 29/29／webkit 9/9 全PASS**。ライト/ダーク両テーマの実走スクショで目視確認済み。

## 2026-07-21 回答チップの1列化+「タップできる」明示（本人指摘・実装済み）

前エントリの続き。本人「一列表示の方いいかも。タップするってわかりやすく表示して。他の回答チップも」対応。
- **オンボチップ1列縦積み**: `#obChips{flex-direction:column}`+チップ全幅16px/14px padding+ボタンらしい立体影(box-shadow 0 3px 0)+押し込み:active。ダークモードは影を濃く。.chipは検索タグ等と共用のためスコープ限定。
- **tap-hint（「タップできる」明示）3箇所**: ①オンボ=毎問チップ直上に「👇 タップしてえらんでね」(obAskQ) ②相談室シート=#sdTapHint「👇 タップでえらべるよ」(sdRenderChipsがチップ空=crisis後noneのときは隠す) ③ホーム相談カード=チップ直上「👇 タップでそのまま聞けるよ」(renderSoudanEntry)。共通CSS `.tap-hint`。
- 恒久テスト: qa 418→423（1列/影/3箇所のtap-hintのピン。関数系はmainScriptを持つcheckTutorialV2側に配置——checkContrastはhtmlしか受け取らない点に注意）。sw C=v62。**qa 423／smoke 29/29／webkit 9/9 全PASS**。実走スクショで縦積み+ヒント表示を目視確認済み。

## 2026-07-21 オンボ回答チップの視認性改善（本人指摘「腰とか肩の文字が視認しにくい」）

実測したところ2つの要因: ①ライトモードのchip-c（桃・Q2悩みで使用）の文字#C2477Eが背景#FFEDF3上で4.15:1とAA未達（他7組=chip-a/b/dのライト・ダークとchip-cダークは達成済み） ②オンボの回答チップが14pxと主役UIにしては小さい。対応: chip-c文字を#B0366E（5.18:1）へ／`#obChips .chip{font-size:16px;padding:12px 18px}`のスコープ付きサイズアップ（.chipは検索タグ等と共用のため全体は変えない。zoom1.18併用で実効約19px）。恒久テスト: checkContrastにチップ4色×2モードの全8組AA検査+サイズピンを追加（qa 405→418）。sw C=v61。**qa 418／smoke 29/29／webkit 9/9 全PASS**。

## 2026-07-21 本人実機レビュー第2弾の修正4件+文字サイズ調整（実装済み）

本人のiPhone実機レビュー（スクショ3枚+音声）対応。

1. **ガイド中の結果画面から「きょうの1本へ」「もう一回チェックする」を非表示**（本人指摘「混乱しそう」）: 両ボタンにid（rGoHomeBtn/rRecheckBtn）を新設しshowResultのguideトグル配列へ追加。一本道=①タップ→もどる→記録以外の分岐を見せない。タブバーからの脱出は常に可能。通常時（翌日以降・再チェック）は従来どおり表示。
2. **オンボCTA直前の案内2枚を削除**（本人指摘「いらない」）: ONBOARDING_SCRIPT.close（「ホーム画面についかすると…」「困ったら、ホームの相談室から…」）を配列ごと削除、obGoは`obSay(rt.say,…)`に。ホーム画面追加は1日目クリア後カード・相談室はツアーが案内するので情報の重複でもあった。
3. **1日目クリア下のカード（A2HS再提案）は残す判断**: 本人「削除していいかな…迷う」に対しFable判断で維持。初回起動ポップアップを廃止した今、記録保全（特にiOS Safari ITP）の主導線がこのカードのため。1日1枚キューで渋滞は既に解消済み。不要なら1行で消せる。
4. **もじの大きさ調整**（本人指摘「まだ小さい」）: ふつう=zoom1.05（従来は等倍）・大きめ=1.12→1.18。zoom依存の座標計算（相談室シート等）はgetComputedStyleで動的に読む実装のため定数変更のみで追従。**副作用対応**: 大きめ1.18でヘッダー「#きょうのオガトレ」が390px幅で2行に折り返す実測→h1を22px→20px+nowrap。

**恒久テスト**: qa 401→405（close削除・obSay形・zoom両値・guideボタン非表示のピン）。sw.js C=v60。**最終ゲート: qa 405／smoke 29/29／webkit 9/9／safety 111/111 全PASS**。Android UA実走スクショでCTA前の吹き出し削減・結果画面下部ボタン消滅・文字拡大・タイトル1行を目視確認済み。

## 2026-07-21 5視点検証の提案A〜F全件を実装（PO「全てやろうか」承認）

下の検証エントリのA〜Fを一括実装。

- **A: 初回起動時a2hsポップアップ廃止**: 起動IIFEは`obOpen()`直行に変更、`a2hsBoot()`は関数ごと削除し環境分岐は`a2hsKindFor()`（app-env.js）に一本化（旧「qa.jsがa2hsBoot本体を検査するため委譲禁止」制約は解消）。導線は1日目クリア後カード+7/14日homehint+gd-mamoriの3つ（総量維持）。
- **B**: オンボ挨拶2枚目に「ぜんぶ無料・とうろく不要🆓 あんしんしてね」（「無料」が初回導線に登場するのはここが初）。
- **C: ガイド中の結果画面削ぎ落とし**: rHope/rPT/rPace(id新設)をhiddenトグル・②③非表示（`rx.slice(1)`廃止）・worryExtra/rSoudanLinkも非表示・rxHead「きょうはこの1本だけでOK！」+「あと2本とくわしい解説は あしたから見られるよ🌱」注記。v1の「②③はそのまま表示」設計を上書き。実測で結果画面の全高約35%減。
- **D: 動き3点**: `.fd-point`「👇 ここを押してみて」ボブアニメ（結果画面①の直上）／guide cheerに`card-sample.png`の`.fd-cardpop`ぽんっと出現演出／`fdFocusHome()`がdoneBtnに`.fd-breathe`呼吸アニメ。**全部`@media (prefers-reduced-motion: no-preference)`内**＝設定次第で静止。
- **E: fdFocusHome複数日貼りつきバグ修正**: `obGo`で`fdday`（ガイド開始日）を保存し、ホーム絞り込み+チュートリアル中FAB非表示は`fdActive()&&fdday===todayStr()`の当日限定に。記録せず離脱した人の翌日は通常ホーム。fd="go"自体は残る＝翌日記録してもguide cheer/ツアー予約は有効。
- **F**: 練習宣言の吹き出しにOS別「もどりかた」1行（iOS=左上の◀/YouTubeをとじる・Android=もどるボタン・その他=ブラウザのもどる）。
- **追加（実走で発見）**: チュートリアル当日はオガトレ通信FABも全面非表示（結果画面の練習吹き出しに被っていたため。翌日から通常表示）。

**恒久テスト**: qa 390→401（checkTutorialV2追補7件+checkA2hsPopupをa2hsKindFor基準に全面書き換え+showResult/obGoピン差し替え）。smoke 7cを全面書き換え（全UA環境で「起動→welcome直行・ポップアップなし」→`a2hsShowForce()`で環境別kind表示を検証・シナリオにラベル+finallyでpage close）。**smokeハマりどころ4件（2026-07-21実測）**: ①welcomeシートのスライドイン中のp.click空振り→450ms待ち+evaluateクリック ②doneBtn/rDoneNudgeBtnはnudge-pulse/fd-breatheのtransformアニメ中に空振り→evaluateクリック ③android-menuの閉じるボタンは「わかった」（「あとで」ではない）④7cのシナリオ失敗でページをclose()せず放置すると後続ステップがprotocolタイムアウトで連鎖死。sw.js C=v59。

**最終ゲート**: qa 401／smoke 29/29／webkit 9/9／safety 111/111 全PASS。Android UA実走スクショで「起動→ポップアップなしでwelcome直行（無料明示）」「削ぎ落とし結果画面+指差し」「カードぽんっ演出」を目視確認済み。

## 2026-07-21 初日・初回体験の5視点検証（PO依頼「動きを入れたり削ぎ落としたり」・Fable・報告のみ→同日、上のエントリで全件実装済み）

チュートリアルv2実装直後のPO依頼。5視点=①最初の30秒（要求の後送り）②安心（お金の不安）③1画面1メッセージ（文字量）④動きで導く⑤一本道から外れた翌日。**提案6件・未実装（PO選定待ち・詳細は提案メッセージ参照）**:
A) 初回起動時のa2hsポップアップ廃止→1日目記録後カード+既存7/14日homehint再提案に一本化（①・初対面のインストール要求は「広告」反射を誘発。ITPリスクへの導線総量は維持）
B) オンボ冒頭に「ぜんぶ無料・とうろく不要」明示（②・現状「無料」はFAQ内にしか存在しない）
C) ガイド中の結果画面から②③動画と長文解説を削ぎ「タイプ+①+ボタン」だけに（③・v1設計「②③はそのまま表示」の上書き）
D) 動き3点=①カード指差しゆらし／1日目クリア時の記録カードぽんっと出現演出／doneBtn呼吸パルス拡張（④・すべてprefers-reduced-motion尊重）
E) **fdFocusHomeの複数日貼りつきバグ級発見**（⑤・要修正）: fdActive()=fd==="go"&&total===0は記録せず離脱すると翌日以降も真→ホームが何日も1カードのまま「壊れた」見えに。当日限定化が必要
F) 練習宣言に「YouTubeからの戻りかた」1行（⑤・タスクスイッチできない層が最初の脱落点）

## 2026-07-21 1日目チュートリアルv2「一本道」を実装（PO「GO」承認・推奨デフォルト採用）

下の設計案エントリをそのまま実装した。採用デフォルト: カレンダー=2日目お願いカード維持／「試し押しで1日目成立」=締めスライドに一行添える案(b)。

1. **結果画面の吹き出し=練習宣言**（app-quiz.js）: 「ここからは練習だよ🏫 ①を試しにタップ→YouTubeがひらいたら すぐ戻ってきてね（ぜんぶ見るのは あとでゆっくりでOK）」
2. **ガイド中(未記録)のホームを1カードに絞る**: `fdFocusHome()`（index.html）がtodayCard/ckCard/soudanCard/anchorCardを専用クラス`.fd-hide`で隠す（各カード自身のhidden管理と非干渉）。renderHomeから毎回呼び、記録後(fd=1)は自動復帰。ホームが1画面に収まるためスクロール誘導が不要になった。※これによりrenderTodayのfdGuide①だけ表示分岐は実質不可視（コードは残置・無害）。
3. **記録直後に図鑑の肝**（app-record.js guide cheer）: 「きょうの記録が1まい目のカードになったよ ためると図鑑がうまっていく📖」
4. **使い方ツアーの自動起動**: markDone(guide)が`tourpend`を予約→`fdTourMaybeStart()`（index.html）が「区切り」=closeCard()/switchTab()で拾い、一度だけ`obOpenTour()`（tourpend消費+tourseen付与）。メモ入力には割り込まない。スキップ常時可。
5. **締めスライド`OB_TOUR_CLOSING`**: 自動起動時のみ`obTourSteps()`が末尾に足す（使い方タブからの再訪ツアーには出ない）。文言=「あしたも待ってるね🌱 きょうのぶんの動画をちゃんとやるなら ホームの「きょうの1本」からどうぞ💪」。obTourEndでフラグリセット+fdFocusHome解除（シートの裏で復帰させレイアウトジャンプを見せない）。
6. **ツアーの図鑑スライド追加**（7→8枚・記録カードの直後）+マイ記録スライドに「カレンダー通知は続ける設定から」1行追加。
7. **お願いキューはa2hs→calの2枚に**（tourエントリ削除）。`#tourAsk`はDOMごと廃止（renderStreakの残骸クリアからも除去）。

**恒久テスト**: qa 369→390（`checkTutorialV2`新設19checks+checkTourSlides8枚化+キュー2枚化などピン差し替え）。smoke 7dを全面改修: 1日目=カレンダーのみ+tourpend予約+シート非起動→タブ移動で自動起動（1/9表示=締めスライド実在）→8クリックで締めスライド到達（文言+おわる）→2回目のタブ移動で再起動しない→fd-hide解除、まで実測。**1cのハマりどころ**: オンボtodayルート通過後はfdFocusHomeで#ckBtnが不可視→fd除去+renderHome()してから待つ。sw.js C=v58。

**最終ゲート**: qa 390／smoke 29/29／webkit 9/9／safety 111/111 全PASS。Android UA実走スクショで「チュートリアル中ホーム=続けた日数カード1枚」「締めスライド」を目視確認済み。

## 2026-07-21 かたさチェックQ3の最上位基準を「頭より上」→「鼻より上」に修正（本人指摘・実装済み）

本人のiPhone実機レビューで「一番できる項目は正しくは鼻より上まで上がる」との指摘。`app-quiz.js`のQUESTIONS[2]選択肢1を「鼻より上まで上がる／ひじをつけたまま鼻の高さをこえる」へ、QUIZ_ART[2]のガイド線・ラベルを「あたま(y45)」→「はな(y57)」に修正（あご線は微調整のみ）。qa 369／smoke 29/29 PASS。

## 2026-07-21 1日目チュートリアルv2「一本道」設計案（本人の実機動画フィードバック起点・提案中・実装はまだ）

本人の実機動画（記録カード保存→共有シート→閉じたあと行き場がなく迷子になる様子）と口頭要望を受けた再設計案。要望の骨子: ①記録(+カード保存)後に使い方ツアーを自動ポップアップ ②チュートリアルは削ぎ落とした強制一本道（チェック→動画は「試しに押してすぐ戻る」→記録→図鑑が肝）③チュートリアル中はきょうの1本カード非表示 ④カレンダーの位置は迷い中。**Fable設計案の詳細は本人への提案メッセージ参照。骨子**: ガイド中未記録のホームを「続けた日数」カード1枚に絞る／結果画面吹き出しを練習宣言に／記録後に図鑑の肝を一言→記録カードモーダルclose等の「区切り」で使い方ツアー自動起動（スキップ常時可）／ツアーに図鑑スライド追加（7→8枚）／お願いキューからツアーを外しa2hs→calの2枚に／カレンダーは2日目キュー維持を推奨。PO判断待ち: 全体GO・カレンダー位置・「試し押しで1日目成立」の扱い。

## 2026-07-20 デジタル弱者監査 第2弾の全9件を一括実装（PO「全て実装すべき」承認）

下の監査エントリのTop3+その他6件を全部実装した。

1. **お願いカードの1日1枚キュー（Top1）**: markDone直後の3枚同時表示（A2HS再提案+カレンダー+ツアー）を廃止し、`askQueue`（app-record.js）で優先順=①a2hs2→②calseen→③tourseen（新設キー）の「その日の1枚」だけ表示。出せない環境の候補（デスクトップ等でa2hs不適合）はフラグだけ消費して同日中に次候補へ繰り上げ（従来の「再訪のたび判定し直さない」流儀）。典型: Android/iOS実機は1日目=A2HS→2日目=カレンダー→3日目=ツアー。デスクトップは1日目=カレンダー→2日目=ツアー。カードの見た目・文言・「あとで」挙動は全て従来どおり。renderStreakの残骸清掃に#a2hsAskも追加。**副次的な仕様変更**: ツアーカードは旧実装では「はじめの1本ガイド完了者だけ・1日目のみ」だったが、キュー化でオンボスキップ民にも1度だけ出るようになる（デジタル弱者には益と判断）。
2. **チェック後のモード粘着リセット（Top2）**: `finishQuiz()`でタイプ保存後に`state.mode=null`+`mode_manual`リセット。チェック前のホーム描画で「よる」等に固定されたstate.modeが残り、結果画面「きょうはこれ1本でOK！」と別動画がホームの主役になる連続性切れを解消（renderTodayの既定`mineAvail→"mine"`が効くようになる）。Android UA実走で結果①=ホームあなた用①（肩甲骨基本12分）の一致をスクショ確認済み。
3. **記録引っ越しの※脚注廃止（Top3）**: gd-mamori手順1のiPhone枠内に「だいじ！置いたあと…記録をコピー→よみこむ（場所つき）」を明記、Android枠に「記録はそのまま引きつがれます」を追加、全員向け※脚注（下の3.と同じやり方です）は削除。
4. **FAB非表示の拡大（④⑤）**: `updateFabs()`で相談室FABをguide/resultでも非表示（FAQ見出し▾の物理被り・結果画面の二重導線対策）。オガトレ通信FABはguideのみ非表示（他タブは従来どおり）。
5. **使い方タブのdetails折りたたみ（⑧）**: gd-start/daily/mamori/tsuzuku/myrecを`<details class="card gd-fold">`化（gd-startのみ既定open）。gd-help/gd-faqは常時表示のまま。目次下に「見出しカードは タップするとひらきます」ヒント追加。`gJump()`は元から対象/先祖のdetailsを自動で開くため目次チップ・🆘ボタンからの遷移は常に開いた状態で表示される。実測で全高16216px→8596px（約47%減）。
6. **文言3件（⑥⑦⑨）**: ダチョウcopyを新Q3基準「ひじをつけたまま上がらないのは…」へ／オンボQ0返答に「マイ記録」タブの場所案内を追加／ガイド上部説明を絵文字参照からボタン名ベースへ。

**恒久テスト**: qa 345→369（`checkGuideDwAudit20260720`新設24checks+checkFirstDayGuide/checkA2hsYoutubeRedesignのお願いキュー対応で旧ゲート正規表現を差し替え）。smoke 7dを新仕様に改修（1日目=カレンダーのみ・#tourAsk/#a2hsAsk空・フラグ消費/温存の実測→7bb流儀の日送りで2日目markDone→ツアーカード出現・タップでツアー開始）。**7dのハマりどころ**: 7d失敗時は後続7f（フォーカス管理）が巻き添えで落ちる（welcomeシート残留のカスケード）。sw.js C=v56→v57。

**最終ゲート**: qa 369／smoke 29/29／webkit 9/9／safety 111/111 全PASS。Android UA実走スクショで1日目=A2HSカード1枚のみ・あなた用①一致・guideタブFABなし・折りたたみ表示を目視確認済み。

## 2026-07-20 初回動線+使い方タブのデジタル弱者監査 第2弾（PO依頼・Fable・報告のみ未実装→同日、上のエントリで全9件実装済み）

Android Chrome UA・390x844の実走（フレッシュ→a2hsポップアップ→オンボ4問→かたさチェック→結果→初日ホーム→きょうやった！、スキップ経路、使い方タブ・FAQ・ツアー）でスクショ約20枚を撮って精査。**修正はまだしていない（PO選定待ち）**。実走ハーネスはscratchpad/walk.js（セッション終了で消える・smoke.js流儀のUA偽装+ポップアップ通過処理が要点）。

**Top3（Fable推奨順）**:
1. **1日目達成直後の「お願い渋滞」**: きょうやった！1タップで お祝い2段+A2HS再提案カード+メモ欄+記録カード保存+カレンダーカード+使い方ツアーカードが縦一列に同時表示（実写確認）。黄色主ボタンが同時2つ（やり方を見る/使い方ツアーを見る）・「あとで」も2つ。各カードは別セッションで個別承認された正しい部品だが、積むと成功体験の頂点が最も混雑した画面になる。→提案: 優先度順の1日1枚キュー化（1日目=A2HS〈記録安全上最重要〉→2日目=カレンダー→3日目=ツアー。既存の一度きり制御を順送りに変える）or 1枚の「つぎの一歩」カードへ統合。
2. **「はじめの1本」の連続性切れ**: 結果画面で「きょうはこれ1本でOK！」（タイプ別①）を約束した直後、ホームの「きょうの1本」は時刻基準タブ（夜なら「よる」）の別動画が主役になる（実写: ①=肩甲骨基本12分 vs ホーム=夜専用15分）。fdガイド中は「あなた用」タブを既定にして①と同一の1本を見せるべき。
3. **記録引っ越し（コピー→よみこむ）が※脚注**: ホーム画面追加後のブラウザ→アプリ版の記録移行は iPhone では必須（Safari A2HSはストレージ別）なのに「※…下の3.と同じやり方です」の脚注+相互参照で記載。Android（Chrome WebAPK）はそもそも不要な手順で、全員向け脚注は両OSに不親切。→iPhone枠内に手順として明記・Android枠からは消す。

**その他**: ④FAB2段がFAQグループ見出し▾を物理的に覆う位置がある（実写・既存提案#4の実害追認）／⑤結果画面でFABが「ひとくち解説」本文に被る（既存提案#5と同根）／⑥ダチョウcopy「背中で手がつながらない」が新Q3（両ひじ）と不一致（既知保留の再確認）／⑦オンボQ0返答の「続ける設定」に場所案内なし（「マイ記録→続ける設定」へ）／⑧使い方タブ全高約10画面（ただし目次チップ+目次へ戻る構造は良い・details折りたたみで圧縮余地）／⑨ガイド上部「🌱は…📖は…」の絵文字参照説明が回りくどい。

**良くできていて触るべきでない点**: 「おぼえるのはこれだけ！」3行宣言／Q1の実写お手本＋大きい設問文字／チャット形式オンボ4問のテンポ／FAQ検索と0件時の逃げ道文言／スキップ民ホームのかたさチェック先頭出し分け／「サボってもだいじょうぶ 責められません」。

なお実走中、新タイブレークの実動作も確認（Q1〜Q4全て3番目=全部2点の4部位同点+悩み肩こり→ダチョウ）。

## 2026-07-20 かたさタイプ判定の同点バイアスを修正（悩みタイブレーク+日付ローテーション・PO承認）

本人の「タイプの出現に偏りがある気がする」を受けて`decideType()`（app-quiz.js）を検証したところ、構造的偏りを確認。旧実装は最高点部位を`momo→koka→kenko→ashi`の固定順走査（`>`のみで交代）で選んでいたため**同点は常に先頭側の勝ち**になり、全回答等確率の理論分布（256通り）でモモンガ27.3%・ペンギン12.9%と2倍超の偏り。タイプ判定に進むケースの約27%（69/256）が同点で、その勝者内訳はmomo37/koka22/kenko10/**ashi 0**（ペンギンは単独最下位柔軟性のときしか出られなかった）。

**新実装（本人「めちゃ良い」承認済み）**: 同点タイブレークを2段に。①Q5「いちばんの悩み」で自覚症状に寄せる（`WORRY_TIEBREAK`: katakori→kenko／yotsu→momo,koka。tsukare/yawarakaは部位に紐づかないため無指定）。②悩みで決まらなければ`rotationIndex()%同点数`の日付ベース決定的ローテーション（乱数なし・drawCard再現性原則と整合）。robot(total>=9)/yawara(total<=2 or max<=1)のゲートは不変。`decideType(s,worry)`と引数化し`finishQuiz()`から`state.scores/state.worry`を渡す形に（presetWorry経路もQ5経路も語彙は同一キーなので両対応）。呼び出し元はfinishQuizのみ。

**恒久テスト**: qa 331→343（`checkQuizTypeTiebreak`新設・12checks）。checkRxRotationと同じrotationIndexスタブのvmサンドボックスで`decideType`を直接実行し、ゲート不変・単独最高点不変・悩みタイブレーク・ローテーション再現性に加え、**全256通り×r=0..11合算で4部位の当選数が完全一致（各603）** を機械検証（12日周期は同点数2/3/4をすべて割り切るため理論上ちょうど揃う）。smokeは全問先頭選択肢（全0点→しなやかネコ）でタイプ同点に非依存のため変更なし。qa 343／smoke 29/29／webkit 9/9 全PASS。

## 2026-07-20 配布計画の確定を反映（β=fanicon 50人→コメント→本番=YouTubeコミュニティ投稿）

本人確定: **βはfanicon（オガトレ部）内で50人だけに配布→コメントをもらって本番環境整備→本番はYouTubeコミュニティ投稿で公開**の2段階。invite-kit.mdを「β用fanicon版（投稿文+本人撮影のスクショ指示+βで集めたい声3点）」+「本番用YouTube版（保管）」の2部構成に再編。アプリ側のA2HS再設計（脱出→追加の2段構え・YouTube referrer検出等）は両段階で有効なので変更なし。faniconアプリ内ブラウザの脱出UIは実機依存のため、スクショは本人がfanicon実機で撮る方式にした（撮った実画面が正になる）。本人TODO: ①スクショ撮影 ②β投稿文の最終チェック ③新文言の実機ざっと見 ④配布実行（+急がない: チェック用写真2枚・本番用ショート動画）。
...
**Top3（Fable推奨順）**:
1. iOS Safari分岐の手順が実際の4ステップ中2つしか書かれていない（「共有シートを下にスクロール」「右上の追加」が欠落）+視覚アンカーが文字だけ→絵入り4ステップ化（app-env.js:60・iPad文言出し分け込み）
2. 初回ポップアップが一発勝負（「あとで」1タップで導線がほぼ終了・次接点は3日目バナー）→1日目「きょうやった！」成功直後の再提案カード+使い方タブから案内再表示ボタン
3. ios-other分岐が実行不能な案内（iOS Chromeに「Safariで開く」メニューは存在しない・iOS16.4+ならChrome共有→ホーム画面に追加が直接可能）→文言修正

**その他の発見**: LINE内ブラウザ警告が13px・実行手段なし／gd-mamori再発見経路にAndroid手順なし・FAQに直球項目なし／invite-kit③にスクショ枠なし（撮影メモにも共有シートなし。**尾形さん本人の60秒ショート動画が最強の武器**との提案）／追加もブックマークもしない人はSafari ITPの7日非訪問でlocalStorage消去=1週間休むと記録全損リスク（おやすみ券でカバー不能・これは構造的制約として認識共有）

**良くできていて触るべきでない点**: 進行不能ゼロ設計・Androidのネイティブprompt活用（実走で発火確認）・配布リンクのopenExternalBrowser=1・タップ寸法全44px超・バナー再提案の7/14日ロジック。

## 2026-07-20 C1検証依頼3件の総括（C2 Fable艦隊・監査3隻+修正1隻）

VERIFICATION-REQUEST-2026-07-20.mdの3依頼を完遂（各詳細は本日の先行エントリ群）。**修正済み**: 赤旗kw34語追加（すり抜け16→6件・safety 111/111）／修正バッチ9件（crisisチップ抑止=承認済み設計への復元・FAQ検索sdNorm正規化・gJump×絞込バグ・プラン中文言・転記演出silent化・コントラストAA・a2hs FAB・最下部padding・図鑑✕44px）。**最終ゲート: qa 302／smoke 27/27／webkit 9/9／safety 111/111**（司令塔の独立再実行でも確認）。

**本人判断待ちの提案リスト（次のセッションはここから拾う）**:
1. 赤旗kw追加4件（誤爆リスク高くFableが見送り）: 「動かせなくなっ」（四十肩相談を全ブロックする懸念）／「むくんで痛」（立ち仕事むくみ相談を巻き込む）／「体重が急に減」（ダイエット成功報告誤爆）／「痛くて動けな」系（筋肉痛の誇張表現誤爆）
2. 脳卒中サイン（ろれつ・顔の片側下がり）: 2026-07-14のPO保留判断のまま今回もすり抜けを再確認。優先度再考の材料として返す
3. 赤旗/crisis直後の陽気なタイプ挨拶（「つっぱりモモンガのあなたには〜」）抑止: sdFlavorShown=trueの1行で可能だが演出仕様変更のため本人YES/NO
4. 使い方タブでFAB2段が🆘ボタン右端に重なる（47x31px等・実測）: guideタブでもsoudanFab非表示にするか右マージンか
5. 結果画面の相談室二重導線（rSoudanLink+FAB）: ホームでFABを消した論理と同型（観察・実害小）
6. ~~meter.jpg 567KB（表示212px幅）の圧縮~~ → **完了（2026-07-26・750px/94KBに圧縮・sw C=v65・QAはメインPC復帰後）**
7. タップ44px未満の残り: FAQグループ見出し(27.5px)・summary行3つ(29-31px)・フッターリンク(22px)・obSkipBtn(37.7px)
8. 過去の骨折話（「昔骨折したけど平気」）にも赤旗が出る現行仕様: 安全側の意図的動作だが文言「診てもらってほしいな」が過去形の話には少しズレる（低優先）

依頼3の証跡（4サイズ×2テーマ・スクショ約110枚・結果JSON）はC2セッションのscratchpadにあり、セッション終了で消える。重大な崩れゼロという結論と上記の指摘のみが恒久記録。VERIFICATION-REQUEST-2026-07-20.mdは役目を終えたため削除（内容はgit履歴ddad447周辺に残存）。
...
- **すり抜け16件のうち12件をkw追加でカバー**: `soudan-kb.js`の`redFlags.kw`に34語追加(産んだばかり系3語はstateKwにも追加=主治医OK待ち文面)。各語は無害文プローブ(「子どもが生まれたばかり」「予定が立てられなくなった」「尿もれが気になる」「お風呂で体がじんじん温まる」等15文)で誤爆なしを事前確認済み。特に危険だった誤マッチ: 「足がジンジンして眠れない」→nemuri(睡眠ストレッチ動画)、「産んだばかりで骨盤矯正」→kotsuban2(骨盤矯正動画)、「頭を打ってから頭痛」→zutsuu、「立てなくなった」→youtsuu。
- **kw追加を見送り提案に回した4件**(誤爆リスクに迷いあり・司令塔経由で本人YES/NO待ち): D01「動かせなくなった」(四十肩ユーザー誤爆懸念)/D17「むくんで痛い」(立ち仕事むくみ=mukumi核ユーザー誤爆)/D29「体重が急に減って」(ダイエット報告誤爆)/D33「痛くて動けない」(筋肉痛の誇張表現誤爆)。
- **既知の保留(kw追加せず)**: D16「ろれつがまわらない」/D30「顔が片方だけ下がってる」=脳卒中サイン系。2026-07-14にPO保留判断済みの領域のため別枠報告のみ。
- 恒久テスト: `redflag-safety-test.mjs`に攻撃テスト20ケース+state判定1ケースを追加→**111/111 pass**。`npm test`=281checks全PASS。`node soudan-ai-poc/build-data.mjs`でdata.mjs再生成済み。
- トーン所見(修正せず報告のみ): 赤旗/crisis回答自体は絵文字・キャラ名なしの真面目な文面でOK。ただし①crisis後もインテントチップ一覧が出る(コードコメント「チップを出さず」と不一致)、②赤旗/crisis直後にチップを押すと`sdFlavorShown`が未消費のため「つっぱりモモンガのあなたには、これが効くはず！」が次の回答冒頭に出うる。②は`sdAnswerRedFlag`/`sdAnswerCrisis`で`sdFlavorShown=true`にする1行で抑止可能(本人判断待ち)。

## 2026-07-20 使い方タブ「はじめてガイド/使い方ツアー」ボタンの色分け・下の説明文との余白を修正

本人指摘「色違う方が良くない？」「その下の説明と距離近くない？」に対応。両ボタンとも`.daychip`クラス(teal-soft背景)を共有していて実質同色だったため、`#obTourLink`に`.daychip-alt`クラスを追加し黄系(`--yellow-soft`背景・`#8A6D00`文字、ダークモードは`#E8C74C`文字)に変更。ボタン行と直下の説明文の間の余白も`margin-bottom:6px`→`12px`(ボタン行)・説明文側`margin-bottom:14px`→`18px`に拡大。`npm test`=281checks、`npm run smoke`=25/25で確認、ローカルサーバー+Claude-in-Chromeで実際に色分けと余白を目視確認済み。

## 2026-07-20 かたさチェックQ3(kenko)を本人のYouTube「肩甲骨12分」動画の実際のチェック内容に修正

本人指摘「Q3の内容が違う。肩甲骨12分動画にあるような、肘合わせ→上げのチェックが正しい」に対応。参照動画(ZYTlwh_FhoU「【基本版12分】肩甲骨が『超』硬い人向け」)の「肩甲骨の硬さチェック」スクショ3枚を見て、実際の判定基準(①頭より上／②あごより上／③ひじはつくけどあまり上がらない／④ひじがそもそもつかない)を確認した上で書き直した。

- `app-quiz.js`のQUESTIONS[2](kenko)を、旧「指先を鎖骨において ひじはどこまで上がる？」(単発の片腕上げ・眉/鼻/あご基準)から、新「胸の前で両ひじをつけて上げると どこまで上がる？」(両ひじを合わせて上げる・頭/あご基準+ひじが付くかどうかの基準)に全面差し替え。
- QUIZ_ART[2]のSVG図解も、片腕を上げるだけの絵から、両腕がVの字に合流してひじが触れ合い(ピンクの丸で強調)、さらに上で手が合わさって上げる動きを描く絵に描き直し(あたま/あごの点線ガイド付き)。
- `TYPES.kenko`(飛べないダチョウ)のflavorテキスト(背中で手がつながらない、という別の古典的テスト風の表現)は今回の指摘範囲外と判断し、あえて変更していない(気になるようなら別途対応)。

`npm test`=281checks、`npm run smoke`=25/25、`npm run smoke:webkit`=9/9で確認済み。加えてローカルサーバー+Claude-in-Chromeで実際にQ3画面を表示し、新しい設問文・note・イラスト(両ひじが合流して上がる絵)が正しくレンダリングされることを目視確認済み。

## 2026-07-20 壁打ち3・4件目「ホームカード並び順」「動画タグ集約」は現状維持と判断

7/19の5視点アプリ全体レビューで見送った4件のうち、残り2件を本人と壁打ち。いずれも「実際に使っていて困った」ではなく「レビューを見て気になった程度」との回答で、かつ実コードを確認すると懸念の大半は既に解消済みだったため、対応不要・現状維持と判断した。

- **ホームカードの並び順**: `renderHome()`と起動時IIFEに、既に賢い出し分けロジックがある。未チェックのユーザーは`ckCard`(かたさチェック)が`anchorCard`より前に自動移動し、チェック済みになると`streakCard`(続けた日数)の直後(=実質最後尾のミニ表示)に移動する。返ってくる日常ユーザーが実際に見る順は「相談室→きょうの1本→続けた日数→(下部)かたさチェックmini」で、最重要の2枚は埋もれていない。相談室が先頭に来る点だけはやや気になるが、実害の報告がないため見送り。
- **動画タグの大幅集約**: `app-search.js`のTAG_CATSで既に4カテゴリ(からだの場所/時間・シーン/目的/その他)にタグを分割済み。一度に全26タグを見せているわけではなく、最多の「からだの場所」でも11個。横スクロールのフェードヒントもあり。レビューは「全タグ一括表示」という誤った前提だったため、実態としては大きな問題ではないと判断。

これで7/19の5視点レビュー起点の壁打ちは全4件完了(実施2件・現状維持2件)。

## 2026-07-19 とどくメーターとかたさチェックQ1の二重計測を統合(壁打ち2件目・A案=自動転記)

本人指摘「とどくメーターとかたさチェックが同じ前屈動作を二重に測っている」に対応。壁打ちの結果、A案(自動転記)で確定。`app-quiz.js`の`finishQuiz()`で、とどくメーターの記録が1件も無い場合に限り、Q1(momo・立って前屈)の回答をとどくメーターの初回記録として自動で書きこむ。対応表`REACH_FROM_MOMO=[5,4,2,1]`(クイズの「床につく/つま先/すねの途中/ひざ下」4段階→メーターの「ひざ/すね/足首/つま先/ゆか」5段階のうち対応する段位)。メーター側の「足首」に相当する選択肢はクイズに無いため自動では選ばれない(スケールの粗さの違いによる仕様上の許容差、両者は完全一致しない)。既にとどくメーター記録がある場合は上書きしない(ユーザーが自分で測った値を優先)。結果画面(`#rReachNote`)に「📏 いまの前屈「◯◯」を とどくメーターにも記録したよ」と一言表示し、静かに書き換わって困惑しないようにした(2回目以降・記録済みの場合は表示なし)。

`npm test`=281checks、`npm run smoke`=25/25、`npm run smoke:webkit`=9/9に加え、ローカルサーバー+Claude-in-Chromeで実際にmomo=1(つま先)→reach lv=4「つま先タッチ」の自動記録と一言表示、および2回目(momo=3)で上書きされない・一言も出ないことを直接確認済み。

## 2026-07-19 マイ記録「続けた記録」カードの重複感を解消(カード機能一元化の壁打ち1件目)

本人指摘「記録カード(続けた日数)がホームとマイ記録にあるのはみづらい・重複している感」に対応。壁打ちの結果、じまんカード/カード図鑑は現状維持(使用頻度低い/マイ記録に置いておきたいとの回答)、「続けた記録」カードのみ改修。方針は「A: マイ記録側の数字表示を控えめにする」。`#history`の「続けた記録」カードで、通算/いま連続の大きい数字(36px)を先頭に置く構成から、①次のお祝いまでの進捗メッセージ+バー→②通算/いま連続(22pxに縮小・インライン表記)→③おやすみ券、の順に変更。ホームの「続けた日数」カードは「きょう」の達成感を担い、マイ記録側は「軌跡とごほうび」を担う、という役割分担にした。`npm test`=281checks、`npm run smoke`=25/25(1回目2-かたさチェック完走、2回目1c-カレンダー登録カードがそれぞれ無関係な一過性flakyで失敗したが3回目で25/25クリーン)、`npm run smoke:webkit`=9/9で確認済み。

## 2026-07-19 アプリ全体をFable5視点(デジタル弱者視点を重点)でUXレビュー・対応

本人指示「アプリ全体を5つの視点から検証して。特にデジタル弱者の視点が大事」に対応。全画面・全タブの構成をExploreエージェントで棚卸ししたテキストを共有コンテキストとして、5体のFable並列エージェント（デジタル弱者・高齢者／初見ユーザー／情報構造・タブ横断一貫性／安全性・医療的配慮／モバイル操作性、デジタル弱者視点は指示文を厚くし詳細度も800字まで拡張）にレビューさせた。全視点で総合3.5/5、複数視点にまたがる指摘（FAB縦積みの誤タップ・重複が5視点中4視点で指摘）を中心に対応。

**実装した項目**:
1. **相談室FABをホームタブでは非表示に**（`updateFabs()`に`currentSection==="home"`条件を追加）。ホームには相談室カードが既にあるため、FABとの二重導線・縦積み誤タップを解消。ホーム以外のタブでは引き続きFABから開ける。`scripts/smoke.js`の「7f-モーダルのフォーカス管理」テストは、相談室FABがホームで非表示になったことで`el.focus()`が効かなくなるため、historyタブに切り替えてから検証するよう修正。
2. **ホーム画面追加の案内を強化**: `app-env.js`の`a2hsShow()`でiOS Safari向け・Android「⋮」メニュー向けの説明文を①②の番号つき手順に変更。ホーム下部の再案内バナー(`envBanner`)のリンクを「使い方タブの先頭」ではなく`gJump('gd-mamori')`で「記録を守る」カードへ直接ジャンプするよう変更。
3. **記録のひっこしにクリップボード自動読み込みボタンを追加**(`importFromClipboard()`)。エクスポート側は既に`navigator.clipboard.writeText()`で自動コピー済みだったが、インポート側は手動貼り付けのみだったため、`navigator.clipboard.readText()`で1タップ読み込みを追加（非対応環境は手動貼り付け欄にフォールバック）。QR方式も検討したが、自作QRエンコーダの動作検証ができないリスクを踏まえ見送り、より低リスクで効果の近いクリップボード自動化を採用。
4. **かたさチェックのFAQ「ストレッチ中に痛かったら？」に危険サインの具体例を追加**（ビリッとした鋭い痛み/しびれ、力が入らない、安静時痛・夜間痛、発熱・腫れ）。soudan-kb.jsの赤旗システム（90件のキーワード＋クライシス対応、redflag-safety-test.mjsで90/90検証済み）は既に非常に手厚いが、チャットに入力しなくても静的に読める形の自衛情報がFAQに薄かったため補強。
5. **「あなた用」セグメントに一言説明を追加**(`#segMineHint`、`renderToday()`でsegMineと同じ条件で表示切替)。「あなた用＝かたさチェックの結果に合わせたおすすめ3本」という独自ネーミングの意味を初出時点で明示。

**確認の結果、既に実装済みだったため対応不要と判断した項目**（レビューはテキスト要約ベースのため実コードとの齟齬があった）:
- かたさチェックの身体感覚の設問（あぐら/鎖骨に手を置いて肘）→ 既に「こぶし1個ぶん」「まゆ/鼻/あご」等の具体的な基準＋Q1/Q2は実写真、Q3/Q4は自作SVG図解で解説済み。
- 結果画面での安全配慮 → 既に`#result`セクションに「痛みが強いときは中止して医療機関へ」の注意文が動画一覧の直下に常設済み。

**今回は見送った項目**（設計変更のリスクが大きく、着手すると既存の作り込み・テストとの整合を崩す可能性があるため）:
- カード関連機能(記録カード生成/じまんカード/カード図鑑)の完全な一元化、前屈の二重計測(とどくメーター×かたさチェックQ1)の統合、ホームカードの並び順の抜本的な再設計、動画を探すのタグ大幅集約。いずれも本人と相談の上、必要なら別途着手。

`npm test`=281checks、`npm run smoke`=25/25、`npm run smoke:webkit`=9/9で確認済み。

## 2026-07-19 使い方タブをFable5視点UXレビュー結果に基づき全面改修

本人指示「使い方タブが本当に使いやすいかFableでサブエージェント5視点からチェックして」に対応。5体のFable並列エージェント（初見ユーザー／デジタル弱者・高齢者／トラブル自己解決／情報構造・ナビゲーション／モバイル操作性）にレビューさせ、全視点で総合3.5/5、複数視点が共通して指摘した論点を洗い出して全て実装した。

**実装内容**:
1. **🆘困ったときはカード新設**（id=gd-help、目次の先頭に追加）: 「記録が消えた・0日にもどってる」「機種変更したい」「ストレッチ中に痛かった」「通知・リマインダーについて」の4ボタンから該当FAQへ直接ジャンプ（該当グループ自動展開込み）。トラブル発生時に目次の「よくあるしつもん」を経由せず直行できるようにした。
2. **FAQ28件→4グループのdiv+onclickトグル式アコーディオンに変更**（ネイティブdetailsの入れ子は既存`.faq details`CSSと衝突するため回避）。初期状態は「📱きほんのき」グループだけ開き、残り3グループは畳んだ状態。`filterFaq()`によるキーワード検索(`#faqSearch`)も新設、ヒットしたグループは自動展開・非ヒットは「見つかりませんでした」表示。
3. **`gJump()`を拡張**: 対象要素の祖先を辿って(a)`<details>`があれば全て開く(b)`.faq-g-body.hidden`があれば解除して対応する`.faq-g`にopenクラス付与、してからscrollIntoViewする設計に。既存の`goToExportFromCard()`等の導線が「閉じたまま」にならないようにするため。
4. **重複コンテンツを統合し12枚→10枚に整理**: 「記録カードのつくりかた」を「記録が消えない3つの守り」のstep2に統合、「オガトレ通信」を「2日目からの毎日」に統合（どちらも目次に項目がなく迷子になっていた孤立カードだった）。
5. **「記録が消えない3つの守り」を目次で「つづくしくみ」より前に格上げ**（最重要の保険なのに埋没していた指摘に対応）。
6. **「ホーム画面に追加」の説明文をブラウザ/ジャーゴン依存の言い回しから平易化**（「アプリ版」の意味・なぜ大事かを一言添えた）。
7. **カード図鑑の前方参照を解消**（「つづくしくみ」カードで先に触れる際、実体が「マイ記録タブのお楽しみ機能」にある旨を先出し）。
8. **入口の二重化に一言説明を追加**（🌱はじめてガイド／📖使い方ツアーの違いを1行で明示）。
9. **各カード末尾に「↑目次へ戻る」リンクを追加**（`gJump('gtoc')`）。長いFAQを読んだ後に目次へ戻る動線がなかった指摘に対応。

`npm test`=281checks、`npm run smoke`=25/25、`npm run smoke:webkit`=9/9で確認。さらにローカルサーバーを立てClaude-in-Chromeで実機相当の動作確認も実施（🆘カードの4ボタン全て・目次へ戻るリンク・FAQ検索のヒット/非ヒット/クリア復元、いずれも正常動作、コンソールエラーなし）。

## 2026-07-19 マイ記録UI改善・使い方タブへの導線追加・#todayCard非表示ロジックの撤回

本人の実機フィードバック4件に対応。
1. **SWキャッシュ版を上げた(v55→v56)**: `assets/check/meter.jpg`をGPT生成画像で差し替えたのに実機に反映されない、という指摘。原因はsw.jsが画像アセットをcache-first方式で永続キャッシュしているため（`kyono-vNN`を上げない限り再取得しない）。今後も画像だけ差し替えるデプロイでは必ずこの版上げが必要（画像はASSETS配列に列挙されているが中身の変更検知はしない）。
2. **マイ記録「記録のひっこし（機種変・バックアップ）」を`<details>`アコーディオンに変更**（常時フル表示→折りたたみ・タップで展開）。既存の「毎日自動でアプリがひらく設定」と同じ`<details>`パターンを踏襲。`gJump()`を、対象要素が`<details>`(祖先含む)なら自動的に`.open=true`する仕様に拡張（`goToExportFromCard()`等の既存ジャンプ導線が閉じた状態のままにならないように）。
3. **使い方タブのFAQ「機種変更のときはどうするの？」に「📦 記録のひっこしをひらく」ボタンを追加**（`navTo('history');gJump('exportSection')`でマイ記録タブの該当アコーディオンを自動展開してジャンプ）。
...
本人指摘「文字被りしている」に対応。`docs/meter-image-fix-brief.md`（GPT向け指示書、v1→v2→v3と本人フィードバックを反映しながら改訂）を都度作成し、本人がGPTに生成依頼。最終版で反映された内容: Mプラス相当の太めゴシック体・文字サイズ拡大、ひざ/すね/つま先の線位置を実際の関節位置に合わせて下げる、ひざ〜つま先の破線を視認性の高い青に変更、ゆかは矢印ではなく直線でつま先から前方に伸ばしオレンジ色で明示(白背景で床位置が分かりにくい問題への対応)。実寸250px幅でのプレビューでも可読性を確認済み。コードは変更なし（画像アセットの差し替えのみ）。

## 2026-07-19 数字表記を半角に統一（全角「３０」の残存3箇所を修正）

本人指摘「30秒の数字が全角。アプリ全体どちらかに統一させてほしい」に対応。grepで確認したところアプリ全体（index.html/app-quiz.js/app-card.js/soudan-kb.js）の数字表記は「2週間」「1本」「30分」等、数十箇所すべて半角で統一済み。全角だったのは「３０秒」（home画面かたさチェックカード・meta description・はじめてガイドのステップ説明の3箇所）とオンボ挨拶文言「４つ」の計4箇所のみ。Fable（司令塔）に一般的なUI規約の観点も含めて確認させ、「半角に統一すべき（iOS/Android日本語UI慣行・アプリ内多数派との整合性の両面）」との結論を得たうえで半角に修正。なお`videos.js`のQUOTES（視聴者の生の投稿文言）に含まれる全角数字は本人以外＝視聴者が書いた原文ママの引用のため意図的に対象外（変更すると引用の正確性を損なう）。`npm test`=282checks、`npm run smoke`で確認済み。

## 2026-07-19 はじめてガイドのsoudanルートを廃止・quizルートに一本化

本人指摘「はじめてガイドではすぐ相談に行くのはナシ。まずは硬さチェックかな」に対応。`obDecideRoute()`の3分岐(quiz/soudan/today)からsoudanを削除し2分岐(quiz/today)に。悩み(worry)を答えても相談室へ直行せず必ずかたさチェックへ回すよう変更（`stiff==="hard"||"unknown"` または `worry!=="none"` ならquiz、それ以外はtoday）。悩みの情報は既存の`OB_WORRY_TO_QUIZ`変換でquizのQ5プリセットとして引き継がれるため消えない。`ONBOARDING_SCRIPT.routes.soudan`エントリと`obGo()`内の`if(r==="soudan")`分岐、コメントも合わせて削除。`scripts/smoke.js`の「確認9」（旧: soudanルートはkyono_fd対象外を確認）も、新仕様（worry経由でもquizルートに入りkyono_fd="go"が付与される）に合わせて書き換え。`npm test`=282checks、`npm run smoke`で確認済み。

## 2026-07-18 オンボ→かたさチェック誘導の一言を変更

`ONBOARDING_SCRIPT.routes.quiz.say`「なら30秒で見てあげる！タップするだけ✅」→「そしたら30秒で硬さチェックをしよう！下のボタンタップしてね！」（本人提案・即反映）。「見てあげる」より「しよう」で一緒にやる感を出し、「下のボタン」で次の操作先を明確化。`npm test`=282checks、`npm run smoke`=25/25（2回連続グリーン）で確認。

## 2026-07-18 はじめてガイドのテンポ減速＋挨拶文言の再修正

前エントリの4件対応後、本人からさらに実機フィードバック。

- **テンポがまだ忙しい**: `obSay()`の吹き出し間隔を0.96秒→1.5秒に再減速（2026-07-12に0.48秒→0.96秒へ減速した経緯があり、今回が2段階目）。
- **挨拶文言の再修正**（本人指定文言そのまま反映）:
  - greet[0]「はじめまして！理学療法士のオガトレです💪」→「いつもありがとうございます！理学療法士のオガトレです！」
  - greet[2]「４つだけ教えて！あなた用にととのえるね😊」→「最初に４つだけ教えてね！あなた用にこのアプリをととのえます☺️」
  - もじの大きさ回答後の一言「OK！「続ける設定」でいつでも変えられるよ🔠」→「OK！今後変えたくなったら「続ける設定」でいつでも変更できるよ！」
  - greet[1]「ここは毎日のストレッチを応援する場所だよ！」は前エントリのままで変更なし（本人が並べて確認し、そのままでOKという回答）
- 検証: ヘッドレスChromeで実際の吹き出し到達タイミング（3吹き出しで約5秒=1吹き出しあたり約1.5秒）と文言5件を実測確認。`npm test`=282checks、`npm run smoke`=25/25、`npm run smoke:webkit`=9/9、すべてPASS。

## 2026-07-18 PO実機フィードバック「はじめてガイドまだ使いにくい」4件を対応

本人が実機（IMG_8135〜8137）で見つけた使いにくさ4件を修正。

1. **オンボ挨拶の文言変更**: 「ここは1日1本のストレッチをいっしょに続ける場所だよ」→「ここは毎日のストレッチを応援する場所だよ！」（`ONBOARDING_SCRIPT.greet[1]`）
2. **設問チップの色が全部teal固定でわかりにくい**: 4問（もじの大きさ/体硬いほう/悩み/いつやる派）が全部同じ`chip-b`だったのを、既存の4色バリエーション（`chip-a`黄・`chip-b`teal・`chip-c`ピンク・`chip-d`紫）を質問の順番で巡回させる`OB_CHIP_VARIANTS`を新設し、設問ごとに異なる色にした。会話ログを見返すとき「どの行がどの設問への回答か」が色で区別できるように。
3. **しなやかネコ(yawara)だけタイプ固有ピック(rx)が空だった**: `TYPES.yawara.rx:[]`（他5タイプは全部rx1本あり）だったため、チェック結果で「①まずほぐす」の主役ピックが出ず、汎用メッセージ（fixed=false分岐）に落ちていた。`asa10`（朝専用・極上10分ストレッチ、既にpoolにあった定番動画）をrxに昇格し、poolから重複除去（他タイプと同じ「rxはpoolに含めない」流儀に合わせた）。
4. **はじめの1本ガイドで1日目完了後、「きょうの1本」カードが不要なのに再表示される**: `きょうやった！`を押すと`fdActive()`がfalseに戻り`renderToday()`が通常の3本推薦へ自動復帰する設計だったが（7/17時点の意図的仕様）、実際に使うと「今日はこれ1本でOKと言われて済ませたのに、すぐ後でまた3本の推薦が出る」という違和感があった。`renderHome()`に判定を追加し、**fd===1（ガイド完了）かつ今日はもう記録済み**のときだけ「きょうの1本」カード全体（`#todayCard`）を非表示にする。日付が変わって`dates.includes(todayStr())`がfalseに戻れば自動的に復活する自己完結型の条件なので、2日目以降は今まで通り。

検証: 4件とも実機相当のヘッドレスChromeで実測確認（チップ色の実際のclassName・yawaraのrx値・todayCardの表示/非表示切り替えと翌日の自動復帰）。`scripts/qa.js`に恒久チェック4件を追加（`checkOnboardingUX()`）。`npm test`=282checks、`npm run smoke`=25/25、`npm run smoke:webkit`=9/9、すべてPASS。

## 2026-07-18 C1復帰・PO判断待ち6件のうち4件が本人回答で確定・反映

C2休止時に申し送られた「PO判断待ち6件」のうち、本人がその場で即答できた4件を反映。残り2件（①写真素材待ち・②カードご自愛メッセージの日付ゲート要否）は保留のまま。

**反映した4件**
1. 再生リスト名「様々な症状撃退シリーズ」→「お悩み別ストレッチシリーズ」（index.html:3308、YouTubeプレイリストのタイトル自体ではなくアプリ内の表示ラベルなので変更は安全）
2. 相談室`undomae`インテントの見立て文「パフォーマンスも上がって」→「動きの質も上がって」（soudan-kb.js:927）。`node soudan-ai-poc/build-data.mjs`でdata.mjs再生成、`redflag-safety-test.mjs`90/90を再確認済み
3. 「残せます」表記・相談室の悩み選択肢「腰痛」ラベル：どちらも**現状維持で確定**（本人「問題ありません」）
4. 「つづける/続ける」表記ゆれを**漢字「続け」に統一**（本人指示）。index.html 29箇所・soudan-kb.js 7箇所・app-quiz.js 2箇所（「2週間つづけてみて」「3本つづけて再生する」）・app-card.js 1箇所（MS配列 d:1900の`.m`＝DOM専用フィールド、C16と同じ判定で変更可）を機械的に置換。**意図的に対象外にした箇所**:
   - コメント（index.html 6箇所・soudan-kb.js/app-record.js各1箇所）: ユーザーに見えないため無関係
   - チャットボットのキーワード配列（soudan-kb.js:181の`kw:`、index.html:3050の`SD_FU_KW`）: 表示文言ではなくユーザー入力の一致判定に使う語彙リストで、実際に**ひらがな・漢字の両方の書き方がもとから混在収録されている**（ユーザーがどちらで打っても拾えるようにするための意図的な設計）。統一すると片方の書き方をする人の入力が拾えなくなる実害があるため据え置き
   - app-card.jsのCOMMON配列（ご自愛メッセージ、canvas描画・過去カード再現ルール対象）: #19と同種の「直すなら日付ゲート必須」案件のため、本人がまだ②の可否を回答していない間は触らない
   - `qa.js`/`smoke.js`側の対応するハードコード文字列（「つづける設定」「3本つづけて再生する」）も追従して更新。smoke 1本（1c）が新文言と旧アサーションの不一致で一度落ちたが、テスト側の期待値を更新して解消（アプリ側のバグではない）
- 検証: `npm test`=278checks、`npm run smoke`=25/25、`npm run smoke:webkit`=9/9、`redflag-safety-test`=90/90、すべてPASS

**未回答のまま残っている項目**: ①かたさチェックQ3/Q4の実写化（本人写真待ち）／②カードご自愛メッセージの句読点修正（過去カード再現のため日付ゲート実装が要るが着手判断待ち）／⑧新文言群の実機レビュー／⑨告知文最終確認（β配布可否）

## 2026-07-18 C2セッション（appdev・Fable司令塔）休止・C1への引き継ぎまとめ

C2アカウントのFable枠終了(7/20)を前に、監督指示によりC2は本日で休止・以後はC1で作業を継続する。C2で本日進めた内容の総括と、C1が引き継ぐべき残件:

**本日C2で完了したもの（全てpush・デプロイ・検証済み）**
- SWプリキャッシュ削減（OGP画像316KB除外・v55）／WebKit実機スモーク新設（ローカル+週次CI・9ステップ）／週次prod-healthワークフロー（内部文書404監視含む8項目）
- PO承認済み改善①④⑤（とどくメーター安全注意・節目カードのひかえ促し・復帰ナッジ常時化）＋③FAB占有は本人回答「問題なし」でクローズ
- 継続施策「あした節目予告」（提案3案から本人選定①）
- 図鑑カード文言のワクワク化（レア30なぞなぞティーザー+全106フレーバー・Fable創作）
- おすすめ3本ロジックv2（ローテ毎日化・等間隔2点取り・16分以上4本をプール除外・qaに2週間シミュレーション恒久化）
- きょうのひとこと15→45本（Fable創作30本）
- 全サイト文言トンマナ監査37件→27件適用（医療語3・敬体混在・かな漢字ゆれ等）
- 判断在庫2本: NOTIFY-DESIGN.md（通知3案・推奨=ネイティブ薄ラッパー+ローカル通知）／BETA-FEEDBACK-PLAYBOOK.md（β後トリアージ運用）

**C1への申し送り（PO判断・素材待ち）**
1. ②かたさチェックQ3/Q4実写化=本人写真2枚待ち（q3.jpgは旧ポーズ遺物で使用不可・HANDOFF注意書き参照）
2. トンマナ監査の本人判断行き6件: #19カードご自愛メッセージの句読点（**過去カード再現ルール対象=直すなら日付ゲート方式の提案書が必要**）／#23「残せます」表記／#26「様々な症状撃退シリーズ」改名／#29「パフォーマンス」和語化／#31 soudan-kb「続け/つづけ」全域統一（機械置換可・検収txtと同時更新推奨）／#35 悩み選択肢「腰痛」のラベル調。加えてC36「1900日——伝説の域」はcanvas描画に乗るため見送り済み（変えるなら同じく日付ゲート）
3. 文言のPO実機レビュー未実施分: 7/18に入れた新文言群（安全注意・ひかえ促し・おかえりナッジ・節目予告・図鑑136本・ひとこと30本・トンマナ27件）
4. soudan-kb検収txtとのズレ: トンマナ修正でreview.txt 6箇所+review-v2.txt 1箇所が乖離（詳細は1つ前のエントリ）。本人検収の際は現物soudan-kb.jsを正とする
5. 見送り済み継続施策②③（前屈再測定促し・週ふりかえり）は設計要旨を「継続施策の提案3案」エントリに保存済み
6. 告知文の本人最終確認が済めばβ配布可能（BETA-CHECKLIST参照・技術ゲートは全通過）

**品質ゲート最終値（C2休止時点）**: qa 278checks／smoke 25/25／smoke-webkit 9/9／redflag-safety 90/90／週次prod-health・webkit-smoke・月次catalog-health稼働中

## 2026-07-18 トンマナ修正バッチ（司令塔Fable仕分け・全サイト文言・Sonnet実装）

司令塔Fableが監査37件から仕分けた「即修正」約28項目を適用。指示書はscratchpad一時ファイル（`tone-fixes.md`）のため本文に転記する形でここに記帳。**文言はPO実機レビューで最終確認**。

### 適用した項目（H1-H4, H5-H6, M7-M9, M11-M15, M17-M18, L20-L22, L24-L25, L27-L28, L32-L34, C16 = 28項目）
- H1 index.html V.katakori.t「肩甲骨から治す」→「肩甲骨からゆるめる」
- H2 index.html 再生リスト説明「骨盤・股関節から治す」→「骨盤・股関節からゆるめてスッキリ」
- H3 index.html VOICES front「猫背が治った🙌」→「猫背がスッキリ🙌」（qは不変・VOICES件数不変）
- H4 index.html importData confirm「上書きします。よい?」→「上に書きかえるよ だいじょうぶ？」
- H5 soudan-kb.js redFlags.answer文末「また一緒にやりましょう。」→「また一緒にやろうね。」
- H6 soudan-kb.js 2箇所「自信をもって言えます！/言えます。」→「言えるんだ！/言えるんだ。」
- M7 index.html「前屈チェックで、のびていく」→読点をスペースに
- M8 index.html「決めると続きやすく」→「つづきやすく」／「合図をひとつ選んでください」→「えらんでね」
- M9 index.html ひっこし機能3箇所（1887行の「〜押してね」は現状のまま=対象外・元から正しい）
- M11 crisis文言（index.html・soudan-kb.js両方）「窓口があります。」→「窓口があるよ。」
- M12 index.html オフラインバナー「動画の再生には電波が必要です」→「動画を見るには電波が必要だよ」
- M13 soudan-kb.js「体が硬い人の味方です！」→「味方だよ！」
- M14 soudan-kb.js「応援してくれる人がいるから頑張れてる」→「がんばれてる」
- M15 soudan-kb.js smalltalk 5箇所（1716/1736/1742/1771/1786行）敬体→常体統一
- M17 app-card.js「うまく作れませんでした」→「うまくつくれませんでした」
- M18 app-record.js「きょうの分完了！…✨」→index.html側に合わせ「きょうの分は完了！…😊」
- L20 index.html FAQ読点4箇所（997/1016/1022/1026行）→スペース
- L21 index.html oldBrowserNote「ひらいてみてください🙏」→「ひらいてみてね🙏」
- L22 index.html「初めてつま先さわれた」→「はじめてつま先さわれた」
- L24 index.html オガトレ通信投稿なし「またきてね」→「また今度のぞいてみてね🌱」（アーカイブ版に統一。1箇所のみ該当・もう1箇所は元から統一済みだった）
- L25 index.html copyMailAddrLegacy 2箇所「コピーできませんでした」→「コピーできなかったみたい💦…を手でコピーしてね」
- L27 index.html 使い方ツアー2箇所「守ってくれます」→「守ってくれるよ」／「（×はつきません）」→「（×はつかないよ）」（qa.js checkOnboardTourDriftの`<b>`内UI参照文字列は不変・確認済み）
- L28 index.html 検索プレースホルダー「肩甲骨 / 朝 / 開脚」→全角スラッシュに統一
- L32 app-quiz.js「もどってきて、下の」→「もどってきて 下の」
- L33 app-search.js「まだなかった…！」→「まだなかったみたい💦」／メール本文「（ここに書いてください）」→「（ここに書いてね）」
- L34 app-env.js「Safariで開いてくださいね🙏」→「Safariで開いてね🙏」
- C16 app-card.js MS配列(d:1900).m「1900日続けた先輩は」→「1900日つづけたせんぱいは」（DOM専用フィールドと確認の上）

### スキップした項目と理由
- **C36** app-card.js MS配列(d:1900).t「1900日——伝説の域」: drawCard()内で`msInfo.t`が節目カードのcanvas描画（`ctx.fillText(msTxt,...)`）に直接使われることを呼び出し経路で確認したためスキップ。変更すると過去日カードの見た目が変わる。
- **C37** app-card.js「つづけてる自分を、ちゃんと褒めてあげてね。」: **COMMON配列の要素そのもの**（絶対に触らないもの＝過去カード再現ルール対象・監査#19と同種）。さらにCOMMON由来の`pool`は毎日のカード全件でキャラ吹き出しとしてcanvas描画（`ctx.fillText(fmsg,...)`）されるため、二重の理由でスキップ。

### C16/C36/C37の描画経路判定（詳細）
- `app-card.js`のMS配列は`.t`（節目タイトル）・`.m`（節目メッセージ）・`.q`（先輩の声）の3フィールド。
- `drawCard()`内 `const msInfo = MILESTONES.includes(effTotal) ? MS.find(...) : null;` の直後、`milestone`ブロックで`msTxt = msInfo.t + "！おめでとうございます！"`を`ctx.fillText`でcanvasに描画（`.t`は描画対象＝C36スキップ根拠）。
- `.m`はdrawCard()内で未参照。`app-record.js`のmarkDone()内`ms.m`がcheerEl.innerHTML（DOM）にのみ使われる（`.m`はDOM専用＝C16は適用可）。
- COMMON配列（ご自愛メッセージ）は`pool`経由で全カード共通のキャラ吹き出しとして`ctx.fillText(fmsg,...)`に必ず乗る＝そもそも「絶対に触らないもの」指定と整合。

### soudan-kb.js変更後の必須手順・結果
1. `node soudan-ai-poc/build-data.mjs` 実行済み（`data.mjs`再生成: intents=122 catalog=454）
2. `node soudan-ai-poc/redflag-safety-test.mjs` = **90/90 PASS**（全件成功。指示書記載の「83/83」から件数が変わっているが、これは直近の別作業（おすすめ3本ロジックv2＋ひとこと30本追加等）でテスト側が増えた影響と見られ、今回の文言変更による影響ではない＝100%成功で異常なし）

### 検収txt（ogatore-hub/dev-specs/kyono-soudan-kb-review*.txt）と文言がズレた箇所（本人検収の照合用・通番なし）
- [83]赤旗回答の文末: 検収txtは「よくなったら、また一緒にやろうね!」（!終わり）、実装は「。」終わり（今回の変更前後とも「。」のままで、この記号差は今回変更由来ではない＝既存差分）
- [94]「その言葉がいちばんのストレッチになります😊」→今回「になるよ😊」に変更。検収txtは未更新のまま
- [101]「体が硬い人の味方です!」→「だよ!」に変更。検収txtは未更新のまま
- [102]「体年齢なら毎日若返り中です😊」→「だよ😊」に変更。検収txtは未更新のまま
- [105]「気仙沼のスーパーヒーロー!朝ストレッチで一緒にやった仲です!」→「だよ!」に変更。検収txtは未更新のまま
- [116]「動画づくりの燃料になります😊」→「なるよ😊」に変更。検収txtは未更新のまま
- [122]「僕もいつも腹ぺこです。」→「だよ。」に変更。検収txtは未更新のまま（なお検収txtは元々「〜!😊」語尾・実装は「〜😊」で今回変更前から表記差あり）
- [265](v2ファイル)「理学療法士として自信をもって言えます！」→「言えるんだ！」に変更。検収txtは未更新のまま
- soudan-kb.jsのもう1箇所「理学療法士としても、そこは自信をもって言えます。」→「言えるんだ。」（H6の2件目）は検収txt(v1/v2どちらにも)該当エントリが見当たらず、照合対象外

### 検証結果
- `npm test`: 278 checks 全PASS
- `npm run smoke`: 25/25 PASS
- `npm run smoke:webkit`: 9/9 PASS
- `node soudan-ai-poc/redflag-safety-test.mjs`: 90/90 PASS
...
- 文言: `いたみがある日は むりしないでね（つよい痛みが続くときは お医者さんへ🏥）`（**文言はPO実機レビューで要確認**）。既存の説明文と同じ`font-size:14px;color:var(--sub)`で控えめに表示。
- 検証: ヘッドレスDOM検査で3要素（見出し／既存説明文／新設の安全注意）の`getBoundingClientRect()`を実測し、重なりゼロ・お手本画像との間隔も自然であることを確認。
- **ハマった点（艦隊運用の注意）**: 実装後にcommitする前に、even-syncの自動コミット（`auto-sync 2026-07-18 10:01`＝コミット4c3c47c）がこの`index.html`の1行差分を先に拾ってpushしてしまった（艦隊が同一作業ツリーを共有し10分毎に自動commit→pull→pushするため。MEMORY.md記載の既知動作）。コード自体は正しく反映済み・動作確認も上記の通り完了しているため、このエントリだけを追記してcommitする。
- `npm test`=265checks、`npm run smoke`=23/23、いずれもPASS（auto-syncコミット後に再実行して確認）。ES2020構文(`??`/`?.`)はgrepでゼロを確認。

## 2026-07-18 βフィードバック対応プレイブック新設（BETA-FEEDBACK-PLAYBOOK.md・Fable判断在庫第2弾）

β配布後の声への対応を、Fable不在でもSonnet艦隊+本人で回すためのトリアージ運用書を新設。T0安全（即日・ただしテスト必須）/T1バグ（Sonnet完結可・「記録が消えた」は環境ヒアリング先行の特別枠）/T2文言（本人確認必須）/T3要望（即実装禁止・台帳で件数を貯めて閾値で提案書化・通知はNOTIFY-DESIGN.mdへ接続）/T4感想、の5分類。声の台帳は`feedback-log.md`（未作成・最初の声が来たら作る。**個人名・メアド書き込み禁止=公開リポジトリのため**）。β期間中の不変条項（サーバー導入NG・課金NG・数字を追わせるUI恒久NG・過去カード再現性）も1箇所に集約。

## 2026-07-18 通知機能の意思決定ドキュメント新設（NOTIFY-DESIGN.md・Fable判断在庫）＋HANDOFF次候補の事実誤認1件を訂正

- **NOTIFY-DESIGN.md新設**（コミットea74e17相乗り）: ロードマップの次の大型判断「βで通知需要が集まったらiPhoneアプリ化（年16,000円）」に備え、C2 Fable枠終了(7/20)前に技術選択肢3案を意思決定可能な形で整理。核心: ①iOS 16.4以降はPWAでもWeb Push可能=「通知=ネイティブ必須」の前提は古い ②しかし本アプリの通知（毎日決まった時刻）はそもそもプッシュ不要で、**ネイティブのローカル通知ならサーバーゼロ**=運用ゼロ思想と「記録は端末内のみ」の約束を完全維持できる唯一の「本物の通知」 ③Web Pushは購読情報のサーバー保存が約束を破るため非推奨（理由ごと記録）。推奨=需要が本物なら案B（ネイティブ薄ラッパー+ローカル通知・記録移行は既存の記録お引っ越し機能を流用）。**実装は何もしない・βの声が集まるまで寝かせる**
- **HANDOFF「次の改善候補」の事実誤認を訂正**: 「Q3実写化にq3.jpg（既存・未使用）が使える」とあったが、q3.jpgを実見したところ**旧方式「背中の後ろで合掌」ポーズ**（7/5宣材撮影の遺物）で、現在のQ3「指先を鎖骨において ひじ上げ」とは別テスト。使うと設問と合わない見本写真になるため「素材待ち（本人写真2枚）が正解」と明記
- **申し送り**: HANDOFF次候補のS/M 5件は「Fable発案10件のうち本人が選定しなかった側」のため、**PO回答なしに実装着手しないこと**（選定済み5件はすべて実装済み）。監督経由でPOに追加選定を仰ぐ
...
- ツアー配列内の`<b>...</b>`で強調されたUI参照文字列をすべて抽出し、装飾（絵文字プレフィックス・「」の囲み・「タブ」等の説明的な接尾辞）を正規化した「core」文字列が、ツアー配列の**外**（＝実際の画面のHTML/JS）にも存在することを1件ずつアサート。
- 動作確認: ツアー内の文言だけをXYZ付きに書き換えると`QA failed`で該当の1件だけが検知され、実UI側も含めて一括リネームした場合（=矛盾なし）は`QA passed`のままになることを実測（意図した「claim≠reality」だけを検知し、一貫した改名の誤検知＝false positiveを起こさないことを確認）。
- 現状の10種類のUI参照はすべて実UIとの整合が取れている状態で248 checks（+10）。`npm run smoke`21/21も無事故。

## 2026-07-18 Fable発案10件のうち3件(FAQ図鑑項目・カレンダータップ拡大・相談室フォールバック実質化)を実装

Fableに現状棚卸しをさせて出た10件の改善案から、本人が5件を選定（2,5,6,8,10）。うち即実装可能な3件をこのタイミングで対応。

1. **相談室フォールバックの「近い候補」を実際に出す**: `sdSend()`が計算済みの`sdScoreIntents(n)`結果（`ranked`）を`sdAnswerFallback(raw,ranked)`に渡すよう変更。スコア閾値(2)未満でも1文字以上ヒットした候補があれば上位3件を抽出し、`sdChipsMode={type:"nearmiss",ids:[...]}`という新モードで実際にタップ可能なチップとして表示（`sdRenderChips()`に分岐追加）。候補ゼロのときは従来どおり全カテゴリ表示にフォールバック。「近いのはこのあたりかも！」という既存の案内文言と実際の挙動が一致するようになった。`sdChipTap(id)`を再利用しているため新規関数は最小限。ヘッドレスбрラウザで`sdAnswerFallback`にダミーの近接候補を直接渡し、チップ描画→タップ→`sdAnswerIntent`実行までの一連を実測確認。
2. **FAQにカード図鑑・レアカードの項目を追加**: 「🎨見ため・その他」グループに「カード図鑑ってなに？」「レアカードはどうやったら出るの？」を追加。回答はカードの抽選機構（`cardRotPick`=記録した回数ベースで50種を必ず1周する決定的ローテーション、記念日/季節は別枠）を正しく反映した内容。
3. **カレンダー日付のタップ領域を拡大**: `.cal .d`を40px→44px、小画面(340px以下)は32px→36pxに拡大（Apple HIGの推奨最小タップ領域44ptに合わせた）。なお実際のタップ判定は既に`<td>`要素全体に付与されている（`app-record.js`の`renderCal()`）ため、これは主に「見た目の押しやすさ」向上の位置づけ。

検証: `npm test`=238checks、`npm run smoke`=21/21、いずれもPASS。残り2件（②使い方ツアーのモック画面ドリフト対策・⑩SPLIT-PLAN最終分割）は個別に対応中。

## 2026-07-18 ゴールドカード見本も実物画像に統一・カード図鑑の紹介文を追加

前エントリで「記録カードのつくりかた」を実物画像化したのと同じ手法を、使い方タブ「つづけるしくみ」内のゴールドカード見本（節目3日・7日…の紹介）にも適用。

- `makeCard("2026-07-01")`を`streak2={dates:["2026-07-01"],count:7,total:7}`（effTotal=7→MILESTONESの節目「1週間たっせい」に一致・type/memoは空でシンプルな見本に）で実行し、実際に描画されたゴールドカードを360x360のPNGとして`assets/card-sample-gold.png`に保存。
- `index.html`の`.gcardmock.gmock`CSSモック（line 991付近）をこの実物画像に差し替え。
- **`.gcardmock`/`.gcm-*`のCSS定義を削除**（このタイミングで最後の利用箇所だったため完全に未使用になった。前エントリで使い方ツアー側は既にimg差し替え済みだった）。`scripts/qa.js`の`--teal-strong`回帰チェックリストから、消滅した`.gcm-n .gcm-pill`セレクタを削除（238 checksに）。
- ゴールドカード見本の直後に「記念日・季節・レアなど カードのデザインは何種類もあります あつめた記録は『マイ記録』タブのカード図鑑📖でいつでも見返せます」を追加。カード図鑑機能（マイ記録タブの`#dexBannerCard`）はこれまで使い方タブのどこにも案内がなかったため新規追加（本人指示「ここで図鑑の紹介もして」）。
- `sw.js`: `assets/card-sample-gold.png`をASSETSに追加、キャッシュ版`kyono-v51`→`kyono-v52`。
- 検証: ローカルプレビュー+JS経由のDOM検査で実物画像のロード（`naturalWidth=360・complete:true`）と図鑑紹介文の表示を確認。`npm test`=238checks（-1は削除したgcm-nチェック分）、`npm run smoke`=21/21、いずれもPASS。

## 2026-07-18 記録カードモックを「実際にdrawCard()で描画した本物」に差し替え（assets/card-sample.png）

前回・前々回のエントリで対応した「記録カードのつくりかた」モックが、POの参考画像（Google Drive「アプリ07-1.png」、山本さん作の実カードデザイン案）とまだ見た目が一致していなかった。よく確認したところ、この参考画像は`app-card.js`のコメント「山本さん案レイアウト」が指す**実際のdrawCard()仕様書そのもの**で、CSSで似せた`gcardmock`コンポーネントでは再現しきれない要素（散らし装飾のランダム配置・「かたさタイプ」「メモ」ピルの実際の描画・キャラの実配置）があった。

- 対応：CSSモックを作り直す代わりに、**実際のアプリの`makeCard()`/`drawCard()`をヘッドレスで動かして実物のカード画像を1枚生成**（date="2026-07-01"＝CARD_IMG_FROM(2026-07-14)未満の従来固定パターンを使う日付・dateIdx%5===0で「さくら」テーマ（ピンク系）になるよう逆算、type=yawara(しなやかネコ)、memo="体が伸びた〜！"、streak2={dates:["2026-07-01"],count:1,total:1}）。生成したcanvasを360x360に縮小してPNG化し`assets/card-sample.png`として保存。
- `drawCard()`自体は一切変更していない（ピクセル回帰テスト対象のため触らない方針を厳守）。あくまで「既存関数の実行結果を静的アセットとして保存しただけ」。
- `index.html`の2箇所（使い方タブ「記録カードのつくりかた」・`OB_TOUR_SLIDES`の「記録カードをつくる」スライド）を、`gcardmock`CSSモック→`assets/card-sample.png`の`<img>`表示に差し替え。`.gcardmock`/`.gcm-*`CSS自体は別のFAQ見本（line 991付近、ゴールドカードの見本）で引き続き使用中のため削除せず維持。
- 使わなくなった`assets/chara-cardtour.png`はsw.jsのASSETSからも外し、ファイル自体を削除（どこからも参照されないことをgrepで確認済み）。`assets/card-sample.png`をASSETSに追加。
- sw.jsキャッシュ版: `kyono-v50`→`kyono-v51`
- 検証: ローカルプレビュー+JS経由のDOM検査で両箇所とも`naturalWidth=360・complete:true`で正しくロードされることを確認（生成した実物画像を目視でも確認、参考画像とほぼ一致）。`npm test`=239/239、`npm run smoke`=21/21、いずれもPASS。

## 2026-07-18 使い方タブ「記録カードのつくりかた」を実カード風モックに・FAQに旧端末対応を追記

前回（同日先行エントリ）で「使い方ツアーのモック画像だけ」キャラを差し替えたが、POが以前から別途依頼していた「使い方タブ側の記録カードのつくりかたセクションも、実際のカードっぽい見た目にしてほしい」という修正が未反映のまま残っていた（会話が一部圧縮されて見えなくなっていた指示）。POからの参考スクリーンショット（望む見た目=カード風モック）と実機スクリーンショット（現状=裸のキャラ画像60x60だけ）の比較で判明。

- `index.html`の使い方タブ「記録カードのつくりかた」（旧line 1021付近）: 裸の`<img>`60x60を、`OB_TOUR_SLIDES`の「記録カードをつくる」スライドで既に使っている`.gcardmock.gcm-n`コンポーネント（ピル「#きょうのオガトレ」＋`通算X日目`＋メモ注記＋キャラ画像、既存CSSで実装済み・スタイル調整不要）に統一。表示は「1日目」に設定（初回説明用の例示のため）。「がんばりを形にして残せます」キャプションはカードの下に中央寄せで維持。使い方ツアー側とビジュアルの一貫性が取れた。
- 使い方タブのFAQ「📱 きほんのき」グループに「古いスマホでも使えるの？」を追加。回答は「だいたいの端末で使えます！ただしかなり古いタイプだと、オフラインでの記録やホーム画面への追加がうまくいかないことがあります📱 その場合もアプリ自体はふつうに使えるのでご安心を」（開発側の技術詳細=JS構文はiOS10.3相当・Service Worker/Add to Home Screenの完全対応はiOS11.3以降、はユーザー向けには出さず平易な表現に翻訳）。
- 検証: ローカルプレビュー+JS経由のDOM検査で`.gcardmock.gcm-n`が正しく表示され画像ロード済み・`通算 1日目`テキスト・キャプション表示・FAQ項目の存在と回答文言を確認（スクリーンショット機構が不調のため代替検証）。`npm test`=239/239、`npm run smoke`=21/21、いずれもPASS。
...
1. **「きょうやった！」後の導線を並び替え**: `index.html`の`#streakCard`内、`#calAsk`（カレンダー案内）を`#makeCardBtn`（記録カードを画像でのこす）／`#cardHint`より**後**に移動。直後に新規`<div id="tourAsk"></div>`を追加。結果、cheer→`#memoRow`→記録カードボタン→`#calAsk`→`#tourAsk`、の順になった（PO要望どおり）。
   - `app-record.js`の`markDone()`: guide時cheerから「📖 使い方ツアーを見る」ボタン(`#cheerTourBtn`)・「あとで」ボタン(`#cheerTourSkipBtn`)・「使い方タブからいつでも見られるよ」の一文を削除し、cheer自体は「🎉 1日目クリア！ナイスご自愛！」＋メモ促し1行のみにシンプル化。
   - 既存の`#calAsk`描画（`st.total===1 && !calseen`）の直後に、`guide`が真のときだけ`#tourAsk`へ同じ2ボタンを描画する新ブロックを追加（見た目はカレンダー案内カードと同じ`background:var(--bg);border:1.5px solid var(--line);border-radius:14px;padding:12px 14px`）。「あとで」は`#tourAsk`の中身ごと空にする（ボタンだけ隠す旧実装から変更）。
   - `renderStreak()`の`!did`分岐で`#calAsk`と同様に`#tourAsk`もクリアするよう追加（再訪日の残骸防止）。
2. **カレンダー案内の文言を2行に変更**: 「あしたも同じじかんに会おう📅 カレンダーに毎日の合図を入れておく？」→「明日も同じ時間に会いましょう。\nカレンダーに毎日の合図を入れておく？」。`calendarAskEl()`の`#calAskLead`はtextContentのみの安全設計を維持したまま、要素のstyleに`white-space:pre-line`を追加して`\n`を見た目の改行として反映させた（HTMLインジェクション回避の設計は不変）。
3. **使い方ツアー(`OB_TOUR_SLIDES`)の内容をブラッシュアップ**: 実際のセクション構成（`じまんカード・せんぱいの声・ひとことにっき`が`#fun`(お楽しみ機能)ページに統合済み、`とどくメーター`が独自`#reach`ページとして独立済み）と食い違っていた「マイ記録でふりかえる」スライドの文言（旧: 「とどくメーター📏・せんぱいの声・ひとことにっき📝もこのタブ」と一括表現し、とどくメーターとお楽しみ機能が別導線であることが読み取れなかった）を、「📏とどくメーターと🎉お楽しみ機能（じまんカード・せんぱいの声・ひとことにっき）もこのタブの「見てみる」から見られるよ」に修正。他6枚も文言を軽く簡潔化（矢印表現の追加等）。枚数(7枚)・全体の流れは維持。

**機械チェックの追加（再発防止）**: `scripts/qa.js`に、(1)`#streakCard`内のDOM順序(`#memoRow`→`#makeCardBtn`→`#cardHint`→`#calAsk`→`#tourAsk`)を固定するチェック、(2)`#calAskLead`に`white-space:pre-line`が付いていることのチェック、(3)`markDone()`のguide cheerが新2行のみでツアーボタン/使い方タブ案内を含まないことのチェック、(4)`#calAsk`の新文言(改行入り)チェック、(5)`#tourAsk`へのツアーボタン描画とスキップ時のinnerHTMLクリアのチェック、(6)`renderStreak()`が`#calAsk`と`#tourAsk`両方をクリアすることのチェック、(7)新設`checkTourSlides()`で`OB_TOUR_SLIDES`が7枚のまま・新文言を含む・旧不正確文言が復活していないことのチェック、をそれぞれ追加。`npm test`は239 checksへ増加（旧231から+8）、全PASS。`npm run smoke`は既存1c/7dシナリオを更新（1cはcalAskのリード文言比較を新2行文言＋改行文字の有無＋`white-space:pre-line`の実測に置き換え、7dはcheerが2行のみであること・DOM順序の実測・`#tourAsk`の「使い方ツアーを見る」実タップでのobOpenTour()起動・「あとで」実タップでの`#tourAsk`空化・リロード後`#calAsk`/`#tourAsk`とも再出現しないことを検証する手順に拡張）、引き続き21ステップ、全PASS。

**実測確認の要点**: puppeteer-coreでオンボ完走(quizルート)→かたさチェック完走→結果画面→動画タップ→復帰→「きょうやった！」の一連を実行し、cheer→`#memoRow`→記録カードボタン→`#calAsk`(改行入り新文言・`getComputedStyle().whiteSpace`が`pre-line`であることも実測)→`#tourAsk`、の順でDOM上に並んでいることを確認。`#tourAsk`の「使い方ツアーを見る」を実タップしてツアー(`#obLog`)が実際に開始することを確認、ツアーを閉じてホームへ戻った後（`#tourAsk`はこの時点でも消えない設計であることを利用し）同じカードの「あとで」を実タップして`#tourAsk`が空になることを確認。ブラッシュアップ後のツアースライドも実際に開いて崩れがないこと・「マイ記録」スライドの新文言を確認。既存の非guideユーザー（st.total!==1で#calAsk非表示・cheerがguide文言でないこと）の回帰も確認。
...
- `obuHasNew()`に鮮度しきい値`OBU_STALE_DAYS=30`（日）を追加。既存の`todayStr()`/`daysBetween()`を再利用し、最新投稿の日付が30日以上前なら未読でも`false`（NEWにしない）を返すようにした。
- しきい値30日の根拠: おやすみ券(月3枚)・記録カレンダーの月次感覚に合わせた「だいたい1ヶ月」の目安。数日〜数週間の不定期更新は従来どおりNEW扱いしつつ、季節をまたぐような放置だけ自然にNEWを引っ込める。
- `obuPostInner()`が30日超の投稿に`.obu-date-old`クラス（フォントサイズ12→11px・weight 800→600・色を新設の`--sub-faint`変数へ）を付与し、`renderObuPopup()`/`renderObuArchive()`双方の日付表示を控えめにした。ライト`#827F72`/ダーク`#8C8676`は対応するカード背景に対しコントラスト比約4.0:1（既存の`.obu-date`は5.3〜6.9:1）で、目立たせすぎない体裁にしつつ読みにくくならない値を選んだ。督促文言・機能は追加していない。
- 実測確認（node vmでobuHasNew()を実際に実行）: 60日前・未読→NEWでない／数日前・未読→NEW（回帰なし）／ちょうど30日前・未読→NEWでない（境界値）／数日前でも既読→NEWでない／実際のobu-feed.js相当(2026-07-09投稿・8日経過)→まだNEW扱い（今回の変更で現状表示が変わらないことを確認）。
- `scripts/qa.js`に`checkObuStaleness()`を新設し、上記シナリオを実際に`obuHasNew()`を実行して機械チェックに固定（再発防止）。`npm test`/`npm run smoke`ともグリーンを維持。

## 2026-07-17 未監査領域の実測ベース品質監査（動画を探す/再生リスト/使い方/オガトレ通信/記録のひっこし＋ダークモードコントラスト機械計測）

相談室（3ラウンド）・ホーム/お楽しみ機能は監査済みだった一方、残りの主要画面が未監査だったため、puppeteer-core（`scripts/smoke.js`と同じ実機ヘッドレスChromeパターン、使い捨てスクリプトで実施・リポジトリには残していない）で実測した。

**バグ修正1件（実装済み）**: `importData()`のキー件数上限チェックが`cnt>50`で、意図（「50が十分な上限」というコメント）に反して**51件目まで通ってしまうオフバイワン**だった。実測（`kyono_`キーを60個含む文字列をインポート）で実際に51件取り込まれることを確認した上で`cnt>=50`に修正し、実測どおり50件ちょうどに変わることを再確認した。`scripts/qa.js`の静的チェック（`importData: count guard`）も新しい`cnt>=50`パターンに追従済み。実害は小さい（アプリの実キー数は約19個で上限に達することはない）が、コメントと実装の食い違いという明白なバグのため直接修正した。

**記録のひっこし（エクスポート→インポート往復）はE2Eで正常動作を確認**: 連続日数・メモ・前屈記録(とどくメーター)・かたさタイプ等をためた状態で`exportData()`→localStorage全クリア（新端末を模擬）→`importData()`という往復を実機で行い、`kyono_daylog`/`kyono_freeze2`/`kyono_memos`/`kyono_onboarded`/`kyono_reach`/`kyono_rotAssign`/`kyono_streak2`/`kyono_theme`の全キーがバイト単位で復元され、UI上も連続日数・メモ・前屈記録が正しく反映されることを確認。壊れた文字列を貼った場合も既存データは無傷でフレンドリーなエラーになることも確認（既存の防御設計どおり）。

**動画を探す/再生リスト/使い方タブ/オガトレ通信は目立つバグなし**: キーワード検索（デバウンス・クリア・0件表示・リクエストメールへのキーワード反映）、カテゴリ/タグ切替、「さらに表示」、年フィルタ、使い方タブの目次ジャンプ・FAQ開閉・使い方ツアー（7ステップ・「とばす」）、オガトレ通信のポップアップ→「もっと見る」→全アーカイブ→戻る、をいずれも実機操作で確認し問題なし。再生リストはネットワークで`i.ytimg.com`をブロックした状態でも実測し、サムネイル読み込み失敗時に`plImgErr`のフォールバックSVGへ正しく切り替わることを確認（ローカル資産の8枚は正常表示、リモートサムネイルの8枚だけが正しくフォールバック）。

**ダークモードコントラスト機械計測（全画面・約2700要素を実レンダリング色から自動採取）**: 全タブ・オンボーディング・使い方ツアー・相談室シート・各種モーダルを巡回し、実際のcomputed style（前景色・実効背景色）からWCAG相対輝度でコントラスト比を計算した。しきい値未達は以下の4パターン（重複除去後）。判断が必要なため**実装はしていない**:

1. **🔴最重要: `--teal`(#2BB3A3)地に白文字がコントラスト比約2.6:1（WCAG AA基準4.5:1を大きく下回る）**。対象はホームの最重要CTA「きょうやった！」ボタン（`#doneBtn`/`.done-btn`）、「動画を探す」のタグ選択中チップ（`.chip-b.on`）、使い方ツアー内の記録カード見本の帯（`.gcm-pill`）。**これはダークモード固有ではなく、`--teal`が`body.dark`ブロックで上書きされていないためライトモードでも同じ2.6:1**（実測確認済み）。「きょうやった！」はアプリ全体で最も押される主要ボタンであり、対象ユーザー層（50-60代）を踏まえると最優先で判断してほしい。直し方の方向性（未実装・案のみ）: ①teal自体をもっと暗く（白文字のまま4.5:1を満たすには概ね#1E7A70程度まで暗くする必要がある）、②文字色を白でなく濃色にする、のどちらか。
2. **タブバー非選択ラベルのダークモード側を再計測**: AUDIT-MEMO.mdに残っていた「#847D6C on #211E19≒4.1:1・辛うじて許容」を実レンダリングで再検証し、**4.06:1**で確認一致。ただしこれは通常文字（12px・800ウェイトだが18.66px未満のため「大きい文字」扱いにならない）の基準4.5:1には届いておらず、UIコンポーネント/大文字の3:1は超えている、という位置づけが正確。「辛うじて許容」は妥当だが「解決済み」ではなく、正式なAA基準（本文4.5:1）は依然未達であることを明記しておく。
3. カレンダーの他月グレーアウト日付（`.cal .d.mute`）が2.57:1。他月であることを示す意図的な弱調表示に見えるため優先度は低いと考えるが、実測値として記録。
4. 使い方タブ/使い方ツアー内の記録カード見本（ゴールド版・節目カード）の帯`.gcm-pill`（固定色#B8860B・白文字）が3.25:1。実画像生成（`drawCard`のcanvas描画）とは別の、HTML側の説明用モックアップにのみ存在する装飾要素で優先度は低いと考える。

**判断待ち（未実装）: エクスポートに端末の表示設定（テーマ/文字サイズ）が含まれ、機種変更時に新端末側の設定を上書きする**。`buildExportString()`は`kyono_`接頭辞の全キー（`kyono_theme`/`kyono_bigtext`含む）をコピーし、`importData()`側のクリーンアップロジックは「取り込んだキー＋端末の表示設定は除外して掃除」という意図のコメントがあるが、`kyono_theme`/`kyono_bigtext`はエクスポート側で既に取り込み対象に入っているため実質的には**旧端末の見た目設定が新端末に上書きされる**（実測: ダークモードにした状態でエクスポート→まっさらな新端末に見立ててインポートすると新端末もダークモードになる）。「記録の引っ越し」なのでテーマも一緒に運ぶのが自然という見方もでき、バグと言い切れないため実装はせず記録のみ。

**検証**: `npm test`は本セッションの変更単体では新規リグレッションなし（`importData: count guard`はcnt>=50に追従して引き続きPASS、最終確認時点で159 checks全PASS）。作業中、同一リポジトリで並行セッションが記録カード分割（`app-card.js`化）を進行中でindex.html/scripts/qa.jsが繰り返し上書きされたため、都度`git status`/`grep`で自分の担当行が残っているか確認しながら進めた（先行事例の`git stash`分離に倣った運用）。並行セッションの完了後に再実行した`npm test`はクリーンな159 checks全PASS。`npm run smoke`も既存17ステップに影響なし（変更箇所が競合しないことを確認）。
...
また、リポジトリ直下に文字化けした断片ログ`hc_dev32/33/34/35/38/39/65/66/67/68.txt`（計10個）がgit追跡されており、`.github/workflows/pages.yml`が`.git`/`.github`以外を丸ごとGitHub Pagesへ配信する仕組みのため公開サイトからも配信されてしまっていた。内容を確認した結果、いずれもUTF-8バイト列が壊れた復元不能な断片（qa.js/smoke.jsのcheck数やgitコミットハッシュへの言及はあるが文章として成立せず）で、重要な設定・データではないことを確認し`git rm`で削除した（削除後`npm test`影響なしを確認）。

作業中、同一リポジトリで複数セッションが並行編集中だったため、README.md編集とhc_dev削除はeven-syncの自動コミット（`auto-sync 2026-07-17 14:02`、および並行セッションのコミット`1a371a8`）に取り込まれる形で先に反映された。本エントリ以降の`HANDOFF.md`残り分の是正のみを本セッションで別途コミットする。

## 2026-07-17 「せんぱいの声」積み残し2件、本人YESで両方残すことに決定

2026-07-12の赤旗方針整合の削除（VOICES 120→116枚）で保留になっていた2件（「薬が効かない頭痛にも支えに💊」「産後3ヶ月 2日で眠れた😭」）について、本人に内容を確認してもらい**両方とも残す**でYES。理由: 「薬が効かない頭痛」は本文中に「ロキソニンが効かなくなり病院へ行くと緊張型頭痛ということで」と**実際に受診・診断済み**である旨が明記されており、削除した4件（受診をスキップして「1回で治った」型の危うい体験談）とは性質が異なると判断。「産後3ヶ月」も1回・即効性を謳う体験談ではなく2日間の継続ケアの話で、削除4件のパターンとは異なる。コード変更なし、対応不要。
## 2026-07-17 相談室に新インテント3件を追加（NEW-INTENTS-PROPOSAL.md②③④・本人承認済み）

`NEW-INTENTS-PROPOSAL.md`の②指の関節の朝のこわばり（`yubikansetsu`）／③頭のてっぺん/頭皮のこり・圧痛（`touhi`）／④シニアの階段・歩行の衰え（`kaidan`）の3件をsoudan-kb.jsに追加した（①腕・上肢のだるさは別セッションで2026-07-14に不採用決定済みのため対象外）。chip/empathy/mitate/keizoku/kwは提案書の文言を一字一句変更せず使用、動画は承認どおり各1本構成（提案書内の3本構成推奨は今回不採用）。

- **配置**: `touhi`はA節(首・頭まわり)末尾にA-8として追加。`yubikansetsu`/`kaidan`はD節(脚・足まわり)末尾にD-21/D-22として追加（`yubimukumi`が手・指のむくみでD-7にある前例を踏襲）。
- **kw移管**: `yubimukumi`のkwから「指がこわば」を削除し`yubikansetsu`側に一本化（こわばり=関節系／むくみ=指輪・手のむくみ系に整理、本人承認済み）。実測で「朝起きると指がこわばる」が`yubikansetsu`に正しく着地し、`yubimukumi`の他kw（「指がむくむ」等）は健在であることを確認。
- **kw微修正（QA対応・文言変更ではない）**: `yubikansetsu`の「ヘバーデン」と`kaidan`の「エスカレーター」は、既存のひらがな表記kw（「へばーでん」「えすかれーたー」）と正規化後に完全一致し`scripts/qa.js`の「同一インテント内カナ違い重複」検出に引っかかるため削除（マッチング挙動に影響なし。ユーザー向けの見立て等の文言は無変更）。
- **文字数バンド例外**: 3件ともmitateが受診の線引きを含むため60-120字の目安を超過（`yubikansetsu`240字/`touhi`198字/`kaidan`218字）。`youtsuu.mitate`(160字)の前例を踏襲し、`scripts/qa.js`の`LEN_EXCEPTIONS`に3件を追加して対応（文言は不変）。
- **soudan-ai-poc/data.mjs**: `build-data.mjs`で再生成（intents 119→122件）。
- **実機検証**: puppeteer-core（使い捨てスクリプト）で実アプリの相談室チャットに「指の関節がかたい」「朝起きると指がこわばる」「頭のてっぺんが痛い」「階段がしんどい」を入力し、それぞれ意図どおりのインテントに着地・指定動画ID・safety一文（「医療機関に相談してね」）の表示を確認。既存インテントの横取りが無いことも実測（「指がむくむ」→yubimukumi/「腕が上がらない」→shijukata/「階段がつらい」→hiza、いずれも変化なし）。redFlags優先順位も実測（「指の関節が腫れてしびれる」「頭のてっぺんが急に激痛でこぶがある」「階段でつまずいて転んだ」等はすべて新規インテントより先に赤旗が拾う）。
- 検証: `npm test`=**132 checks PASS**（119→122件化に伴う件数変化のみ、チェック項目数は不変）。`npm run smoke`=**17/17 PASS**（回帰なし）。`node soudan-ai-poc/redflag-safety-test.mjs`=**90/90 PASS**（回帰なし）。

## 2026-07-17 除外62本の目視確認、ポケモンのラジオ体操3本をallowlist化して復活（CATALOG-AUDIT.md棚卸しの続き・本人承認済み）

`CATALOG-AUDIT.md`の「除外62本を目視確認して、復活させるものだけ明示allowlist化」を実施。前回セッションで洗い出されていた復活候補3グループ（ポケモンコラボのラジオ体操3本／ライブ配信シリーズ6本／グッズ紹介4本）を本人に提示し、**ラジオ体操3本のみ復活・他は見送り**で承認を得た。

- `scripts/build_catalog.py`: 動画IDを直指定する`ALLOWLIST_IDS`（3件）を新設。既存の`EXCLUDE`正規表現（「ポケモン」等）はそのまま維持し、除外判定の直前に`vid not in ALLOWLIST_IDS`を条件追加する形でピンポイントに例外化（正規表現を緩めて他の除外動画を巻き込む事故を回避）。タグは3本ともタイトルに「全身」の語が無く自動タグ付けルールでは拾えないため、`ALLOWLIST_IDS`該当時のみ既存タグ`全身`を明示追加する分岐を追加（新規タグは起こさず既存体系に収めた）。
- 復活3本: `RmjQNmxILMg`（【ポケモン】ラジオ体操第一を"ルカリオ・ラプラス"と一緒にやってみよう！）／`mO8bXLLky4I`（【ポケモン】ラジオ体操第二を"ラッキー・ルカリオ"と一緒にやってみよう！）／`kOyRA7cvLD8`（ラジオ体操第一をポケモンたちとやってみた）。付与タグはいずれも「引き締め・全身・10分以内・ショート」。
- 見送りとした復活候補（除外のまま）: ライブ配信シリーズ6本、グッズ紹介4本（うちポケモン関連4本＝ピカチュウとのお買い物・潜入系は「グッズ紹介」寄りとして除外継続）。
- **検証**: `npm run catalog:update:offline`実行で`videos.js`が451→454本に。検索ロジック（`app-search.js`の`currentHits()`相当）をNode.jsで再現し、キーワード検索「ポケモン」「ラジオ体操」・タグ絞り込み「全身」いずれもこの3本がヒットすることを実測。除外を維持する動画（「ポケモンセンター」「潜入」「生配信」「ライブ」等のキーワード）は0件のままで意図しない復活がないことも確認。
- **npm test/npm run smoke**: 作業中、同一リポジトリで並行セッションが`soudan-kb.js`/`scripts/qa.js`を編集中（新規相談室インテント追加のWIP、未コミット）で、その状態を含めて`npm test`を実行すると別件のkw重複チェックで1件FAILが出た。`git stash`で自分の担当ファイル（`scripts/build_catalog.py`・`videos.js`）のみを隔離した状態で再実行し、**132 checks全PASS**であること（＝自分の変更が原因でないこと）を確認してから並行セッションの変更を`git stash pop`で復元した。`npm run smoke`は**17/17 PASS**。並行編集中のファイルには一切手を触れていない。
- 詳細は`CATALOG-AUDIT.md`の「除外62本の目視確認 結果（2026-07-17）」を参照。
...
- `soudan-kb.js` `redFlags`: 既存の`kw`配列（178語）は**無変更**（削除・改変なし＝カバレッジ後退なし）。新たに`stateKw`（妊娠,妊婦,手術,術後,入院,産褥,さんじょく,悪露,大量出血,恥骨,帝王切開,ていおうせっかい,出産直後,しゅっさんちょくご,産後直後,産後すぐ,産後まもなく,産後まだ,退院したばかり,たいいんしたばかり,産後1ヶ月,産後1か月,産後3週,産後2週,産後1週,産後数週,産後半月の27語=`kw`の部分集合）を追加。`answer`（症状系文面・既存のまま）に加えて`answerState`（状態系専用文面「その状態のときは、無理せず主治医の先生にOKをもらってからにしよう。落ち着いてOKが出たら、また一緒にやろうね。」）を追加。
- `index.html`: `sdRedFlagHit`（赤旗ヒット判定=true/false）は無変更。新規`sdRedFlagKind(n)`を追加し、ヒットした語が`stateKw`のみなら`"state"`、症状語(stateKw外のkw)が1つでも混ざれば`"symptom"`を返す（安全側優先＝両方ヒット時は症状系）。`sdAnswerRedFlag(n)`はこの判定で`answer`/`answerState`を出し分ける。呼び出し元`sdSend()`は`sdAnswerRedFlag(n)`に正規化済み入力を渡すよう変更。
- `soudan-ai-poc/norm.mjs`・`worker.mjs`（PoC/Cloudflare Worker側の同型ロジック）にも`redFlagKind`を追従実装し、`redFlagAnswer(n)`が引数を受け取って同じ分岐をするよう更新（挙動の一貫性維持）。`soudan-ai-poc/data.mjs`は`build-data.mjs`で再生成。
- `soudan-ai-poc/redflag-safety-test.mjs`に恒久回帰テストを追加: 状態系3件（妊娠中で腰が痛い/術後でまだ痛い/産後1ヶ月です→state）、症状系3件（激痛がある/胸が痛くて冷や汗が出る/しびれがある→symptom）、混在1件（妊娠中で急に激痛が走った→symptom優先）の計7件。
- **実測確認**: 上記7件すべて期待通りの分岐を確認。`node soudan-ai-poc/redflag-safety-test.mjs`は83件→**90/90 PASS**（既存の赤旗関連テスト全件を含めて回帰なし）。`npm test`は**114 checks 全PASS**。`npm run smoke`は**16/16 PASS**（相談室ゴールデンフロー含む）。
- **開発メモ（並行編集への対処）**: 作業中、同一リポジトリで並行セッションが`app-record.js`分離・`app-quiz.js`のQ5スキップ機能などを並行編集しており、複数回にわたり本作業のuncommitted変更が巻き込まれた`git stash`（"check-head-baseline"等）で退避され消失する事象が発生した。原因は特定できなかったため、パッチをべき等なPythonスクリプト化して即座に`git add && git commit`する運用で確定させた（コミット`5acfed0`）。次にこのファイル群を触るセッションは、着手前に`git status`で他セッションの未コミット変更と衝突していないか必ず確認すること。

## 2026-07-17 OGPカード画像を新規作成（LINE/Xシェア時の表示専用・叩き台）

シェア時のリンクカードが`icon-512.png`（アプリアイコンの流用）のままだったため、1200×630の専用OGP画像`assets/ogp-card.png`をPython(Pillow)で合成した。生成スクリプトは使い捨てで保存していない（作業用一時ファイルのみ）。
- **構図**: 暖色グラデーション背景（`--bg`寄りのクリーム色→`--pink-soft`、`.grad-soft`系のトーン参照）に、`assets/logo/logo-main.png`（透明背景・アプリ名「#きょうのオガトレ」＋キャッチコピー「みんなで一緒にストレッチを習慣化」を含む既存ブランド資産、フォント/配色は最初から確定しているためテキスト再描画は行わずこれをそのまま使用）を左上に配置。右下に`chara-good.png`（ガッツポーズ）を大きく、左下に`chara-cheer.png`（旗を振る）を小さく配置し、それぞれの背後に`--yellow-soft`/`--teal-soft`のぼかし円で奥行きを追加。ロゴの既存スパークル装飾に合わせて`--pink`/`--teal`/`--yellow`の4方向スパークルを散らし、下端に`--yellow`の細いアクセントバーを敷いた。300px幅のサムネイル縮小でもタイトル・キャラクターとも視認できることを確認済み。
- `index.html`の`og:image`を`assets/ogp-card.png`（絶対URL）に変更し、`twitter:card`を`summary`→`summary_large_image`に変更、`twitter:image`を新設。
- `sw.js`のASSETSに`assets/ogp-card.png`を追加、キャッシュ版を`kyono-v46`→`kyono-v47`に更新。
- **注記**: 作業時、同一リポジトリ上で並行して別セッションが`store`→`app-record.js`分離のリファクタを進行中だった。even-syncの自動コミット`f5aec8f`（auto-sync 2026-07-17 10:14）に、このOGP変更（index.html/sw.js/assets/ogp-card.png）と並行リファクタの一部が一緒に入ってしまっているが、内容は独立しておりOGP側の差分はコミット内で完結している。`npm test`はこの時点で114checks全PASS。`npm run smoke`は並行リファクタが未完了(`store is not defined`等)のため0/16で失敗中だが、原因はOGP変更とは無関係（並行セッション側の完了待ち）。

## 2026-07-17 配布文書2点の旧タブ名を実際のタブ定義に合わせて修正（コード変更なし）

`index.html`の`.tabbar`実体（`TAB_OF`定義周辺、1067-1071行目）で現行タブ名を確認: 使い方／マイ記録／ホーム／再生リスト／動画を探す の5つ。これに対し配布文書2点に旧称が残っていた（README.mdは7/15棚卸しで既に修正済みだったが、この2ファイルは対象外だった模様）。
- `docs/qa-checklist.md`: 「きろく」タブ→「マイ記録」タブに修正（25・34・54行目、計3箇所）
- `docs/invite-kit.md`: 「つかいかた」→「使い方」、「さがす」→「動画を探す」に修正（28・33・75・76行目）。「つかいかた」は「使い方」の単なる仮名表記で実害は小さいが、「さがす」は実際のラベル「動画を探す」の一部しか一致せず（リクエスト送信導線＝`app-search.js`の「さがす」タブ最下部という機能自体は実在を確認済み）、鉤括弧でタブ名を明示している文脈なので実文言に揃えた。
- コードは無変更。`npm test`はドキュメントのみの変更のため影響なし（実行して回帰なしを確認）。

## 2026-07-16 カレンダー通知（唯一の再来訪装置）をオンボーディングに接続（本人監修待ちだった項目・仕様承認済みで実装）

AUDIT-MEMO.mdの本人監修待ちリストにあった「唯一の再来訪装置（カレンダー通知）がオンボーディングに接続されておらず、マイ記録タブの設定カード深部に埋没」を、プロダクトオーナー承認済み仕様で実装した。

- **差し込み場所**: オンボーディングQ3「いつやる派？」に回答した直後（`obPick`の`q.key==="anchor"`分岐）。`setAnchor(c.v)`→アンカーの相槌(`anchorAck`)が出た直後に、カレンダー登録カードを挟んでから自動で次（Q3が最終問のためルーティングCTA）へ進む。
- **タップしなかった人の扱い**: 何も言わず自動的に次へ進む（スキップ用チップは作らない・本人承認済み仕様）。
- **実装**: `obBubble()`（台本文言をtextContentで描画する安全設計・意図的にHTML埋め込み禁止）は不変のまま、新設の`obCalendarBubble()`（index.html）が開発者ハードコードの固定HTMLテンプレートだけを`obLog`に追加する。ICS/Googleカレンダーのリンク生成ロジックは重複実装せず、既存`renderIcs()`を呼んでから「つづける設定」カードの`#icsLink`/`#gcalLink`（SPA構造上、非表示タブでも常にDOMに存在＝`SECTIONS`はhiddenクラス切替のみで確認済み）が計算した結果のhrefをそのままコピーする方式。standalone時の案内（Appleカレンダーはdata URLダウンロードが無反応になる旨）も既存`icsNote`と同じ判定で添えている。
- **実測確認（puppeteer-core、390×844）**: あさ(07:30)・おふろ上がり(20:30)・ねるまえ(21:30)の3アンカーそれぞれで、Q3回答直後にカレンダー登録カードが表示され、ICS(`data:text/calendar`のDTSTART)・Googleカレンダーリンク(`dates=`)とも正しい時刻になっていることを確認。タップした場合（あさ）はページ遷移せず質問フローが継続、タップしなかった場合（おふろ上がり・ねるまえ）も自動的に次へ進むことを確認。最後はかたさチェックQ1への遷移も壊れていないこと、マイ記録タブ側の既存`#icsLink`/`anchorNow`（回帰確認）も正しく動作することを確認した。
- **機械チェック追加**: `scripts/qa.js`に`obBubble`の安全設計不変・`obCalendarBubble`がICS生成を重複実装していないこと（`renderIcs()`呼び出し・`BEGIN:VCALENDAR`不在・`#icsLink`/`#gcalLink`href読み取り）・`obPick`のanchor分岐が`obCalendarBubble`→`obAskQ`の順で自動接続されていることを固定する静的チェックを追加（103→111checks）。`scripts/smoke.js`に新規ステップ「1c」を追加し、3アンカー×実ブラウザでの上記実測をCI/手元双方で再現可能にした（15→16ステップ）。
- `npm test`は111checks全PASS、`npm run smoke`は16/16 PASS（既存15ステップも回帰なしで通過）。
...
- **Gmailフィルタ設定済みに伴い「朝6時ブリーフにリクエスト集計を追加する約束」が発動**。ハブ側=alan本体の作業のため保留中欄に📮付きで残し、監督へ報告済み（このリポジトリでは作業しない）
- **過去エントリの待ち表記に解決済み注記を追記**: 7/13夜の申し送り「7日目は記念イラストでは？」（→同日深夜の記念イラスト5種追加で解決）／7/13棚卸しリストの「初期チップ代表化(119個問題)」（→7/14の5カテゴリタブ化で解決）・「NEW-INTENTS-PROPOSAL 4件」（→①は7/14本人回答で不採用決定・残り②③④のみ）・「茜さんイラスト『お祝い』」（→受領・実装済み）
- **継続が正しいと確認した保留**: q3/q4実写化（assets/check/の現q3.jpgは7/5旧宣材版の未参照遺物と判明・注記追加）／節目30秒動画／赤旗の状態語2バケット化・15時よるバッジ境界・FAB占有・ありがとう文言・カレンダー通知オンボ接続（監修待ち）／visualViewport×2（実機待ち）／オンボQ2→Q5スキップ（仕様判断待ち）／soudan-kb検収txt v1/v2（本人ペースで実施中）／HOMESCREEN-FUN-AUDITのbrag画面FAB重なり（7/15起票・回答待ち）／SOUDAN-QUALITY-AUDIT系はラウンド1〜3とも本人回答済み・反映済みを各ファイルのステータス注記で確認（新たな待ちなし）

## 2026-07-15 相談室の長文コメント見切れ、追加で根治（本人「まだ見えにくい」の指摘を受けて）

直前のスクロール位置調整（下記エントリの2件目）だけでは不十分だった。実測すると、`sdChipTap`でチップを押した瞬間、`sdChipsMode`はすでに`followups`（会話モード）へ切り替わっているのに、DOM側のカテゴリタブ列（`#sdCatRow`）は**返信メッセージが全部出終わるまで**古い2段表示のまま残っていた（`sdRenderChips()`がメッセージ列の一番最後でしか呼ばれず、そこで初めてタブ列が畳まれる設計だったため）。つまり返信が流れているあいだ中ずっとメッセージ欄が狭いままで、スクロール位置をどう工夫しても根本解決にならなかった。
- **根本修正**: `sdPush(msgs)`の先頭で、`sdChipsMode.type!=="intents"`（＝会話に入るとき）なら`#sdCatRow`を即座に空にするようにした。これでチップを押した瞬間にタブ列が畳まれ、最初の吹き出しから広い表示領域を使える（実測: カテゴリタブ2段=168pxのタブ高さ→タップ直後に2px、メッセージ欄117px→283pxへ即座に拡大）。
- **副次修正**: `sdScrollToTop`が入場アニメ(`sdpop`のtranslateY)やスクロールバー出現による再折り返しの影響で数十px単位でずれることがあったため、スクロール適用を即時1回＋`requestAnimationFrame`で1フレーム後に再補正する2段構えにした。実測（puppeteer-core、160字の見立て文で検証）で全ての吹き出しの冒頭が可視領域内に収まることを確認。
- `npm test`(103checks)・`npm run smoke`(15/15)に影響なし。

## 2026-07-15 改修2件: しなやかネコ画像の左端の線／相談室の長文コメントが見切れる問題

- **`assets/type-yawara.png`（硬さチェック結果「しなやかネコ」）の左端に線が入る**: 実測（PILでアルファチャンネルを列ごとに走査）したところ、x=3〜4列に画像全体の高さのほぼ全域にわたる半透明の帯（アルファ値96/66程度）が残っていた。type-momo.png/type-kenko.pngの同じ左端では完全に透明（alpha=0）なのに対し、type-yawaraだけに存在する孤立した帯で、実際の猫のイラスト自体はx=90付近から始まっており無関係と確認。書き出し時のクロップ/はみ出し跡と判断し、x=0〜9列のアルファを0に潰して除去（イラスト本体には触れていない）。
- **相談室で、カテゴリタブが2段になっているあいだに長い見立て文が来ると、その冒頭が画面外に押し出されて読めない**: `#sdLog`は`flex:1`でカテゴリタブ(`.sd-foot`)の高さに応じてメッセージ表示領域が伸縮する。従来の`sdScrollEnd()`は常に「ログの一番下」まで送っていたため、タブが2段でメッセージ領域が狭いときに160字級の見立て文が来ると、その先頭が押し出されて読めないことをpuppeteer-coreで実測確認（190px弱の狭い状態で最大150px超が見切れ）。新規`sdScrollToTop(row)`（`getBoundingClientRect`差分でスクロール位置を計算＝`offsetTop`は最寄りの位置指定祖先基準になり得て信頼できないため使わない）に置き換え、吹き出し追加のたびに「その行の頭」が見える位置までスクロールするよう変更。あわせて、次の吹き出しのタイピングドット出現時にスクロールを動かすのをやめた（動かすと直前の長文の頭が即座に押し戻されてしまうため）。実測で誤差数px程度（CSSの入場アニメーション`sdpop`のtranslateY(4px)分)まで縮小したことを確認（元は80〜150px超の見切れ）。
- 両修正とも`npm test`(103checks)・`npm run smoke`(15/15)に影響なし。
...
- **申し送り（本人質問・未回答）**: 「7日目って記念イラストカードじゃなかった？」→現設計では7日目はTOKU_CARDS対象外（記念イラストは4/30/66/100/200/300/365/500/730/1000/1900日の11種のみ。7/14/21/50/150日は画像なし節目=従来ゴールドカード温存）。7日目を記念イラスト化するには新素材が必要（本人判断待ち）**（→2026-07-13深夜に解決済み: 7/14/21/50/150日の記念イラスト5種を追加。上の同日エントリ参照）**
...
- **AUDIT-MEMO棚卸し結果（2026-07-13時点）**: 要対応10件は全消化（#5=本人判断で現状維持・#10=エクスポート/FAQ等で緩和済み）。中低59件も大半消化済みで、**残りは全部「人待ち」**:
  - 本人監修待ち: 赤旗の状態語2バケット化／初期チップ代表化(119個問題)**（→2026-07-14解決済み: 5カテゴリタブ化）**／15時よるバッジ境界／FAB占有／ありがとう文言(送信明言問題)／NEW-INTENTS-PROPOSAL.mdの新インテント4件**（→①腕・上肢のだるさは2026-07-14本人回答で不採用決定済み。残り②③④のみ監修待ち）**／カレンダー通知のオンボ接続(新規文言)
  - 実機待ち: 相談室シートのvisualViewport対応(iOSキーボード)×2
  - 素材待ち: q3/q4実写化(本人写真2枚)・茜さんイラスト「お祝い」**（→お祝いは2026-07-14解決済み: chara-congrats.png受領・接続、Drive正式版へ差し替え完了。q3/q4のみ継続）**
...
## 保留中（再開時はここから）
- ⏸ 本人のチェック用写真2枚（肘上げ正面・しゃがみ横向き全身）→ 届いたら`assets/check/`のq3/q4を実写化。※`assets/check/`に現存するq3.jpgは7/5の旧・宣材写真版（「背中で両手」時代の遺物・現コードから未参照）なので混同しないこと
