...
### 保留3件（これが残りの全部。TaskList #4 #8 #10 と同じもの）

**D4 記録カード表示の演出が無機質** — 本人指摘。alan5が設計案を出して本人が選ぶ番。
**アイコンでトーンが固まった今が着手適期**（本人にもそう伝えた）。サムネイルが直るまで
待つ、という前提条件は**ビルド4で解消済み**。⚠️ 提案は「刺激を足す」ではなく
「できたことが伝わる」で立てる（憲法: 数字・ノルマで追い立てない／洗練＝引き算）。

**D8 使い方の部位選択が色だけで分かりにくい** — 対象は`OnboardingViews.swift:51-55`の
`worry`チップ（肩こり・首／腰／前屈できない／眠り／とくにない）。**イラストを当てる**という
本人の指示。**スクイズハンターのトーンに合わせる**こと。**未着手。** 手順はD9で確立済みの
`scripts/gen-type-art.py`をなぞればよい（アンカー画像=
`~/Claude/squeeze-hunter/art-raw/_anchor_melonpan.png`をIMAGE 1に、`/v1/images/edits`、
`background=transparent`、出力を`art-raw/`→alpha bboxでトリム→長辺380pxで`assets/`へ）。
**squeeze-hunter配下は読むだけ。書くな。**

**D10 タップできることが見た目で伝わらない** — 本人が3画面で繰り返し指摘（「文字だけじゃ
なくて挙動でわかりやすく」）。**D4と一緒に設計すると一貫する**（どちらも「押せそうに見せる」）。
