...
- **最新: ビルド22配信済み（2026-08-04夜・version22/VALID/紐付け/whatsNew裏取り済み）**。中身=本人の詰めラウンド一括: 黄ボタン復帰(案B=#FFD93B+文字#26261F+縁#8A6D00・**藍D2は1ビルドで引退**・トークン撤去済み)・案B全展開・初回チャット選択肢の濃色化(5色実測表あり)・カード間隔+7pt・**ライト背景#F7EEDC**(line=#EBDFC8に玉突き調整)・ダーク磨き(bg#1C1915?報告参照・card分離1.42:1・構造枠7.7:1、**カテゴリ識別色の枠は据え置き=台帳**)・つづけた日数再設計(通算全廃・数字主役・メモ1行合体)・ホーム相談チップ削除・図鑑1枠化。配色の経緯: 濃地白文字D2(ビルド21)→本人実機で黄色回帰。**色の裁定は必ず実機確認後に確定と心得よ**。
