...
- シェア: カードモーダルは2ボタン構成（茜さん案 2026-07-07）——`saveCardImage()`=文言なしshare（共有シートに「画像を保存」がきれいに出る）／`shareCard()`=`#きょうのオガトレ ◯日目！`つきshare。X=画像+テキスト・インスタ=画像のみ（テキストは仕様で落ちる）。**XやLINEの個別ボタンを自作しないこと**（URLインテントは画像を渡せない・標準共有シートが唯一の画像添付経路）。**シェア文言にURLは意図的に入れていない**（β配布中の拡散制御・正式公開時に追加を検討）
