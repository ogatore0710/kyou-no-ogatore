...
- **iOS**: `ios-native/KyouNoOgatore/SafetyCore/`をローカルSPMパッケージとして新設（`project.pbxproj`を手編集してネイティブXCTestターゲットを足すgitlink/破損リスクを避けるため。判定ロジックを`SafetyGate.swift`1箇所に隔離する構造上の強制にもなる）。`SafetyGateTests.swift`（safety-fixtures.json 111件+norm-golden.json 16件をループでXCTAssert）+`SoudanEngineTests.swift`（crisis>赤旗>通常の優先順位を縛るengine-fixtures 3件）を先に作成。スタブ実装（`swift test`→`safety-fixtures: 42/111 pass`+engine-fixtures 2/3失敗=想定どおりの全赤）を`61f8b38`でコミット後、実装（マスタープラン§3-3の許可リスト数値範囲判定・Unicode.Scalar単位処理・「寝転」除去のNSRegularExpression選択置換）を`f415d0c`で緑化（**111/111+normゴールデン全緑+engine-fixtures 3/3**。実装コミット自体はeven-syncの自動コミット`4055155`に先に取り込まれた＝艦隊の共有作業ツリー特性）。
- **Android**: エミュレータ不要のプレーンJVM JUnitユニットテスト(`app/src/test/`)として実施。JSONデコードはAndroid SDK同梱org.json（JVM単体テストではスタブで動かない既知の罠）を避けkotlinx.serializationを使用。`soudan-kb.json`を`app/src/main/resources/`（AssetManager非依存のクラスパス経由）に配置。JUnit4 `Parameterized`で111件+16件を1件=1テストケースとして展開（`SafetyGateFixtureTest`/`NormGoldenFixtureTest`）+`SoudanEngineTest`（iOS版と同一の3文例）。スタブで`da12063`（**148 tests, 84 failed**=iOSと同数の69件fixture失敗+engine-fixtures 2/3失敗=想定どおりの全赤）→実装（Kotlin CharがJS/Javaと同じUTF-16コード単位である点を活かしiOS版よりシンプルに移植。JVMの`String.contains()`は正準等価比較をしないためSwift版で必要だった自前スカラー比較の回避策も不要）で`37cf382`（**148/148全緑**）。
...
- push済み（iOS: `f415d0c`・Android: `da12063`→`37cf382`）。検証ログは`ios-native/verify/step2-safetycore-{red,green}.log`・`android-native/verify/step2-safetycore-{red,green}.log`にコミット済み（alan5への報告用証跡）。
