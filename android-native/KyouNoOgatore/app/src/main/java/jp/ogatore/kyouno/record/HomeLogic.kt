...
data class RefreshDayResult(val dayChanged: Boolean, val today: String)

object HomeLogic {
    // index.html:1655 fdActive の忠実移植。
    fun fdActive(fd: String?, streakTotal: Int): Boolean = fd == "go" && streakTotal == 0

    // index.html:1660-1667 fdFocusHome の判定式(当日限定)の忠実移植。翌日以降に記録せず戻ってきた
    // 人には通常ホームを見せる仕様(HANDOVER第7項・過去に複数日貼りつきバグが発生した既知の壊れやすい箇所)。
    fun fdFocusHomeActive(fd: String?, streakTotal: Int, fdday: String?, today: String): Boolean =
        fdActive(fd, streakTotal) && fdday == today

    // app-env.js:60 refreshDayの「日付またぎ検知」部分の忠実移植。テーマ適用・renderAnchor等の
    // UI副作用はビュー層の責務であり、ここは「日が変わったか」の判定だけを純粋関数として切り出す。
    fun refreshDay(now: Instant, lastDay: String, zone: ZoneId = ZoneId.systemDefault()): RefreshDayResult {
    ...
    // 保持し(§2-3・sessionStorage相当)、この関数がtrueを返した後は呼び出し側で1回きり消費(nullに戻す)
    // こと(JS版の`sessionStorage.removeItem`に相当。「一度出したら消す」を再現するのは呼び出し側の責務)。
    fun shouldShowDoneNudge(pendingNudgeDate: String?, today: String, streakDates: List<String>): Boolean {
        val d = pendingNudgeDate ?: return false
