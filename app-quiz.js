...
const TYPES = {
  ...
  kenko:{name:"飛べないダチョウ", area:"肩甲骨",
    // 2026-07-20監査⑥: 新Q3(胸の前で両ひじをつけて上げる)に合わせてcopyを更新(旧「背中で手がつながらない」は旧式チェックの表現)
...
function finishQuiz(){
  ...
  // チェック完了後はホーム「きょうの1本」を「あなた用」既定に戻す(2026-07-20監査Top2)。
  // チェック前のホーム描画でstate.modeがautoMode()の値(あさ/よる)に固定されたままだと、
  // 結果画面「きょうはこれ1本でOK！」と別の動画がホームの主役になり連続性が切れるため、
  // モード選択(手動含む)をリセットしてrenderTodayの既定(mineAvail→"mine")を効かせる。
  state.mode=null;
  try{ store.set("mode_manual",null); }catch(e){}
