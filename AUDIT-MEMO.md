...
- 🟡中 **オガトレ通信（OBU_FEED）が投稿1件のみ＝運用ゼロ設計の中で唯一の人力更新箇所が既に息切れ予備軍** — obu-feed.js（投稿は2026-07-09の1件のみ）／index.html 906-910（NEW📣バブル常時表示）
  ...
  - **ステータス: 対応済み（2026-07-17実装）**: 本人に更新を促す方向ではなく「更新が止まっても自然に見える」受け身の対策として実装。`obuHasNew()`に鮮度しきい値`OBU_STALE_DAYS=30`（日）を追加し、未読でも最新投稿が30日以上前なら`NEW📣`を出さないようにした。あわせて`obuPostInner()`が30日超の投稿には`.obu-date-old`クラス（控えめな色・小さめフォント）を付与し、`renderObuPopup()`/`renderObuArchive()`双方の日付表示を目立たせすぎない体裁にした。督促文言・機能は追加していない。しきい値30日はおやすみ券(月3枚)・記録カレンダーの月次感覚に合わせた「だいたい1ヶ月」の目安（数日〜数週間の不定期更新は従来どおりNEW扱いしつつ、季節をまたぐ放置だけ自然にNEWを引っ込める）。実際のobu-feed.js（2026-07-09投稿・しきい値未満）の表示は変化しないことも実測確認済み。`scripts/qa.js`に`checkObuStaleness()`を新設し、実際に`obuHasNew()`を実行して「60日前・未読→NEWでない／数日前・未読→NEW／しきい値ちょうど→NEWでない／新しくても既読→NEWでない」を機械チェックで固定（回帰防止）。詳細はWORKING_NOTES.md 2026-07-17エントリ「オガトレ通信のNEW📣が投稿の古さを見ずに出続ける問題」参照。
