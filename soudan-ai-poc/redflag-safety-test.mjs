...
// 2026-07-20追加: VERIFICATION-REQUEST-2026-07-20 依頼2の攻撃テスト（臨床知識ベースの口語34件）ですり抜けた12件の
// カバーkw(産んだばかり系/ぎゅっと苦し/痛くて夜中/立てなくなっ/咳が止まら/力が入らな活用形/排尿障害/電気が走/頭を打っ/胸がずき/手足じんじん)と
// 無害文プローブ(誤爆回避)の固定。
...
// 2026-07-20 赤旗攻撃テストすり抜け分のカバーkw（VERIFICATION-REQUEST-2026-07-20 依頼2。
// 攻撃フレーズは司令塔Fableがkwリストを見ずに臨床知識から生成→実判定に通してすり抜けた12件+無害文プローブを固定）
const attack2026_07_20 = [
  // 受診側（追加kwで新規カバー）
  ["昨日から急に足がジンジンして眠れない", true],          // 足がじんじん（旧: nemuriインテントに誤マッチ）
  ["産んだばかりですが骨盤矯正やりたい", true],            // 産んだばかり（旧: kotsuban2に誤マッチ）
  ["胸のあたりがぎゅーっと苦しくなるときがある", true],    // ぎゅーっと苦し
  ["背中がいたくて夜中に何度も目がさめる", true],          // いたくて夜中（間に語が挟まる夜間痛）
  ["急に腰が抜けたみたいに立てなくなった", true],          // 立てなくなっ（旧: youtsuuに誤マッチ）
  ["せきが止まらなくて肋骨のへんが痛い", true],            // せきが止まら
  ["力がはいらなくてコップも持てない", true],              // 力がはいらな（活用形）
  ["年のせいか最近手に力が入らなくてお箸を落としちゃう", true], // 力が入らな（婉曲）
  ["おしっこが出にくくて腰も痛い", true],                  // おしっこが出にく（馬尾症候群サイン）
  ["尻から足にかけて電気が走るみたいに痛い", true],        // 電気が走
  ["頭を打ってから頭痛がつづいてる", true],                // 頭を打っ（旧: zutsuuに誤マッチ）
  ["息をすうと胸がズキッとする", true],                    // 胸がずき
  // 巻き込まない側（無害文プローブ）
  ["子どもが生まれたばかりで抱っこで肩がつらい", false],   // 生まれたばかり(赤ちゃん側)は状態語でない
  ["予定が立てられなくなった", false],                     // 立てなくなっ とは別形
  ["夜中までスマホを見てて首が痛い", false],               // 痛くて夜中 の語順でない
  ["尿もれが気になる", false],                             // nyoumoreインテント領域を巻き込まない
  ["お風呂で体がじんじん温まる", false],                   // じんじんは手足限定
  ["切なくて胸がぎゅっとなる", false],                     // 苦し が無ければ発火しない
  ["電気をつけたまま寝てしまう", false],
  ["頭を使う仕事で疲れる", false],
];
for (const [c, want] of attack2026_07_20) {
  if (redFlagHit(norm(c)) === want) pass++; else { fail++; fails.push(`[攻撃テスト2026-07-20] ${c} → 期待:${want ? "受診" : "通常"}`); }
}
...
const redFlagKindCases = [
  ...
  ["術後でまだ痛い", "state"],
  ["産後1ヶ月です", "state"],
  ["産んだばかりですが骨盤矯正やりたい", "state"], // 2026-07-20追加: 産んだばかり系はstateKw(主治医OK待ち文面)
...
console.log(`赤旗の穴4件+寝転+胸痛動悸+crisis+締め付け脱力しこり+締め付け胸限定/夜間痛語順+過呼吸+攻撃テスト2026-07-20: ${pass}/${pass + fail} pass` + (fail ? " ❌" : " ✅"));
