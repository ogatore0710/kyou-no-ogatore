...
## 🔶 中断中: 画面遷移アニメーション §相談室シート化（2026-07-27）
`TASK-C2-2026-07-27-screen-transitions.md`。相談室を下からせり上がるシート(スクリム+角丸+
部分高さ)に変更。Android実機で動作確認済み(スクリム越しHome表示・チップタップ等の内部動作・
✕での復帰すべてOK)。iOSは実測83.3%(タスク記載の92%はWeb版の値で合わせる必要なし・
iOS標準シートの挙動に委ねるのが正解、とalan5が検証・判断済み)。次はオンボ→一般画面の順で
再開(text-size-accessibility完了・local-notifications発注済みの後)。
