...
## 🚀 進行中: ネイティブ移植（2026-07-25・Step7a完了→alan5検収待ち）
本人承認済みのストア版方針（iOS/Android・10月リリース枠。フルネイティブ移植を前倒しで即着手）を受け、[NATIVE-MIGRATION-MASTERPLAN-2026-07-25.md](NATIVE-MIGRATION-MASTERPLAN-2026-07-25.md)をappdevがWorkflowで作成→alan5検収OK→**Step0（断面固定）完了**→alan5が本人操作ゲート①（Xcodeプロジェクト作成）実施→**Step1（両OS空アプリビルド確認）完了**→**Step2（安全系テスト先行移植=最重要工程）完了**→**Step3（データ層+引っ越しインポート）完了**→**Step4（記録カード抽選・かたさ診断の決定的ロジック）完了**→**Step5a（ホーム・記録フロー・チュートリアルフラグ機械）完了**→**Step5b（カレンダー・マイ記録）完了**→**Step5c（オンボーディング・ツアー・診断UI）完了**→**Step6（相談室UI・安全系が絡む重要工程）完了**→**Step7a（検索・再生リスト・図鑑）完了**（appdev・Web版無変更・安全キーワード無変更）。Step7aでは検索(TAG_CATS+catalog.json 454件)・再生リスト・図鑑(getDexStatus相当。判定はStep4のCardLottery呼び出しのみ)・動画再生導線を実装。図鑑のロック/アンロック判定はWeb版実行結果をpuppeteerで採取した新規ゴールデン(`gen-dex-golden.mjs`)と106件全件突合。カード実写86枚(iOS用にPNG変換)・catalog.jsonを両OSに同梱。Androidはkyono_testエミュレータで検索絞り込み・動画タップ→ブラウザ遷移・図鑑4段表示を実タップ確認、iOSはフルビルド成功確認。安全系テスト(111+engine-fixtures)は両OSとも回帰なし（Android197/197・iOS SafetyCore8/8）。**「再生リスト」のスコープ解釈に注記あり**（Web版の実データ源PLAYLISTS配列は未抽出のため、catalog.jsonの一覧表示として実装——詳細はWORKING_NOTES.md参照、alan5確認待ち）。詳細はWORKING_NOTES.md 2026-07-25エントリ。次はStep7b（じまん・声・オガトレ部・設定+パリティ突合）でalan5の指示待ち。
