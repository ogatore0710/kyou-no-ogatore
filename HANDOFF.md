...
## ⏸ 宿題: ダークモードのラインボタン枠線コントラストが基準未達(build21・2026-08-04・alan5確認済み)

`KyonoLineButton`(もどる/とじる等)のダーク側の枠線色`#4A443A`(build18 B-4で導入・build21では
変更していない既存値)が、背景`#211E19`に対し実測**1.72:1**、カード`#2C2822`に対し**1.52:1**しか
なく、WCAG非文字コントラスト基準(3:1)を大きく下回っている。build21の配色刷新(D2)の実測棚卸し中に
発見(該当タスクのスコープはライトのみだったため、今回は修正せず発見のみ報告・alan5確認済み)。

**次にダークモードの入力欄・枠線まわりに手を入れる回があれば、あわせて直す候補として扱うこと。**
