...
92%高さ)に変更。Android実機で動作確認済み(スクリム越しHome表示・チップタップ等の内部動作・
✕での復帰すべてOK)。iOSはシミュレータ目視確認したが92%高さの上部ギャップがダークテーマで
視覚的に判別しづらく、次回再開時に切り分けが必要(詳細はWORKING_NOTES.md参照)。alan5より
優先タスク`text-size-accessibility`が入ったため一時中断。オンボ・一般画面はまだ未着手。
