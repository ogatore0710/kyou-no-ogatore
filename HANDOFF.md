...
## ✅ 解決済み: ダークモードのラインボタン枠線コントラスト未達 → build22 Z-6で根治(2026-08-04)

`KyonoLineButton`のダーク枠`#4A443A`(vs背景1.72:1・vsカード1.52:1でAA非文字基準3:1未達)は、
build22 Z-6で新設した構造系枠線トークン`colors.borderStrong`(ダーク`#B4AB9A`・vs新背景`#1C1915`で
7.70:1・vs新カード`#3A342C`で5.41:1)へ統一し解消。あわせて`line`トークンを枠線用途に流用していた
他の構造系箇所(カード/入力欄/チャット吹き出し 等、両OS計70箇所超)も同トークンへ統一済み。

## ⏸ 宿題(ダーク改修回の検討事項): カテゴリ識別色の枠線はbuild22 Z-6で意図的に据え置き(2026-08-04・alan5承認済み)

相談室/オンボの`obgDark`チップ枠(5色・1.74〜2.59:1)、検索カテゴリチップのダーク枠(4色・
1.82〜2.25:1)、セカンダリボタン/とどくメーターのteal系ダーク枠(3.44:1)は、いずれも背景比5:1
未達だが、build22 Z-6では**意図的に対象外**にした(理由: これらは「案B」で確定した意味づけ色
=カテゴリ識別・teal系統であり、5:1達成のために明度を上げると識別色としての「濃さ」自体が
変わってしまう懸念があったため)。alan5に確認済み・この判断で承認(2026-08-04)。

**次にダークモードの識別色まわりに手を入れる回があれば、個別カテゴリの色相を保ったまま
5:1へ底上げできないか、あらためて検討すること。**
...
5視点デザイン監査で「グラデーション背景(`KyonoGradientCard`)が画面のあちこちに増えすぎて
特別感が薄れている」との指摘を受け、**タブ・画面の性質ごとに上限を設けるルールを新設した**
(`TASK-C2-2026-08-02-build16-polish-and-ia.md` C部)。

- **L0(常設セクション)= グラデ禁止。** 通常のカードは白一色(`KyonoCard`)のまま。
- **L1(タブの顔級・そのタブを開いて最初に目立つ1枚)= 1タブにつき最大1枚まで。**
- **L2(祝い・おかえり・診断結果など、条件が満たされたときだけ出る一過性のカード)= 上限なし。**
  (例: ホームの「おかえりなさい」カード・再チェック催促カード、診断結果画面、オンボ結果画面。
  これらは常設ではなく「ここぞ」の瞬間に使う演出のため、L1のタブ予算にはカウントしない。)

**現在の割り当て(L1・2026-08-02時点、5タブ中4タブが使用済み):**

| タブ/画面 | 割り当て枚 | 用途 |
|---|---|---|
| 使い方 | 1 | ヒーローセクション(`GuideView.swift:175`) |
| 検索 | 1 | 「動画リクエスト」欄(`SearchView.swift:422`) |
| マイ記録 | 1 | お楽しみ機能(図鑑看板)カード(`MyRecordView.swift`・B部で新設) |
| ホーム | 0 | 意図的に空(常設部分は白一色のまま) |
| 再生リスト | 0 | 未使用 |

相談室シート(`SoudanSheetView.swift`)・オンボ結果画面は「タブ」ではなく単独の画面/シートの
ため、この表のL1予算には含めない(L2相当の一過性演出として扱う)。

**新しく`KyonoGradientCard`を追加したくなったら、まずこのルールと上表を確認し、L1タブに
既に1枚あるなら「置き換え」であって「追加」ではないことを確認すること。** 機械チェックは
`npm test`の一部として追加済み(`test/gradient-budget.test.js`。呼び出し件数が承認済みの
基準値を超えると赤くなる。iOS/Android両方の`.swift`/`.kt`ソースを対象に
`grep -c "KyonoGradientCard("`相当のカウントを行う)。

## ℹ️ Web版とネイティブの挙動が意図的にずれる: 相談室の会話が再起動をまたいで10分だけ残る(案7b・2026-07-31・本人GO)

Web版の相談室(`sdGreeted`含む会話状態)はページ再読み込みで消える、セッション内のみの設計(`index.html:3479`)。ネイティブは`TASK-C2-2026-07-30-ux-batch-13.md`第2波・案7でまず「アプリを閉じずにシートを開閉してもセッション内は残る」対応をした後、本人から「10分ほど記憶しておいてくれたらユーザー体験的にはいい」との追加要望が出て(`TASK-C2-2026-07-31-soudan-10min-memory.md`・案7b)、**アプリを完全に再起動しても、最後のやり取りから10分以内なら会話が復元される**機能を追加した(`messages`/`chipsMode`/`lastIntentId`+最終活動時刻を`RecordStore`へ保存、直近30件へトリミング、復元判定はシート初期化時のみでタイマーは持たない)。

**これはWeb版に存在しない、ネイティブだけの機能追加であり、移植漏れではない。** 本人GO済み。Web版側を同じ仕様に揃えるかどうかは未確認・本作業のスコープ外。

## ⚠️ 見出し文字列がジャンプ先のキーとして直接参照されている箇所がある(I・既存アイコン適用バッチ・2026-07-30)

`GuideData.swift`/`GuideData.kt`の`FaqGroup.title`(FAQのグループ見出し文字列)は、
表示用のラベルであると同時に、**Guide内の別ボタンから`jumpToFaq(groupTitle: "...")`で
文字列一致によって参照されるキーでもある**(該当: 「記録が消えた・0日にもどってる」
「ストレッチ中に痛かった」の2ボタン、iOS/Android各2箇所)。

アイコン化のためにtitleから絵文字を外した際、この参照側を直さないと**ビルドもテストも
通ったままジャンプボタンがサイレントに機能しなくなる**ところだった(実機で実際にボタンを
押すまで気づけない)。**見出し文字列を変更するときは、必ずこのキー参照も合わせて確認すること。**

## ℹ️ Web版とネイティブの見た目が意図的にずれる: 見出し・ボタンの絵文字をネイティブだけアイコン化(I・2026-07-30)

本人ガイドライン(2026-07-29)「端末由来の絵文字は文章中以外では使用しない。ボタンや
タブに使うアイコンはデザインに合わせて生成したものを使う」に基づき、**見出し・ボタン用途の
絵文字(34種類・のべ73箇所)を、タブバーと同じ手描き風Canvasアイコン(`KyonoIconGlyph`/
Android版同等物)に置き換える作業を開始した**(`TASK-C2-2026-07-30-icon-system.md`)。

**Web版配信ファイルは触っていない。** そのためこの作業が進むほど、見出し・ボタンの
見た目はWeb版とネイティブとで意図的にずれていく。**これは移植漏れではない。** 文章の中の
絵文字(のべ100箇所)はどちらも変更していない。

Web版を同じ方針に揃えるかどうかは本人承認待ちで、この作業には含まれていない。
...
アイコン(`TYPE_IMG_NAMES`/`TYPE_IMG`)がkoka/ashi/robotの3体ぶん欠けていたことが判明・
修正した。修正後にゴールデンテスト(iOS `card-golden` 55/55、Android `CardRendererTest`/
`BragCardRendererTest`)を実行したが、**どちらも差分ゼロだった。**

理由: `loadBundleImage`(iOS)/`loadDrawableBitmap`(Android)はどちらも実行時のアプリ本体の
リソースを見る作りで、`swift test`・Robolectricのテストランナー実行時はこの画像解決が
そもそも成立しない(常にnil/失敗を返す)。つまり**辞書のキーが3体だろうと6体だろうと、
テスト環境でのレンダリング結果は同じになる。** 今回の欠落(3タイプぶんアイコンが出ない)を
ゴールデンテストは一度も検知できていなかったし、今後この種の欠落が再発しても同様に検知
できない。

**How to apply:** 記録カードのタイプアイコン回り(画像名の追加・差し替え・削除)を触るときは、
ゴールデンテストが通ることを「問題なし」の根拠にしないこと。実機/シミュレータ・エミュレータで
実際に目視するまで検証したことにならない。直す・見張りを追加するかは判断が分かれるところ
だが、まずは「ここは自動チェックの死角である」という事実を知っておくこと。

## ⏸ 保留の記録: D1「相談室の動画カード、サムネイルが出ない」の実態(TestFlight実機フィードバック・2026-07-29)

D1の指示書は検索結果・相談室・使い方ツアーの3画面すべてを「同じサムネイル欠陥」として
扱っていたが、実際にコードを見ると**相談室だけ性質が違った**。検索結果とツアーは
`KyonoAsyncImage`を使っていて実際にバグ(`.task`が発火しない)があったが、**相談室の
動画紹介は`KyonoGhostButton("▶ 動画を見る")`という文字だけのボタンで、そもそも
サムネイル画像を描く実装が無い**(iOS `SoudanSheetView.swift`・Android
`SoudanSheet.kt`とも)。

一方Web版の`sdVideoHTML()`(index.html:3039-3046)は、サムネイル画像・カテゴリバッジ・
タイトル・説明文つきの本格的な動画カード(`vHTML()`と同じ`.video`スタイル)を出している。
つまり相談室のサムネイル欠落は「バグ」ではなく「**Web版にある機能がネイティブに移植され
ていない**」という、もう一段階前の未実装状態。今回はB1〜D6の中で最優先(D1)扱いだった
検索・ツアーの実バグだけを直し、**相談室の動画カードの新規UI実装(サムネイル付きカードを
新設)には手を付けていない**(1個所を直すだけでは済まない別スコープの作業のため)。

次に同じ指摘が来たら、まず「相談室はそもそも実装が無い」という前提から確認すること。
新設するなら`sdVideoHTML()`のマークアップ(img+badge+title+desc)を両OSで1:1移植する形になる。

## ⏸ 保留の記録: B3「保存・シェアする」ボタンの要否(TestFlight実機フィードバック・2026-07-29)

本人から一度「この項目は必要ないと思う」という声が出たが、B4(共有シートに保存系候補が
出ない欠陥)と噛み合わないため確認したところ、**「ボタンは残す。中身(共有シートの中身)を
直す」**との回答だった。**現状維持・ボタン自体には触っていない。** B4の修正(`ShareImage.swift`
の`UIActivityItemSource`化)で中身側は解消済み。今後同じ「ボタン自体が要らないのでは」という
提案が出たときは、この経緯(表面的な不要論に見えたが実際は「中身が使い物にならないから
そう見えていただけ」だった)を踏まえて判断すること。
...
(Android 267緑・Web版配信ファイル無変更・A2/A3/A6/A7/A8/A9のコミット到達を本人確認済み)。
この監査ループ自体は2026-07-29でここまで(alan5「今夜はここまでです」)。iOSズーム第2段階
(共有部品→個別画面の順次展開)はHomeView/DiaryViewの2画面まで完了、残りは次回以降継続。
以下は保留/却下の判断とその理由(将来また提案が出たときに繰り返し議論しないための記録)。

**⚠️ 残作業(次に着手する人はここから始めること)**: alan5が出した合格条件2つが**未達のまま**。
A1(行送り補正)+dex-thumb固定サイズ化で「Web版より一回り大きい」の大きな原因は潰したが、
まだ①「続けた記録」本文がWeb2行/ネイティブ3行 ②図鑑カードがWeb4枚+ボタン/ネイティブ3枚+途中
までしか収まらない、の2点が残っている。alan5が既に切り分け済みで**「原因ではない」と確認済み**
なのは: 文字サイズ(Web`#msNote{font-size:15px}`=ネイティブ`fontSize=15.sp`一致)・カード内側
余白(20px=20dp一致)・左右ガター(18dpで統一済み)・書体(両方M PLUS 1p)。同じ書体・同じサイズ・
同じ幅の箱で行数だけ違うため、**残るのは文字を並べる規則側**という仮説(alan5は断定していない):
1. **letterSpacing** — `KyonoTightLineTextStyle`(Theme.kt)にletterSpacing指定が無く、Compose既定
   がブラウザの`normal`と一致していない可能性
2. **改行規則** — Composeの`LineBreak`既定(Simple)とブラウザの禁則処理が別物の可能性。
   `LineBreak.Paragraph`等を試すと折り返し位置が変わるはず

alan5からの明示指示: 「僕のピクセル計測は当てにしないこと(今日それで矛盾した数字を出しています)。
『同じ文章が同じ行数で折り返すか』を条件に進めるのが確実です」。次の一巡はこの2点(letterSpacing/
LineBreak)の検証から着手すること。

- **⏸ 保留#10: iOS `KyonoPrimaryButton.pressed`固着リスク**——SwiftUIの`if/else`分岐で
  子ビューの生成元が変わる(`flatWhenDisabled`分岐時のみ`DragGesture`が別ブランチに付く)場合、
  タッチ中に分岐が入れ替わるとジェスチャー認識のライフサイクルが割り込まれ`pressed`が
  `true`のまま固着する可能性がある、という**未確認(SUSPECTED)**のリスク。alan5判断:
  「発生条件が極めて限定的。推測で直すと別のバグを生むほうが怖いので、いまは触らない」。
  **対応**: 記録のみ・修正は行わない。今後実機で再現報告が出たときに、まずは再現条件を
  確定させてから着手すること(推測での先回り修正は禁止)。
- **⛔ 却下#11: 同じタブ再タップでスクロール先頭に戻る動作を追加しない**——Web版
  (`index.html` switchTab)は同一タブ再タップで先頭へ戻るが、ネイティブは戻らない
  (タブ切替時のスクロール位置保持が別途実装されているため)。alan5判断:
  「Web版はそうですが、実害が薄い。むしろ対象層には『押したら勝手に戻った』と映りかねません。
  現状維持。パリティの原則から外す判断」。**対応**: 実装しない。これは
  「Web版と完全一致させる」という通常のパリティ原則から**意図的に外れる**判断であり、
  今後のパリティ監査で同じ差分を再度「欠落」として報告しないこと(既知の意図的差分として扱う)。
...
- **App Group**: `group.jp.ogatore.kyouno`はPersonal Team(FMR8VB3QLX)側で既に登録済みのため、
  ...
  選択肢2=Personal Team側の削除はしない)。
- **Bundle ID**: 同じ理由で本体`jp.ogatore.kyouno`・拡張`jp.ogatore.kyouno.widget`へ変更
  (本人判断・Android版パッケージ名と一致)。**Personal Team版とは別アプリ扱い**(記録データは
  引き継がれない・了承済み)。
- **Personal Team側に残る旧識別子(`group.jp.ogatore.kyouno`・`jp.ogatore.KyouNoOgatore`/
  `.KyonoWidget`)の掃除は今日はやらない。いつか整理する対象として残っている。**
