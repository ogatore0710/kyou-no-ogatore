...
- **follow-up課題（未実装・次タスク候補）**: #resultの`rxList`（かたさタイプ別おすすめ動画3本の
  表示）は、Web版専用の小規模動画カタログ`V`(index.html内・app-quiz.jsとは別)の新規抽出+
  ローテーションロジック移植+専用UIが必要な独立サブシステムのため、今回は実装せず個別タスク化を
  提案。
