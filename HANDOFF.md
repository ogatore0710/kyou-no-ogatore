...
## 🔶 進行中: 画面遷移アニメーション §相談室・オンボのシート化まで完了（2026-07-27）
`TASK-C2-2026-07-27-screen-transitions.md`。相談室(下からせり上がるシート)・オンボ
(中央にscale+fadeで浮き上がるカード+スクリム)の2区切りが完了。両方ともAndroid実機・iOS
シミュレータで確認済み(Screen方式は維持・外側にオーバーレイを被せるだけ)。相談室のiOS高さは
実測83.3%で確定(タスク記載の92%はWeb版の値・iOS標準シートの挙動に委ねるのが正解、と
alan5が検証・判断済み)。残りは「一般画面」の遷移アニメーション(約13画面の切替にfade/slideを
付ける)のみ。詳細はWORKING_NOTES.mdの同日エントリ参照。
