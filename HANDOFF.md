...
## 🚀 進行中: ネイティブ移植（2026-07-25・Step6完了→alan5検収待ち）
本人承認済みのストア版方針（iOS/Android・10月リリース枠。フルネイティブ移植を前倒しで即着手）を受け、[NATIVE-MIGRATION-MASTERPLAN-2026-07-25.md](NATIVE-MIGRATION-MASTERPLAN-2026-07-25.md)をappdevがWorkflowで作成→alan5検収OK→**Step0（断面固定）完了**→alan5が本人操作ゲート①（Xcodeプロジェクト作成）実施→**Step1（両OS空アプリビルド確認）完了**→**Step2（安全系テスト先行移植=最重要工程）完了**→**Step3（データ層+引っ越しインポート）完了**→**Step4（記録カード抽選・かたさ診断の決定的ロジック）完了**→**Step5a（ホーム・記録フロー・チュートリアルフラグ機械）完了**→**Step5b（カレンダー・マイ記録）完了**→**Step5c（オンボーディング・ツアー・診断UI）完了**→**Step6（相談室UI・安全系が絡む重要工程）完了**（appdev・Web版無変更・安全キーワード無変更）。Step6ではStep2で骨格のみだった`SoudanEngine`のNormal分岐（動画選定・followupチップの中身）を実装——**crisis/赤旗の判定(SafetyGateの4関数)には一切触れず、判定順序も変更していない**ことをgrep確認。SoudanSheetView/SoudanSheet（チャット・カテゴリタブ・followupチップ・14日プラン発行）を両OSに実装し、Androidはkyono_testエミュレータで一連の実タップ検証（intentチップ→動画/followup表示→14日プラン開始→ホーム進捗カード表示）を実施。安全系テスト(111+engine-fixtures、検収基準の実例文言を新規テスト化)は両OSとも回帰なし（Android196/196・iOS SafetyCore8/8）。詳細はWORKING_NOTES.md 2026-07-25エントリ。次はStep7a（検索・再生リスト・図鑑）でalan5の指示待ち。
