...
## 🚀 進行中: ネイティブ移植 → 見た目のWeb版パリティ移植（2026-07-26・Phase 1〜3+仕上げ2件完了。次はalan5の検収・次タスク待ち）
本人承認済みのストア版方針（iOS/Android・10月リリース枠。フルネイティブ移植を前倒しで即着手）を受け、[NATIVE-MIGRATION-MASTERPLAN-2026-07-25.md](NATIVE-MIGRATION-MASTERPLAN-2026-07-25.md)をappdevがWorkflowで作成→alan5検収OK→**Step0〜Step7b完了**（ロジック・データ・安全系は1:1で忠実に移植・テスト裏取り済み。詳細はWORKING_NOTES.md参照）。Step7bの直後、alan5が実機で見た瞬間「PWA版と全然違う」と気づき、記録カードの視覚アセット完成タスク（Step4/7bの積み残し5項目=キャラ立ち絵・カード柄画像・タグピル・フッター文言・実フォント。**完了**）に続けて、**`TASK-C2-2026-07-26-native-visual-design-parity.md`（アプリ全体の見た目Web版パリティ移植・最優先）**を発注。ロジック・機能はWeb版と忠実に一致していたが見た目（テーマ・配色・フォント・カード型コンポーネント・下部タブバー・手描き風アイコン）が丸ごと未実装だったと判明したため、デザインシステムを作って全画面へ適用する規模のタスク。**Phase 1〜3すべて完了**: Theme.kt/Theme.swift（Web版CSS変数から抽出した配色）・KyonoComponents.kt/.swift（Card型コンテナ・box-shadow立体ボタン）・KyonoTabBar.kt/.swift（下部タブバー5項目+FAB2段）・KyonoIcons.kt/.swift（sec-head手描き風SVGアイコン14種をCanvas/Pathで直接再現。Material Icons/SF Symbols代替は不可のため）を新設し、**アプリの全画面**（ホーム・マイ記録・使い方・設定・相談室・診断結果・図鑑・じまんカード・せんぱいの声・オガトレ通信・動画を探す・再生リスト・オンボーディング・かたさチェック・使い方ツアー）をカード型レイアウト+Web版デザイントークンへ作り替え。**続けて仕上げタスク2件も完了**: ①タブバー画面から冗長な「◀ もどる」ボタンを削除、②本人実機指摘（フォントがM PLUS 1pになっていない・オガトレくん画像とかたさタイプ画像がアプリ全体で欠落）を受けてMaterialTheme/Typography層でのフォント一括適用+`KyonoCharaImage`/`KyonoTypeArt`新設によるキャラクター画像・かたさタイプ画像（6種、うち3種はCanvas SVG直書き）の移植。Android実機・iOSシミュレータでライト/ダーク双方・タブ切替・オンボ→かたさチェック→診断結果→ホーム→使い方ツアー→相談室チャットの一気通貫フローの実タップ動作を確認。実機検証中に発見した表示崩れ（KyonoCardのBox/Column不整合・とどくメーター5段ボタンの折り返し崩れ・FABと固定末尾テキストの重なり等）は全て修正済み。安全系テスト（111+engine-fixtures）・全ステップのテストは両OSとも回帰なし。詳細はWORKING_NOTES.md 2026-07-26エントリ（Phase 1〜3+仕上げ2件の5本）。次はalan5の検収と次タスク待ち（Step8=9月頭の差分同期は別枠で保留中）。
