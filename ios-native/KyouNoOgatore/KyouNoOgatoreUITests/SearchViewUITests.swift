...
final class SearchViewUITests: XCTestCase {
    ...
    // DO-NOT-COMMIT/TEMP-TEST: TASK-C2-2026-07-30-onboarding-scroll-and-copy.md A1/A2/A3の
    // 目視確認用スクラッチ。screenshot抽出後にこのメソッドごと削除する。
    func testTempOnboardingFlow() throws {
        let app = XCUIApplication()
        app.launch()
        // Q1 bigtext
        let big = app.staticTexts["大きめ（いまのまま）"]
        XCTAssertTrue(big.waitForExistence(timeout: 15))
        big.tap()
        // Q2 stiff
        let hard = app.staticTexts["ガチガチかも"]
        XCTAssertTrue(hard.waitForExistence(timeout: 10))
        hard.tap()
        ...
        add(a1)
        // Q3 worry
        let waist = app.staticTexts["腰"]
        XCTAssertTrue(waist.waitForExistence(timeout: 10))
        waist.tap()
        // Q4 anchor
        let free = app.staticTexts["きめてない"]
        ...
        add(a2)
        // Tap the route CTA (KyonoPrimaryButton in the fixed footer). KyonoPrimaryButton is a
        // 2-layer ZStack (transparent shadow layer + visible layer) so the label matches twice;
        // use firstMatch to disambiguate.
        let cta = app.staticTexts["かたさチェックをはじめる"].firstMatch
        if cta.waitForExistence(timeout: 5) { cta.tap() }
        sleep(1)
        let a3 = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        a3.name = "ob-after-cta"
        a3.lifetime = .keepAlways
        add(a3)
        // Quiz (presetWorry済みなのでworry問は出ない: momo/koka/kenko/ashiの4問)。
        let quizOptLabels = ["床にペタッとつく", "床にペタッと近い", "鼻より上まで上がる", "余裕でしゃがめる"]
        for (i, label) in quizOptLabels.enumerated() {
            let opt = app.staticTexts[label].firstMatch
            XCTAssertTrue(opt.waitForExistence(timeout: 10), "Quiz option not found: \(label)")
            if i == 0 {
                let a4 = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
                a4.name = "quiz-q1"
        ...
        sleep(2)
        let a5 = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        a5.name = "quiz-back-home-buttons"
        a5.lifetime = .keepAlways
        add(a5)
        // Result screen (fdGuideActive expected: fresh install, total=0, fd unset)
        sleep(1)
        let a6 = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        a6.name = "result-fdguide-top"
        a6.lifetime = .keepAlways
        add(a6)
        let scroll = app.scrollViews.firstMatch
        scroll.swipeUp(velocity: .fast)
        sleep(1)
        let a7 = XCTAttachment(screenshot: XCUIScreen.main.screenshot())
        a7.name = "result-fdguide-scrolled"
        a7.lifetime = .keepAlways
        add(a7)
    }
    ...
    //
    // 既知の限界(正直に書く): 本人の元の報告はダークモードだった(タブバーのダーク背景色
    // #211E19より「もっと黒い」という言葉が根拠)。実測したところ、①ライトモードでは画面本体
    // 側の背景(KyonoBackgroundColor)が既にその領域を塗っているため黒く見えず、②アプリ内の
    // 「続ける設定→暗い」を切り替えても`KyonoTheme`は`.preferredColorScheme`を設定していない
    // ため система側のtrait(.regularMaterial等のシステム描画に効く)は変わらず、システムの
    // ダークモードでなければこの欠陥は再現しない。テストプロセス自体はシミュレータ内でサンド
    // ボックスされておりsimctlを呼べないため、**この検査だけは実行前に手動で
    // `xcrun simctl ui <device> appearance dark` を行う必要がある**(HANDOFF.md参照)。
    // 通常のCI実行(既定=ライトモード)では、この検査はダークモード限定のC1再発を捕まえられない。
