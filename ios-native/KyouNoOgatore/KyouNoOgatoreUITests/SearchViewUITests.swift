...
final class SearchViewUITests: XCTestCase {
    ...
    // TEMP-TEST(DO NOT COMMIT): TASK-C2-2026-07-30-button-standard-migration.md 診断2の再現手順の
    // 再テスト。REPORT-C2-2026-07-30-button-feel-diagnosis.mdと同じ「マイ記録→設定をひらく」を
    // 対象に、押してから画面内を大きくドラッグして離す操作を行い、標準Button化後は
    // action()(=設定画面が開く)が発火しないことを確認する。検証後は本メソッドごと削除する。
    func testTempButtonDragAwayNoLongerFires() throws {
        let app = XCUIApplication()
        app.launch()
        ...
        XCTAssertFalse(settingsHeader.waitForExistence(timeout: 2), "標準Button化後もaction()が発火し、設定画面が開いてしまった(診断2の再現手順で発火しないことの確認に失敗)")
    }

    // TEMP-TEST(DO NOT COMMIT・調査用): 標準Buttonのベースライン確認。KyonoTabBar.swift:64は
    // 今回の移行対象外の既存の標準Button実装。同じドラッグして離す手順でこちらも発火するなら、
    // 「標準Buttonはドラッグアウトでキャンセルされる」という前提自体がこの環境で成り立って
    // いない可能性がある(切り分け用・本題ではないので検証後は削除する)。
    func testTempBaselineStandardButtonDragAway() throws {
        let app = XCUIApplication()
        app.launch()
        ...
        let dragStart = myRecordTab.coordinate(withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
        let dragEnd = app.coordinate(withNormalizedOffset: CGVector(dx: 0.5, dy: 0.1))
        dragStart.press(forDuration: 0.3, thenDragTo: dragEnd)

        sleep(1)
        // マイ記録タブに実際に切り替わっていれば「設定をひらく」が見えるはず。
        let settingsBtn = app.descendants(matching: .any)["設定をひらく"].firstMatch
        print("BASELINE_FIRED=\(settingsBtn.exists)")
        XCTAssertFalse(settingsBtn.exists, "既存の標準Button(タブバー)もドラッグアウトでキャンセルされず遷移してしまった(切り分け用ベースライン)")
    }

    // TEMP-TEST(DO NOT COMMIT): 診断1(スクロール中の沈み込み)の再確認。ボタンの真上から
    // スワイプを開始し、(1)スクロールが通常どおり効くこと (2)スワイプ操作がaction()を
    // 誤発火させないことを確認する。検証後は本メソッドごと削除する。
    func testTempButtonScrollFromButtonTop() throws {
        let app = XCUIApplication()
        app.launch()
        ...
        sleep(1)
        let settingsHeader = app.staticTexts["続ける設定"].firstMatch
        XCTAssertFalse(settingsHeader.waitForExistence(timeout: 2), "ボタン直上からのスワイプでaction()が誤発火し、設定画面が開いてしまった")
    }
}
