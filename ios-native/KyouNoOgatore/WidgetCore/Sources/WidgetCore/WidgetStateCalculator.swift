...
//
//  Fable監査GO-14(alan5差し戻し2026-07-28): 以前はKyonoWidgetExtensionターゲット直下のソースで、
//  swift testに乗る手段が無く「手元でswiftcして確認した」一回きりの検証で終わっていた。
//  RecordCore等と同じくSwift Packageへ寄せ、Tests/配下でコミット済みの回帰テストとして残す。
//

import Foundation
import RecordCore

public enum CharaAsset: String {
...
public struct WidgetDisplayState {
    public let chara: CharaAsset
...
public enum WidgetStateCalculator {
    public static func compute(summary: WidgetSummary?, at date: Date) -> WidgetDisplayState {
