...
private struct ResultContentView: View {
    ...
    private func performPracticeRecord() {
        ...
        // markDone/recordDaylog/通知resync/ウィジェット書き出しを行わず、演出(0日目カード・紙吹雪)
        // だけを出す。本物の1日目は、ツアー後に通常の「きょうやった!」を押した時点で初めて記録される。
        // ツアー進行フラグ(fd/tourpend)は進行制御に使うため従来どおり残す。
        // (旧仕様=実記録して表示だけ0日はR-13/build27。2026-08-06「現仕様維持」裁定を本人が
        //  build41実機確認「ツアー終わってすぐやった事になってる」で上書き)
        let streak = RecordLogic.loadStreak(store)
        let today = RecordLogic.todayStr(now: Date())
        store.set("fd", "1")
        store.set("tourpend", true)
        // 0日目カードは演出のみ(streakは未記録のまま=total 0)。rotAssignの抽選台帳だけは
        // renderTodayCard内で追記されるが、実記録日に本物の記録をしたとき同じ絵柄が再現される
        // だけで、図鑑解放・日数には影響しない(解放は記録済み日付から導出される)。
